...
| `scripts/critical_path_nsys.py` | new     | Per-stream / per-NCCL-kernel breakdown of an `nsys` trace: classifies each NCCL kernel as "with concurrent compute on another stream" vs "on the critical path", lists the busy streams, and rasterizes a 1–2-iter ASCII timeline to expose where comm sits relative to compute. Used to produce sections 7.4a and §8.2d's exposed-NCCL numbers. |
| `scripts/measure_gpu_idle.py`   | new     | Reads an `nsys` SQLite export, merges per-GPU kernel intervals across all streams, reports busy/idle ms and idle %, and dumps a `cudaGraphLaunch` p50/p90/p99/max histogram. Used to produce §8.2a (virtualization validation) and §8.2d (winning-config breakdown). |
| `scripts/measure_kernel_launch_lat.py` | new     | Companion to `measure_gpu_idle.py`: per-stream inter-kernel gap percentiles (p50/p90/p99), cudaLaunchKernel host-side duration distribution, and direct AllReduce-to-AllReduce iter-cycle timing on GPU 0. |
| `scripts/nsys_to_perfetto_gpu0.py` | new     | Converts one GPU's events from an `nsys` SQLite export to a Chrome / Perfetto JSON trace (drag-drop into <https://ui.perfetto.dev>). Per-stream lanes, host-side CUDA API calls. Whole-window export — produces ~40 MB JSON for a 4-second trace. |
| `scripts/nsys_to_perfetto_annotated.py` | new     | Smarter version of the above: filters to N consecutive iterations (uses `ncclAllReduce` as iter marker), and **annotates every kernel with its DLRM-DCNv2 semantic role** — `[emb_a2a]`, `[emb_fwd]`, `[mlp_fwd]`, `[mlp_bwd_dgrad]`, `[allreduce]`, `[opt_emb]`, `[loss]`, etc. — visible as the kernel-name prefix and as Perfetto category colors. Produces a ~200 KB JSON for 3 iterations that loads instantly in Perfetto. |
| `scripts/nsys_to_perfetto_annotated_v2.py` | new     | PyTorch-profiler-style enhancement of the above. Adds: (1) per-kernel grid/block/regs-per-thread/static+dynamic shared-memory in tooltip args (visible by clicking any kernel); (2) **host-launch → device-kernel arrows** via CUPTI `correlationId` (Chrome trace `ph:"s"`/`"f"` flow events; click a kernel and Perfetto highlights the originating `cudaLaunchKernel` / `cuLaunchKernelEx` on the host row); (3) **fwd/bwd pair arrows** within each iter (uses BCE loss as the fwd→bwd boundary, then LIFO-pairs `mlp_fwd`↔`mlp_bwd_dgrad`, `interaction_fwd`↔`interaction_bwd`, `emb_fwd`↔`emb_scatter`, `emb_a2a_fwd`↔`emb_a2a_bwd`). For the bs=1× best-perf trace this adds ~187 launch arrows and ~27 fwd/bwd arrows for 3 iters in a 325 KB JSON. |
