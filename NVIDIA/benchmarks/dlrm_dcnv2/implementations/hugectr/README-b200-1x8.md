...
| `scripts/nsys_to_perfetto_annotated.py` | new     | PyTorch-profiler-style smart converter. Filters to N consecutive iterations (uses `ncclAllReduce` as the iter marker) and produces an annotated Perfetto/Chrome trace JSON with: (1) **DLRM-DCNv2 semantic role** on every kernel — `[emb_a2a]`, `[emb_fwd]`, `[mlp_fwd]`, `[mlp_bwd_dgrad]`, `[allreduce]`, `[opt_emb]`, `[loss]`, etc. — visible as the kernel-name prefix and as Perfetto category colors; (2) per-kernel **grid / block / regs-per-thread / static + dynamic shared-memory** in tooltip args (click any kernel); (3) **host-launch → device-kernel arrows** via CUPTI `correlationId` (Chrome flow events; click a kernel and Perfetto highlights the originating `cudaLaunchKernel` / `cuLaunchKernelEx` on the host row); (4) **fwd/bwd pair arrows** within each iter (uses BCE loss as the fwd→bwd boundary, LIFO-pairs `mlp_fwd`↔`mlp_bwd_dgrad`, `interaction`↔`interaction`, `emb_fwd`↔`emb_scatter`, `emb_a2a_fwd`↔`emb_a2a_bwd`); (5) iter boundary instant markers. For the bs=1× best-perf trace this produces a 325 KB JSON for 3 iters (424 kernels + 234 host_api + 187 launch arrows + 27 fwd/bwd arrows + 4 iter markers) that loads instantly in Perfetto. |
