...
Usage:
    python3 nsys_to_perfetto_annotated.py <input.sqlite> <gpu_id> \
        [first_iter] [n_iters] [output.json]

    python3 nsys_to_perfetto_annotated.py /r/nsys_bs1x_auto.sqlite 0 1000 3 /r/out.json
"""
import sqlite3
import sys
import json
...
OUT        = sys.argv[5] if len(sys.argv) > 5 else \
             f"{os.path.splitext(DB)[0]}.gpu{GPU}.iter{FIRST_ITER}-{FIRST_ITER + N_ITERS}.json"
