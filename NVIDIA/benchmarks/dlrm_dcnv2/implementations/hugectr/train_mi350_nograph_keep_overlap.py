...
# Opt-in Python frame tracer for the Tier-B path of the trace analysis
# pipeline. Enabled by HCTR_PY_TRACE=1; writes a Chrome-trace sidecar to
# /tmp/py_trace_rank{rank}.json that nsys_to_perfetto_annotated.py merges
# via --py-trace=PATH. Zero overhead when disabled (env var unset).
if os.environ.get("HCTR_PY_TRACE") == "1":
    # Path manipulation so this works whether invoked as `python train_mi350.py`
    # from /workspace/dlrm (Docker working dir) or as a module.
    _here = os.path.dirname(os.path.abspath(__file__))
    if os.path.join(_here, "scripts") not in sys.path:
        sys.path.insert(0, os.path.join(_here, "scripts"))
    import hctr_py_tracer  # type: ignore
    hctr_py_tracer.maybe_autostart()
...
# async_wgrad=False -> MLP wgrad runs on the main computation_stream_ instead
# of computation_stream_2_, collapsing the wgrad lane into the main one.
# This matches MI350X's stream layout (main carries mlp_fwd + mlp_bwd_dgrad +
# mlp_bwd_wgrad together).
