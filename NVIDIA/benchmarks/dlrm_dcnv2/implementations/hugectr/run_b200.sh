...
docker run --rm \
    ...
    -e LD_PRELOAD \
    -e MALLOC_ARENA_MAX -e MALLOC_TRIM_THRESHOLD_ -e MALLOC_TOP_PAD_ \
    -e CUDA_MODULE_LOADING -e CUDA_AUTO_BOOST \
    -e HUGECTR_DISABLE_OVERFLOW_CHECK -e HUGECTR_LOG_LEVEL \
    '${IMAGE}' \
    bash -c '
