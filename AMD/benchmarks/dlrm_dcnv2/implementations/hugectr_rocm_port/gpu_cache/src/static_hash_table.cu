...
#pragma unroll
    for (size_t i = lane_idx; i < emb_vec_size_in_float; i += warp_size) {
      d_dst[i] = out_value_type(float(__hip_fp8_e4m3_fnuz(d_src[i])) * scale);
    }
  } else {
#pragma unroll
    for (size_t i = lane_idx; i < emb_vec_size_in_float / 4; i += warp_size) {
      ...
      float4 float4Tmp;
      float4Tmp.x = ((float)reinterpret_cast<const __hip_fp8_e4m3_fnuz *>(&tmp.x)[0]) * scale;
      float4Tmp.y = ((float)reinterpret_cast<const __hip_fp8_e4m3_fnuz *>(&tmp.y)[0]) * scale;
      float4Tmp.z = ((float)reinterpret_cast<const __hip_fp8_e4m3_fnuz *>(&tmp.z)[0]) * scale;
      float4Tmp.w = ((float)reinterpret_cast<const __hip_fp8_e4m3_fnuz *>(&tmp.w)[0]) * scale;
...
// Allocate embedding on host
  ...
  if (n != 0) {
    ...
    CUDA_CHECK(hipHostRegister(p, n, hipHostRegisterMapped));
    table_values_ = static_cast<value_type *>(p);
  }
#else
  if (enable_pagelock) {
    CUDA_CHECK(
        hipHostAlloc(&table_values_, sizeof(value_type) * num_values, hipHostMallocPortable));
  } else {
    CUDA_CHECK(hipMalloc(&table_values_, sizeof(value_type) * num_values));
  }
  if constexpr (nv::is_fp8<value_type>::value) {
    CUDA_CHECK(hipMalloc(&quant_scales_, sizeof(float) * num_keys));
  }
#endif
...
void StaticHashTable<key_type, value_type, out_value_type, tile_size, group_size, hasher>::insert(
  ...
  CUDA_CHECK(hipMemcpyAsync(table_values_ + size_ * value_dim_, values,
                             sizeof(value_type) * num_keys * value_dim_, hipMemcpyDefault,
                             stream));

  if constexpr (nv::is_fp8<value_type>::value) {
    CUDA_CHECK(hipMemcpyAsync(quant_scales_ + size_, quant_scales, sizeof(float) * num_keys,
...
#ifdef USE_HUGE_PAGES
  if (table_values_) {
    CUDA_CHECK(hipHostUnregister(table_values_));
    size_t align_m = 16;
    size_t num_values = (value_capacity_ * value_dim_ + align_m - 1) / align_m * align_m;
    size_t n = sizeof(value_type) * num_values;
    constexpr size_t page_size_mb{2};
...
#else
  if constexpr (nv::is_fp8<value_type>::value) {
    CUDA_CHECK(hipFree(quant_scales_));
  } else {
    if (enable_pagelock)
      CUDA_CHECK(hipHostFree(table_values_))
    else
...
template class StaticHashTable<uint32_t, __hip_fp8_e4m3_fnuz, float>;
template class StaticHashTable<uint32_t, __hip_fp8_e4m3_fnuz, __hip_fp8_e4m3_fnuz>;
template class StaticHashTable<long long, float, float>;
template class StaticHashTable<long long, __hip_fp8_e4m3_fnuz, float>;
template class StaticHashTable<long long, __hip_fp8_e4m3_fnuz, __hip_fp8_e4m3_fnuz>;
}  // namespace gpu_cache
