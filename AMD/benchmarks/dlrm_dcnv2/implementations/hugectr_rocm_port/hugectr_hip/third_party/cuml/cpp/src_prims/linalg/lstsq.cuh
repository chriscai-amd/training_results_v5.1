...
void lstsqSVD(math_t *A, int n_rows, int n_cols, math_t *b, math_t *w,
              hipsolverHandle_t cusolverH, hipblasHandle_t cublasH,
...
void lstsqQR(math_t *A, int n_rows, int n_cols, math_t *b, math_t *w,
  ...
  CUDA_CHECK(hipMemcpyAsync(&info, d_info.data(), sizeof(int),
  ...
  CUBLAS_CHECK(cublastrsm(cublasH, side, HIPBLAS_FILL_MODE_UPPER, HIPBLAS_OP_N,
                          HIPBLAS_DIAG_NON_UNIT, n, 1, &one, A, lda, b, ldb,
                          stream));

  CUDA_CHECK(hipMemcpyAsync(w, b, sizeof(math_t) * n, hipMemcpyDeviceToDevice,
                             stream));
}
