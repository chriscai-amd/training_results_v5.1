...
__global__ void mutualInfoKernel(const int *dContingencyMatrix, const int *a,
  ...
  typedef hipcub::BlockReduce<double, BLOCK_DIM_X,
                           hipcub::BLOCK_REDUCE_WARP_REDUCTIONS, BLOCK_DIM_Y>
    BlockReduce;
...
double mutualInfoScore(const T *firstClusterArray, const T *secondClusterArray,
  ...
  CUDA_CHECK(
    hipMemsetAsync(a.data(), 0, numUniqueClasses * sizeof(int), stream));
  CUDA_CHECK(
    hipMemsetAsync(b.data(), 0, numUniqueClasses * sizeof(int), stream));
  CUDA_CHECK(hipMemsetAsync(d_MI.data(), 0, sizeof(double), stream));
