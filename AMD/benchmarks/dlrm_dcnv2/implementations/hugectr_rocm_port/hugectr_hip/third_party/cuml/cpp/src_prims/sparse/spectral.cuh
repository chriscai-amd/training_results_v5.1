...
template <typename T>
void coo2csr(hipsparseHandle_t handle, const int *srcRows, const int *srcCols,
  ...
  CUDA_CHECK(hipMemcpyAsync(dstRows.data(), srcRows, sizeof(int) * nnz,
                             hipMemcpyDeviceToDevice, stream));
  CUDA_CHECK(hipMemcpyAsync(dstCols, srcCols, sizeof(int) * nnz,
                             hipMemcpyDeviceToDevice, stream));
  ...
  CUSPARSE_CHECK(hipsparseCreateIdentityPermutation(handle, nnz, P.data()));
...
template <typename T>
void fit_embedding(hipsparseHandle_t handle, int *rows, int *cols, T *vals,
