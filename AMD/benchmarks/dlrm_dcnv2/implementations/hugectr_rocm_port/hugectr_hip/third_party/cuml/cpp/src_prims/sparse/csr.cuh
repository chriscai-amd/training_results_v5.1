...
class CSR {
 ...
 public:
  ...
  /**
    ...
    */
  CSR(std::shared_ptr<deviceAllocator> d_alloc, hipStream_t stream)
  ...
  void init_arrays(hipStream_t stream) {
    CUDA_CHECK(hipMemsetAsync(this->row_ind_arr.data(), 0,
                               this->n_rows + 1 * sizeof(Index_Type), stream));
    CUDA_CHECK(hipMemsetAsync(this->row_ind_ptr_arr.data(), 0,
...
void csr_row_normalize_l1(const int *ia,  // csr row ex_scan (sorted by row)
                          ...
                          int m,    // num rows in csr
                          T *result,
                          hipStream_t stream) {  // output array
...
size_t csr_add_calc_inds(const int *a_ind, const int *a_indptr, const T *a_val,
  ...
  CUDA_CHECK(
    hipMemsetAsync(row_counts.data(), 0, (m + 1) * sizeof(int), stream));
...
void csr_add_finalize(const int *a_ind, const int *a_indptr, const T *a_val,
                      ...
                      int *c_indptr, T *c_val, hipStream_t stream) {
  dim3 grid(MLCommon::ceildiv(m, TPB_X), 1, 1);
  dim3 blk(TPB_X, 1, 1);
...
void csr_adj_graph_batched(const Index_ *row_ind, Index_ total_rows, Index_ nnz,
                           ...
                           Index_ *row_ind_ptr, hipStream_t stream,
                           Lambda fused_op) {
...
void csr_adj_graph(const Index_ *row_ind, Index_ total_rows, Index_ nnz,
                   const bool *adj, Index_ *row_ind_ptr, hipStream_t stream,
                   Lambda fused_op) {
...
void weak_cc_label_batched(Index_ *labels, const Index_ *row_ind,
                           ...
                           Index_ batchSize, hipStream_t stream,
                           Lambda filter_op) {
  ...
  int n_iters = 0;
  do {
    CUDA_CHECK(hipMemsetAsync(state->m, false, sizeof(bool), stream));
...
void weak_cc_batched(Index_ *labels, const Index_ *row_ind,
                     ...
                     hipStream_t stream, Lambda filter_op) {
