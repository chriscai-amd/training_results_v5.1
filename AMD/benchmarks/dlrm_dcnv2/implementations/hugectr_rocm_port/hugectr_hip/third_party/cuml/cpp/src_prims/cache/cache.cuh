...
*
* // Assume we have cumlHandle_impl& h, and hipStream_t stream
...
class Cache {
 public:
   ...
   */
  Cache(std::shared_ptr<deviceAllocator> allocator, hipStream_t stream,
    ...
    if (n_cache_vecs >= associativity) {
      ...
      CUDA_CHECK(hipMemsetAsync(cached_keys.data(), 0,
                                 cached_keys.size() * sizeof(int), stream));
      CUDA_CHECK(hipMemsetAsync(cache_time.data(), 0,
  ...
  */
  void StoreVecs(const math_t *tile, int n_tile, int n, int *cache_idx,
                 hipStream_t stream, const int *tile_idx = nullptr) {
    if (n > 0) {
  ...
  void GetCacheIdxPartitioned(int *keys, int n, int *cache_idx, int *n_cached,
    ...
    hipcub::DevicePartition::Flagged(d_temp_storage.data(), d_temp_storage_size,
  ...
  void AssignCacheIdx(int *keys, int n, int *cidx, hipStream_t stream) {
    if (n <= 0) return;
    hipcub::DeviceRadixSort::SortPairs(d_temp_storage.data(), d_temp_storage_size,
    ...
    // set it to -1
    CUDA_CHECK(hipMemsetAsync(cidx, 255, n * sizeof(int), stream));
    ...
    CUDA_CHECK(hipPeekAtLastError());
    if (debug_mode) CUDA_CHECK(hipDeviceSynchronize());
  }
