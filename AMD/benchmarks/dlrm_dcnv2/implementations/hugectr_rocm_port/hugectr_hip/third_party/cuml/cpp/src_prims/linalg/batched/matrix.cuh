...
class Matrix {
 ...
 public:
   ...
   */
  Matrix(int m, int n, int batch_size, hipblasHandle_t cublasHandle,
         std::shared_ptr<ML::deviceAllocator> allocator, hipStream_t stream,
         bool setZero = true)
  ...
  hipblasHandle_t cublasHandle() const { return m_cublasHandle; }

  //! Return allocator
  std::shared_ptr<deviceAllocator> allocator() const { return m_allocator; }

  //! Return stream
  hipStream_t stream() const { return m_stream; }
  ...
  */
   ...
   */
  static Matrix<T> Identity(int m, int batch_size, hipblasHandle_t cublasHandle,
...
void b_gemm(bool aT, bool bT, int m, int n, int k, T alpha, const Matrix<T>& A,
  ...
  // Set transpose modes
  hipblasOperation_t opA = aT ? HIPBLAS_OP_T : HIPBLAS_OP_N;
  hipblasOperation_t opB = bT ? HIPBLAS_OP_T : HIPBLAS_OP_N;
...
void b_gels(const Matrix<T>& A, Matrix<T>& C) {
  ...
  CUBLAS_CHECK(LinAlg::cublasgelsBatched(
    A.cublasHandle(), HIPBLAS_OP_N, m, n, nrhs, Acopy.data(), m, C.data(), m,
...
void b_hessenberg(const Matrix<T>& A, Matrix<T>& U, Matrix<T>& H) {
  ...
  CUDA_CHECK(
    hipMemsetAsync(U.raw_data(), 0, sizeof(T) * n2 * batch_size, stream));
  identity_matrix_kernel<T>
