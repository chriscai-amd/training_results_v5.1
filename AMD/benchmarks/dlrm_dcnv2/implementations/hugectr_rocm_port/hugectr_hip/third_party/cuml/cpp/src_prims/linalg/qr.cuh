...
void qrGetQ(math_t *M, math_t *Q, int n_rows, int n_cols,
  ...
  int k = min(m, n);
  CUDA_CHECK(hipMemcpyAsync(Q, M, sizeof(math_t) * m * n,
                             hipMemcpyDeviceToDevice, stream));

  device_buffer<math_t> tau(allocator, stream, k);
  CUDA_CHECK(hipMemsetAsync(tau.data(), 0, sizeof(math_t) * k, stream));
...
void qrGetQR(math_t *M, math_t *Q, math_t *R, int n_rows, int n_cols,
  ...
  CUDA_CHECK(
    hipMemsetAsync(tau.data(), 0, sizeof(math_t) * min(m, n), stream));
  int R_full_nrows = m, R_full_ncols = n;
  CUDA_CHECK(hipMemcpyAsync(R_full.data(), M, sizeof(math_t) * m * n,
                             hipMemcpyDeviceToDevice, stream));
  ...
  CUDA_CHECK(hipMemcpyAsync(Q, R_full.data(), sizeof(math_t) * m * n,
                             hipMemcpyDeviceToDevice, stream));
