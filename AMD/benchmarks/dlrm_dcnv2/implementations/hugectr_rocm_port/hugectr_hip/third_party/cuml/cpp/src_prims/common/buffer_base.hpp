...
class buffer_base {
 public:
  ...
  buffer_base(std::shared_ptr<Allocator> allocator, hipStream_t stream,
              size_type n = 0)
    : _size(n),
      _capacity(n),
      _data(nullptr),
  ...
  void reserve(const size_type new_capacity, hipStream_t stream) {
    set_stream(stream);
    if (new_capacity > _capacity) {
      value_type* new_data = static_cast<value_type*>(
        _allocator->allocate(new_capacity * sizeof(value_type), _stream));
      if (_size > 0) {
        CUDA_CHECK(hipMemcpyAsync(new_data, _data, _size * sizeof(value_type),
                                   hipMemcpyDefault, _stream));
      }
      if (nullptr != _data) {
        _allocator->deallocate(_data, _capacity * sizeof(value_type), _stream);
      }
  ...
  void resize(const size_type new_size, hipStream_t stream) {
 ...
 private:
  ...
  void set_stream(hipStream_t stream) {
    if (_stream != stream) {
      ...
      CUDA_CHECK(hipEventRecord(event, _stream));
      CUDA_CHECK(hipStreamWaitEvent(stream, event, 0));
      _stream = stream;
