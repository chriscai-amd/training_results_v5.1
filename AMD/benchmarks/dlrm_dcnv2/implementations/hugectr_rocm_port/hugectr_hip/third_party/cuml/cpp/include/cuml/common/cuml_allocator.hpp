...
class deviceAllocator {
 public:
   ...
   */
  virtual void* allocate(std::size_t n, hipStream_t stream) = 0;
   ...
   */
  virtual void deallocate(void* p, std::size_t n, hipStream_t stream) = 0;
...
/** Default hipMalloc/hipFree based device allocator */
class defaultDeviceAllocator : public deviceAllocator {
 public:
   ...
   */
  virtual void* allocate(std::size_t n, hipStream_t stream) {
    void* ptr = 0;
    CUDA_CHECK(hipMalloc(&ptr, n));
    return ptr;
  }
   ...
   */
  virtual void deallocate(void* p, std::size_t n, hipStream_t stream) {
    CUDA_CHECK_NO_THROW(hipFree(p));
  }

  virtual ~defaultDeviceAllocator() {}
};

/** Default hipHostMalloc/hipHostFree based host allocator */
class defaultHostAllocator : public hostAllocator {
 public:
  ...
  virtual void* allocate(std::size_t n, hipStream_t stream) {
    void* ptr = 0;
    CUDA_CHECK(hipHostMalloc(&ptr, n));
    return ptr;
  }
   ...
   */
  virtual void deallocate(void* p, std::size_t n, hipStream_t stream) {
    CUDA_CHECK_NO_THROW(hipHostFree(p));
  }
