...
struct KVPMinReduce {
  typedef hipcub::KeyValuePair<LabelT, DataT> KVP;
...
struct MinAndDistanceReduceOp {
  typedef typename hipcub::KeyValuePair<LabelT, DataT> KVP;
...
struct FusedL2NN : public BaseClass {
 private:
  ...
  DataT *sxNorm, *syNorm;
  hipcub::KeyValuePair<IdxT, DataT>* sRed;
 ...
 public:
  DI FusedL2NN(OutT* _min, const DataT* _x, const DataT* _y, const DataT* _xn,
    ...
    : BaseClass(_x, _y, _m, _n, _k, _smem),
      ...
      sRed((hipcub::KeyValuePair<IdxT, DataT>*)_smem),
      maxVal(_mv),
      redOp(op) {}
...
#pragma unroll
    ...
    // reduce
    hipcub::KeyValuePair<IdxT, DataT> val[P::AccRowsPerTh];
...
#pragma unroll
      for (int j = 0; j < P::AccColsPerTh; ++j) {
        ...
        hipcub::KeyValuePair<IdxT, DataT> tmp = {tmpkey, acc[i][j]};
...
#pragma unroll
      for (int j = P::AccThCols / 2; j > 0; j >>= 1) {
        ...
        hipcub::KeyValuePair<IdxT, DataT> tmp = {tmpkey, tmpvalue};
