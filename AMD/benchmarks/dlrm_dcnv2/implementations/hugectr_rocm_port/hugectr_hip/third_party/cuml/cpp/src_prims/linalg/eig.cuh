...
void eigDC(const math_t *in, int n_rows, int n_cols, math_t *eig_vectors,
           math_t *eig_vals, hipsolverHandle_t cusolverH, hipStream_t stream,
           std::shared_ptr<deviceAllocator> allocator) {
  int lwork;
  CUSOLVER_CHECK(cusolverDnsyevd_bufferSize(cusolverH, HIPSOLVER_EIG_MODE_VECTOR,
                                            HIPBLAS_FILL_MODE_UPPER, n_rows, in,
  ...
  CUSOLVER_CHECK(cusolverDnsyevd(cusolverH, HIPSOLVER_EIG_MODE_VECTOR,
                                 HIPBLAS_FILL_MODE_UPPER, n_rows, eig_vectors,
...
void eigSelDC(math_t *in, int n_rows, int n_cols, int n_eig_vals,
              ...
              hipsolverHandle_t cusolverH, hipStream_t stream,
              std::shared_ptr<deviceAllocator> allocator) {
  int lwork;
  int h_meig;

  CUSOLVER_CHECK(cusolverDnsyevdx_bufferSize(
    cusolverH, HIPSOLVER_EIG_MODE_VECTOR, HIPSOLVER_EIG_RANGE_I,
    HIPBLAS_FILL_MODE_UPPER, n_rows, in, n_cols, math_t(0.0), math_t(0.0),
  ...
  if (memUsage == OVERWRITE_INPUT) {
    ...
    CUSOLVER_CHECK(cusolverDnsyevdx(
      cusolverH, HIPSOLVER_EIG_MODE_VECTOR, HIPSOLVER_EIG_RANGE_I,
      HIPBLAS_FILL_MODE_UPPER, n_rows, eig_vectors, n_cols, math_t(0.0),
...
void eigJacobi(const math_t *in, int n_rows, int n_cols, math_t *eig_vectors,
               math_t *eig_vals, hipsolverHandle_t cusolverH,
  ...
  CUSOLVER_CHECK(hipsolverDnXsyevjSetMaxSweeps(syevj_params, sweeps));

  int lwork;
  CUSOLVER_CHECK(cusolverDnsyevj_bufferSize(
    cusolverH, HIPSOLVER_EIG_MODE_VECTOR, HIPBLAS_FILL_MODE_UPPER, n_rows,
  ...
  CUSOLVER_CHECK(cusolverDnsyevj(cusolverH, HIPSOLVER_EIG_MODE_VECTOR,
                                 HIPBLAS_FILL_MODE_UPPER, n_rows, eig_vectors,
                                 n_cols, eig_vals, d_work.data(), lwork,
  ...
  int executed_sweeps;
  CUSOLVER_CHECK(
    hipsolverDnXsyevjGetSweeps(cusolverH, syevj_params, &executed_sweeps));
