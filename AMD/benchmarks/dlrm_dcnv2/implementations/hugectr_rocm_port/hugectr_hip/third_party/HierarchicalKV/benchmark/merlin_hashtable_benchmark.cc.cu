...
float test_one_api(std::shared_ptr<MerlinHashTable>& table,
  ...
  CUDA_CHECK(hipHostMalloc(&h_vectors, key_num_per_op * sizeof(V) * dim));
  CUDA_CHECK(hipHostMalloc(&h_found, key_num_per_op * sizeof(bool)));

  CUDA_CHECK(hipMemset(h_vectors, 0, key_num_per_op * sizeof(V) * dim));
  ...
  CUDA_CHECK(hipMalloc(&d_vectors, key_num_per_op * sizeof(V) * dim));
  CUDA_CHECK(hipMalloc(&d_def_val, key_num_per_op * sizeof(V) * dim));
  ...
  CUDA_CHECK(hipMemset(d_vectors, 1, key_num_per_op * sizeof(V) * dim));
  CUDA_CHECK(hipMemset(d_def_val, 2, key_num_per_op * sizeof(V) * dim));
  ...
  for (int i = 0; i < 9; i++) {
    switch (api) {
      ...
      case API_Select::export_batch: {
        ...
        CUDA_CHECK(hipMemset(d_dump_counter, 0, sizeof(size_t)));
