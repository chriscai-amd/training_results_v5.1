...
S host_nano(hipStream_t stream = 0) {
  ...
  CUDA_CHECK(hipMalloc((void**)&(d_clk), sizeof(S)));
  host_nano_kernel<S><<<1, 1, 0, stream>>>(d_clk);
  CUDA_CHECK(hipStreamSynchronize(stream));

  CUDA_CHECK(hipMemcpy(&h_clk, d_clk, sizeof(S), hipMemcpyDeviceToHost));
  CUDA_CHECK(hipFree(d_clk));
  return h_clk;
}
...
void getBufferOnDevice(T** ptr, size_t size, hipStream_t stream) {
  ...
  CUDA_CHECK(hipMemsetAsync(*ptr, 0, size, stream));
}
...
struct HostAndDeviceBuffer {
 public:
  ...
  void SetFromHost(const T* data, size_t n, hipStream_t stream = 0) {
    CUDA_CHECK(hipMemcpyAsync(d_data, data, n * sizeof(T),
  ...
  void SetFromDevice(const T* data, size_t n, hipStream_t stream = 0) {
    ...
    CUDA_CHECK(hipMemcpyAsync(h_data, data, n * sizeof(T),
  ...
  bool SetValueInRange(T start, T skip, size_t stripe,
    ...
    CUDA_CHECK(hipMemcpyAsync(d_data, h_data, size_ * sizeof(T),
                               hipMemcpyHostToDevice, stream));
    return true;
  }

  void ToZeros(hipStream_t stream = 0) {
    CUDA_CHECK(hipMemsetAsync(d_data, 0, size_ * sizeof(T), stream));
  ...
  void SyncData(bool h2d, hipStream_t stream = 0) {
    if (h2d) {
      ...
      CUDA_CHECK(hipMemcpyAsync(h_data, d_data, size_ * sizeof(T),
                                 hipMemcpyDeviceToHost, stream));
    }
  }
...
bool allTrueGpu(const bool* conds, size_t n, hipStream_t stream) {
  ...
  CUDA_CHECK(hipMemcpyAsync(&nfalse, d_nfalse, sizeof(int),
                             hipMemcpyDeviceToHost, stream));
...
bool allEqualGpu(T* a, T* b, size_t n, hipStream_t stream) {
  ...
  CUDA_CHECK(hipMemcpyAsync(&ndiff, d_ndiff, sizeof(int),
                             hipMemcpyDeviceToHost, stream));
...
template <typename K, typename V, typename S>
bool tables_equal(TableType* a, TableType* b, hipStream_t stream) {
