...
// HIP/ROCm: native atomicCAS overloads for unsigned long/unsigned long long are
// provided by amd_hip_atomic.h, and uint64_t/int64_t typedef to those types on
// AMD64. Defining the overloads here would conflict with the built-ins.
#ifndef __HIPCC__
...
static inline int SAFE_GET_BLOCK_SIZE(int block_size, int device = -1) {
  ...
  CUDA_CHECK(hipGetDeviceProperties(&prop, current_device));
