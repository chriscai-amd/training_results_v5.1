...
class concurrent_unordered_map {
 ...
 public:
  ...
  static auto create(size_type capacity,
                     ...
                     const allocator_type& allocator = allocator_type(), hipStream_t stream = 0) {
 ...
 public:
  ...
  void assign_async(const concurrent_unordered_map& other, hipStream_t stream = 0) {
    ...
    CUDA_TRY(hipMemcpyAsync(m_hashtbl_values, other.m_hashtbl_values,
                             m_capacity * sizeof(value_type), hipMemcpyDefault, stream));
  }
  ...
  void prefetch(const int dev_id, hipStream_t stream = 0) {
    ...
    if (hipSuccess == status && isPtrManaged(hashtbl_values_ptr_attributes)) {
      CUDA_TRY(
          hipMemPrefetchAsync(m_hashtbl_values, m_capacity * sizeof(value_type), dev_id, stream));
    }
    CUDA_TRY(hipMemPrefetchAsync(this, sizeof(*this), dev_id, stream));
  }
   ...
   * @param stream CUDA stream used for device memory operations and kernel launches.
   **/
  void destroy(hipStream_t stream = 0) {
    m_allocator.deallocate(m_hashtbl_values, m_capacity, stream);
    delete this;
  }
 ...
 private:
  ...
  concurrent_unordered_map(size_type capacity, const mapped_type unused_element,
    ...
    constexpr int block_size = 128;
    {
      ...
      if (hipSuccess == status && isPtrManaged(hashtbl_values_ptr_attributes)) {
        int dev_id = 0;
        CUDA_TRY(hipGetDevice(&dev_id));
        CUDA_TRY(hipMemPrefetchAsync(m_hashtbl_values, m_capacity * sizeof(value_type), dev_id,
                                      stream));
      }
    }
