...
size_t convert_input_binaries(rmm::mr::device_memory_resource *mr, std::string input_file_path,
  ...
  while (true) {
    ...
    HCTR_LIB_THROW(hipMemset(dev_int_col_nullmask_ptrs, 0, sz_dev_int_col));
    HCTR_LIB_THROW(hipMemset(dev_cat_col_nullmask_ptrs, 0, sz_dev_str_ptrs));

    HCTR_LIB_THROW(hipMemcpy((void *)dev_int_col_ptrs, (void *)int_col_dev_ptrs.data(),
                              sz_dev_int_col, hipMemcpyHostToDevice));
    HCTR_LIB_THROW(hipMemcpy((void *)dev_int_col_nullmask_ptrs,
                              (void *)int_col_nullmask_dev_ptrs.data(), sz_dev_int_col,
                              hipMemcpyHostToDevice));
    HCTR_LIB_THROW(hipMemcpy((void *)dev_char_ptrs, (void *)char_ptrs.data(), sz_dev_str_ptrs,
                              hipMemcpyHostToDevice));
    HCTR_LIB_THROW(hipMemcpy((void *)dev_offset_ptrs, (void *)offset_ptrs.data(), sz_dev_str_ptrs,
                              hipMemcpyHostToDevice));
    HCTR_LIB_THROW(hipMemcpy((void *)dev_cat_col_nullmask_ptrs,
    ...
    HCTR_LIB_THROW(
        hipMemcpy(host_out_buffer, dev_out_buffer, size_of_output_binary, hipMemcpyDeviceToHost));
