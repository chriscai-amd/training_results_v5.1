  ...
  try {
    ...
    if (!skip_read_) {
      ...
      HCTR_LIB_THROW(hipMemcpyAsync(
          ...
          sizeof(int64_t) * dense_width_dim_->size(), hipMemcpyHostToDevice, dense_stream_));
      ...
      nvtxRangePop();
      {
        ...
        for (int k = 0; k < num_csr_buffers; k++) {
          ...
          HCTR_LIB_THROW(hipMemsetAsync(device_csr_row_offset_buffers[k], 0,
        ...
        for (size_t buffer_id = 0; buffer_id < param_num; buffer_id++) {
          ...
          HCTR_LIB_THROW(
              hipMemcpyAsync(host_pinned_csr_inc_.data(), reinterpret_cast<void*>(dev_row_pointer),
                              offset_datatype.size(), hipMemcpyDeviceToHost, task_stream_));
          HCTR_LIB_THROW(hipStreamSynchronize(task_stream_));
          int64_t nnz = -1;
          if (offset_datatype.size() == 4) {
      ...
      HCTR_LIB_THROW(hipStreamSynchronize(task_stream_));
      HCTR_LIB_THROW(hipStreamSynchronize(dense_stream_));
    }
    buffer23_->state.store(BufferState::ReadyForRead);
  } catch (const core23::RuntimeError& rt_err) {
    if (rt_err.error == Error_t::EndOfFile) {
...
ParquetDataReaderWorker<T>::ParquetDataReaderWorker(
  ...
  if (!thread_resource_allocated_) {
    ...
    HCTR_LIB_THROW(hipSetDevice(device_id_));  // for multiple devices
    HCTR_LIB_THROW(hipStreamCreateWithFlags(&task_stream_, hipStreamNonBlocking));
    HCTR_LIB_THROW(hipStreamCreateWithFlags(&dense_stream_, hipStreamNonBlocking));
    ...
    HCTR_LIB_THROW(hipMemcpyAsync(slot_offset_device_buf_->data(), slot_offset_dtype_.data(),
                                   slot_offset_buf_size, hipMemcpyHostToDevice, task_stream_));
...
ParquetDataReaderWorker<T>::~ParquetDataReaderWorker() {
  ...
  if (thread_resource_allocated_) {
    this->skip_read();
    HCTR_LIB_CHECK_(hipStreamSynchronize(task_stream_));
    HCTR_LIB_CHECK_(hipStreamSynchronize(dense_stream_));
    slot_offset_device_buf_.reset();
    source_.reset();
    HCTR_LIB_CHECK_(hipStreamDestroy(task_stream_));
    HCTR_LIB_CHECK_(hipStreamDestroy(dense_stream_));
    thread_resource_allocated_ = false;
  }
}
