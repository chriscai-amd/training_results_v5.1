...
LocalizedFilterKeyStorage<TypeHashKey>::LocalizedFilterKeyStorage(
  ...
  {
    size_t bytes_size = 0;
    hipcub::DeviceSelect::Flagged(nullptr, bytes_size, (TypeHashKey *)nullptr, (char *)nullptr,
...
void LocalizedSlotSparseEmbeddingHash<TypeHashKey, TypeEmbeddingComp>::filter_keys_per_gpu(
  ...
  {
    hipMemsetAsync(filter_keys_storage.value_select_flag.get_ptr(), 0,
  ...
  // select nnz
  {
    HCTR_LIB_THROW(hipMemcpyAsync(nnz_ptr.get(), filter_keys_storage.value_select_num.get_ptr(),
                                   sizeof(size_t), hipMemcpyDeviceToHost, local_gpu.get_stream()));
  }
}
...
#pragma omp parallel for num_threads(local_gpu_count)
  ...
  for (size_t id = 0; id < local_gpu_count; id++) {
    ...
    HCTR_LIB_THROW(hipMemsetAsync(d_value_index_per_gpu[id], 0, value_index_size_in_B,
                                   embedding_data_.get_local_gpu(id).get_stream()));

    size_t key_size_in_B = counter_per_gpu[id] * sizeof(TypeHashKey);
    HCTR_LIB_THROW(hipMemPrefetchAsync(uvm_key_per_gpu[id], key_size_in_B, hipCpuDeviceId,
                                        embedding_data_.get_local_gpu(id).get_stream()));
  }
  functors_.sync_all_gpus(embedding_data_.get_resource_manager());
...
#pragma omp parallel for num_threads(local_gpu_count)
  for (size_t id = 0; id < local_gpu_count; id++) {
    ...
    HCTR_LIB_THROW(hipMemPrefetchAsync(uvm_key_per_gpu[id], key_size_in_B, id,
                                        embedding_data_.get_local_gpu(id).get_stream()));
    ...
    HCTR_LIB_THROW(hipMemcpyAsync(hash_table_slot_id_tensors[id].get_ptr(), h_slot_id_per_gpu[id],
                                   slot_id_size_in_B, hipMemcpyHostToDevice,
                                   embedding_data_.get_local_gpu(id).get_stream()));

    // memcpy hash_table_value from CPU to GPU
    size_t vec_block_in_B = counter * embedding_vec_size * sizeof(float);
    HCTR_LIB_THROW(hipMemcpyAsync(hash_table_value_tensors[id].get_ptr(), h_value_per_gpu[id],
...
void LocalizedSlotSparseEmbeddingHash<TypeHashKey, TypeEmbeddingComp>::load_parameters(
  ...
  for (size_t id = 0; id < local_gpu_count; id++) {
    context.set_device(embedding_data_.get_local_gpu(id).get_device_id());
    HCTR_LIB_THROW(hipMalloc(&d_hash_table_value_index_chunk_per_gpu[id],
                              hash_table_value_index_chunk_size_in_B));
    // initialize to zeros
    HCTR_LIB_THROW(hipMemsetAsync(d_hash_table_value_index_chunk_per_gpu[id], 0,
                                   hash_table_value_index_chunk_size_in_B,
                                   embedding_data_.get_local_gpu(id).get_stream()));
  }
  ...
  for (size_t id = 0; id < local_gpu_count; id++) {
    context.set_device(embedding_data_.get_local_gpu(id).get_device_id());
    HCTR_LIB_THROW(hipMalloc(&d_hash_table_key_chunk_per_gpu[id], hash_table_key_chunk_size_in_B));
  }
  std::unique_ptr<size_t *[]> h_hash_table_slot_id_chunk_per_gpu(new size_t *[local_gpu_count]);
  for (size_t id = 0; id < local_gpu_count; id++) {
    HCTR_LIB_THROW(hipHostMalloc(&h_hash_table_slot_id_chunk_per_gpu[id],
  ...
  for (size_t i = 0; i < loop_num; i++) {
    ...
    for (size_t id = 0; id < local_gpu_count; id++) {
      ...
      // memcpy hash_table_key from CPU to GPU
      HCTR_LIB_THROW(hipMemcpyAsync(d_hash_table_key_chunk_per_gpu[id],
    ...
    for (size_t id = 0; id < local_gpu_count; id++) {
      ...
      HCTR_LIB_THROW(hipMemcpyAsync(dst_buf_sid, src_buf_sid, slot_id_chunk_size * sizeof(size_t),
      ...
      HCTR_LIB_THROW(hipMemcpyAsync(dst_buf_value, src_buf_value, value_chunk_size * sizeof(float),
                                     hipMemcpyHostToDevice,
                                     embedding_data_.get_local_gpu(id).get_stream()));
    }
  ...
  for (size_t i = 0; i < remain_loop_num; i++) {  // process one tile in each loop
    ...
    if (embedding_data_.get_resource_manager().get_process_id() == dst_rank) {
      ...
      HCTR_LIB_THROW(hipMemcpyAsync(key_dst_buf, &key, hash_table_key_tile_size_in_B,
                                     hipMemcpyHostToDevice,
                                     embedding_data_.get_local_gpu(id).get_stream()));
      ...
      HCTR_LIB_THROW(hipMemcpyAsync(slot_id_dst_buf, &slot_id, hash_table_slot_id_tile_size_in_B,
                                     hipMemcpyHostToDevice,
                                     embedding_data_.get_local_gpu(id).get_stream()));
  ...
  for (size_t id = 0; id < local_gpu_count; id++) {
    context.set_device(embedding_data_.get_local_gpu(id).get_device_id());
    HCTR_LIB_THROW(hipFree(d_hash_table_value_index_chunk_per_gpu[id]));
    HCTR_LIB_THROW(hipFree(d_hash_table_key_chunk_per_gpu[id]));
  }
  for (size_t id = 0; id < local_gpu_count; id++) {
    HCTR_LIB_THROW(hipHostFree(h_hash_table_key_chunk_per_gpu[id]));
    HCTR_LIB_THROW(hipHostFree(h_hash_table_value_chunk_per_gpu[id]));
  }
}
...
void LocalizedSlotSparseEmbeddingHash<TypeHashKey, TypeEmbeddingComp>::dump_parameters(
  ...
  HCTR_LIB_THROW(hipHostMalloc(&h_hash_table_key, total_count * sizeof(TypeHashKey)));
  HCTR_LIB_THROW(hipHostMalloc(&h_hash_table_slot_id, total_count * sizeof(size_t)));
  HCTR_LIB_THROW(
  ...
  for (size_t id = 0; id < local_gpu_count; id++) {
    ...
    HCTR_LIB_THROW(hipMallocManaged(&d_hash_table_key[id], count[id] * sizeof(TypeHashKey)));
    HCTR_LIB_THROW(hipMallocManaged(&d_hash_table_value_index[id], count[id] * sizeof(size_t)));
    HCTR_LIB_THROW(hipMallocManaged(&d_hash_table_slot_id[id], count[id] * sizeof(size_t)));
    HCTR_LIB_THROW(hipMallocManaged(&d_dump_counter[id], sizeof(size_t)));
  }
  ...
  for (size_t id = 0; id < local_gpu_count; id++) {
    ...
    HCTR_LIB_THROW(hipMemcpyAsync(h_hash_table_key + offset_host[id], d_hash_table_key[id],
                                   count[id] * sizeof(TypeHashKey), hipMemcpyDeviceToHost,
                                   embedding_data_.get_local_gpu(id).get_stream()));

    HCTR_LIB_THROW(hipMemcpyAsync(h_hash_table_slot_id + offset_host[id],
  ...
  for (size_t id = 0; id < local_gpu_count; id++) {
    ...
    HCTR_LIB_THROW(hipFree(d_hash_table_slot_id[id]));
    HCTR_LIB_THROW(hipFree(d_dump_counter[id]));
  }
  HCTR_LIB_THROW(hipHostFree(h_hash_table_key));
  HCTR_LIB_THROW(hipHostFree(h_hash_table_slot_id));
  HCTR_LIB_THROW(hipHostFree(h_hash_table_value));
}
...
void LocalizedSlotSparseEmbeddingHash<TypeHashKey, TypeEmbeddingComp>::dump_parameters(
  ...
  *num = total_count;
  ...
  for (size_t id = 0; id < local_gpu_count; id++) {
    ...
    HCTR_LIB_THROW(hipMalloc(&d_hash_table_slot_id[id], count[id] * sizeof(size_t)));
    HCTR_LIB_THROW(hipMalloc(&d_dump_counter[id], sizeof(size_t)));
  }
  ...
  for (size_t id = 0; id < local_gpu_count; id++) {
    ...
    HCTR_LIB_THROW(hipMemcpyAsync(key_ptr + offset_host[id], d_hash_table_key[id],
                                   count[id] * sizeof(TypeHashKey), hipMemcpyDeviceToHost,
                                   embedding_data_.get_local_gpu(id).get_stream()));

    HCTR_LIB_THROW(hipMemcpyAsync(slot_id_ptr + offset_host[id],
                                   hash_table_slot_id_tensors[id].get_ptr(),
                                   count[id] * sizeof(size_t), hipMemcpyDeviceToHost,
                                   embedding_data_.get_local_gpu(id).get_stream()));
  }
...
#pragma omp parallel num_threads(embedding_data_.get_resource_manager().get_local_gpu_count())
  {
    ...
    HugeCTR::UniformGenerator::fill(
        ...
        embedding_data_.get_local_gpu(id).get_stream());
    HCTR_LIB_THROW(hipStreamSynchronize(embedding_data_.get_local_gpu(id).get_stream()));
...
void LocalizedSlotSparseEmbeddingHash<TypeHashKey, TypeEmbeddingComp>::reset() {
  ...
  for (size_t i = 0; i < embedding_data_.get_resource_manager().get_local_gpu_count(); i++) {
    HCTR_LIB_THROW(hipStreamSynchronize(embedding_data_.get_local_gpu(i).get_stream()));
  }
}
