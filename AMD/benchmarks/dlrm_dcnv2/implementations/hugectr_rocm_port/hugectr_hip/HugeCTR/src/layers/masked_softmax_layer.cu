...
void mask_softmax_fprop(T* out, T* in, T* mask, int batch_size, int head_num, int seq_len_from,
                        int seq_len_to, float scalar, hipStream_t stream) {
...
void MaskedSoftmaxLayer<T>::fprop(bool is_train) {
  ...
  mask_softmax_fprop(out_tensor.template data<T>(), in_tensor.template data<T>(), mask_tensor.template data<T>(), shape_in[0],
                     shape_in[1], shape_in[2], shape_in[3], scalar_, get_gpu().get_stream());
  HCTR_LIB_THROW(hipMemcpyAsync((void*)softmax_out_.data(), (void*)out_tensor.data(),
                                 out_tensor.num_bytes(), hipMemcpyDeviceToDevice,
                                 get_gpu().get_stream()));
}
...
void MaskedSoftmaxLayer<__half>::fprop(bool is_train) {
  ...
  mask_softmax_fprop(out_tensor.template data<__half>(), in_tensor.template data<__half>(),
                     mask_tensor.template data<__half>(), shape_in[0], shape_in[1], shape_in[2], shape_in[3],
...
void MaskedSoftmaxLayer<T>::bprop() {
  ...
  mask_softmax_bprop(top_tensor.template data<T>(), bottom_tensor.template data<T>(), softmax_out_.template data<T>(), batch,
