...
void AdamOptimizer<T>::initialize() {
  HCTR_LIB_THROW(
      hipMemsetAsync(m_tensor_.data(), 0, m_tensor_.num_bytes(), gpu_resource_->get_stream()));
  HCTR_LIB_THROW(
      hipMemsetAsync(v_tensor_.data(), 0, v_tensor_.num_bytes(), gpu_resource_->get_stream()));
}
