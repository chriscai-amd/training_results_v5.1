...
#pragma omp parallel for num_threads(resource_manager_->get_local_gpu_count())
  for (int local_id = 0; local_id < static_cast<int>(resource_manager_->get_local_gpu_count());
    ...
    distribute_data->set_debug_name("[sparse_prep] data_distribute");
    ...
    ebc_mp_model_forward->set_debug_name("[emb_fwd] ebc_mp_model");
    ...
    ebc_mp_network_forward->set_debug_name("[emb_a2a] ebc_mp_network");
    ...
    ebc_mp_network_backward->set_debug_name("[emb_a2a] ebc_mp_network_bwd");
    ...
    ebc_mp_backward_index_calculation->set_debug_name("[sparse_prep] ebc_mp_idx_calc");
    ...
    ebc_mp_local_reduce->set_debug_name("[emb_reduce] ebc_mp_local_reduce");
    ...
    // streams. NV upstream relies on cuBLASLt-internal worker streams to
    // overlap MLP compute with collectives that share the default stream;
    // hipBLASLt has no such internal streams on gfx950 so we have to
    // manufacture the separation explicitly at the HCTR layer. Phase 14r
    // 8-GPU trace: tid=32 (default) carries 553us of RCCL serialised with
    // 4071us of MLP compute -- this is the lever that frees them.
    // 2-trial A/B at bs=1x measured +3.3% avg lift (range +2.5..+4.0%),
    // loss bit-equivalent. NOW DEFAULT ON; set HCTR_DEDICATED_RCCL_STREAM=0
    // to disable.
    const char* const dedicated_rccl_env = std::getenv("HCTR_DEDICATED_RCCL_STREAM");
    const bool dedicated_rccl =
        (dedicated_rccl_env == nullptr || dedicated_rccl_env[0] != '0');
    if (dedicated_rccl) {
      // Phase 17 (perf-push, lever 5a): give RCCL streams HIGHER priority
      // (-1 in HIP convention; 0=default, negative = higher). 2-trial A/B
      // showed +1.8% lift but a follow-up 3-trial validation revealed
      // INTERMITTENT first-iter divergence at priority=-1 (1 of 3 runs hit
      // "Loss cannot converge" in the very first iter). Likely a HIP runtime
      // scheduling-order non-determinism that interacts badly with FP16
      // loss-scaling at scaler=16348. Default OFF for production safety.
      // Set HCTR_RCCL_STREAM_PRIORITY=-1 explicitly to opt in (~+1.8% with
      // ~33% chance of first-iter divergence; retry on failure).
      static const int kRcclPriority = []() {
        const char* env = std::getenv("HCTR_RCCL_STREAM_PRIORITY");
        return env != nullptr ? std::atoi(env) : 0;  // default 0 = safe
      }();
      ebc_dp_allreduce->set_absolute_stream("rccl_emb_ar", kRcclPriority);
    }
    ...
    ebc_dp_update->set_debug_name("[opt_emb] ebc_dp_update");
    ...
    network_init->set_debug_name("[mlp_fwd] network_init");
    ...
    bottom_network_fprop->set_debug_name("[mlp_fwd] bot_mlp_stack");
    ...
    top_network_fprop->set_debug_name("[mlp_fwd] top_mlp_stack");
    ...
    init_wgrad->set_debug_name("[mlp_bwd_wgrad] init_wgrad");
    ...
    cal_loss->set_debug_name("[loss] BCE_compute");
    ...
    top_network_bprop->set_debug_name("[mlp_bwd] top_mlp_stack");
    ...
    bottom_network_bprop->set_debug_name("[mlp_bwd] bot_mlp_stack");
    ...
    // ebc_dp_update on the default stream.
    if (dedicated_rccl) {
      // Phase 17: see comment above ebc_dp_allreduce. Default priority 0
      // for production safety; opt-in via HCTR_RCCL_STREAM_PRIORITY=-1.
      static const int kRcclPriority2 = []() {
        const char* env = std::getenv("HCTR_RCCL_STREAM_PRIORITY");
        return env != nullptr ? std::atoi(env) : 0;  // default 0 = safe
      }();
      network_exchange_wgrad->set_absolute_stream("rccl_mlp_wgrad", kRcclPriority2);
    }
    ...
    sync_back->set_debug_name("[sync] sync_back");

    if (solver_.train_intra_iteration_overlap) {
      // Phase 18 attempt: set_stream(name, -1) on mp/dp embedding streams
      // BREAKS CONVERGENCE (loss diverges before iter 200 even with priority=0
      // control). Reverted. The set_stream(name, prio) variant must have a
      // subtly different code path than set_stream(name) on AMD HIP that we
      // don't yet understand. Keeping the original no-priority calls.
    ...
    if (!solver_.train_inter_iteration_overlap) {
      ...
      ebc_cache_train_ddl_output->set_debug_name("[memcpy] ebc_cache_ddl");
      ...
      copy_next_iter_network_input->set_debug_name("[memcpy] copy_next_iter");
...
#pragma omp parallel num_threads(number_of_networks())
    {
      ...
      HCTR_LIB_THROW(hipStreamSynchronize(resource_manager_->get_local_gpu(id)->get_stream()));
    }
