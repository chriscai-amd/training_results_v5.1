...
void ReshapeLayer<T>::initialize() {
  if (!in_place_) {
    HCTR_LIB_THROW(hipMemcpyAsync(selected_slots_tensor_.data(), &selected_.front(),
                                   selected_slots_tensor_.num_bytes(), hipMemcpyHostToDevice,
                                   get_gpu().get_stream()));
  }
}
...
template <typename T>
void ReshapeLayer<T>::prop_common(bool forward, bool is_train, hipStream_t stream) {
  ...
  if (in_place_) {
    if (forward) {
      HCTR_LIB_THROW(hipMemcpyAsync(output_tensor.template data<T>(), input_tensor.template data<T>(),
                                     input_tensor.num_bytes(), hipMemcpyDeviceToDevice, stream));
    } else {
      HCTR_LIB_THROW(hipMemcpyAsync(input_tensor.template data<T>(), output_tensor.template data<T>(),
                                     output_tensor.num_bytes(), hipMemcpyDeviceToDevice, stream));
    }
  } else {
    int block_size = 128;
    ...
    reshape_kernel<<<n_block, block_size>>>(in, out, batch_size_, n_slot_, vector_length_,
                                            selected_slots_tensor_.template data<int32_t>(), n_active_slot_,
                                            forward);
  }
}
