...
AsyncDataReader<SparseType>::AsyncDataReader(
  ...
  for (size_t i = 0; i < resource_manager_->get_local_gpu_count(); i++) {
    ...
    HCTR_LIB_THROW(hipEventCreateWithFlags(&completion_events_[i], hipEventDisableTiming));
    HCTR_LIB_THROW(hipEventCreateWithFlags(&schedule_events_[i], hipEventDisableTiming));
    HCTR_LIB_THROW(hipEventCreateWithFlags(&split_schedule_events_[i], hipEventDisableTiming));
    HCTR_LIB_THROW(hipEventCreateWithFlags(&d2d_schedule_events_[i], hipEventDisableTiming));
    ...
    HCTR_LIB_THROW(hipMemcpy(bucket_id_tensor.data(), bucket_ids.data(),
                              static_cast<int64_t>(total_nnz_) * sizeof(int),
                              hipMemcpyHostToDevice));
    HCTR_LIB_THROW(hipMemcpy(bucket_position_tensor.data(), bucket_positions.data(),
                              static_cast<int64_t>(total_nnz_) * sizeof(int),
                              hipMemcpyHostToDevice));
    HCTR_LIB_THROW(hipMemcpy(max_hotness_tensor.data(), nnz_per_slot_.data(),
                              sparse_dim_ * sizeof(int), hipMemcpyHostToDevice));
  ...
  for (auto& batch_tensors : inflight_batch_tensors_) {
    ...
    for (size_t i = 0; i < resource_manager_->get_local_gpu_count(); i++) {
      ...
      for (size_t fea_id = 0; fea_id < sparse_dim_; ++fea_id) {
        ...
        HCTR_LIB_THROW(
            hipMemcpy(reinterpret_cast<SparseType**>(temp_sparse_tensor_ptrs.data()) + fea_id,
                       &value_ptr, sizeof(SparseType*), hipMemcpyHostToDevice));
        ...
        HCTR_LIB_THROW(hipMemcpy(device_sparse_tensors[fea_id].get_rowoffset_ptr(),
...
#pragma omp parallel for num_threads(num_local_gpus)
  for (int i = 0; i < num_local_gpus; i++) {
    ...
    const hipStream_t& stream = s3w_streams_[i];
    ...
    HCTR_LIB_THROW(hipStreamWaitEvent(stream, split_schedule_events_[i]));
    ...
    HCTR_LIB_THROW(hipEventRecord(sparse_ready_event, stream));

    auto d2d_stream = d2d_streams_[i];

    // Need result from split-3-way
    HCTR_LIB_THROW(hipStreamWaitEvent(d2d_stream, sparse_ready_event));

    // we are safe to overwrite
    HCTR_LIB_THROW(hipStreamWaitEvent(d2d_stream, d2d_schedule_events_[i]));
    ...
    HCTR_LIB_THROW(hipEventRecord(tensors_ready_event, d2d_stream));

    // batch.device_data can be reused. Needs to be called after D2D because hipStreamAddCallback
...
template <typename SparseType>
void AsyncDataReader<SparseType>::set_schedule_streams(hipStream_t s3w_stream,
                                                       hipStream_t d2d_stream, int raw_device_id) {
...
void AsyncDataReader<SparseType>::assign_dense_and_label_tensors(core23::Tensor label_tensor,
  ...
  if ((char*)dst_label_tensor.data() + dst_label_tensor.num_bytes() ==
    ...
    HCTR_LIB_THROW(hipMemcpyAsync(dst_label_tensor.data(), label_tensor.data(),
                                   dst_label_tensor.num_bytes() + dense_tensor.num_bytes(),
                                   hipMemcpyDeviceToDevice, stream));
  } else {
    HCTR_LIB_THROW(hipMemcpyAsync(dst_label_tensor.data(), label_tensor.data(),
                                   dst_label_tensor.num_bytes(), hipMemcpyDeviceToDevice, stream));

    HCTR_LIB_THROW(hipMemcpyAsync(dst_dense_tensor.data(), dense_tensor.data(),
                                   dst_dense_tensor.num_bytes(), hipMemcpyDeviceToDevice, stream));
  }
}
...
template <typename SparseType>
void AsyncDataReader<SparseType>::stream_wait_sparse_tensors(hipStream_t stream, int raw_device_id,
                                                             bool from_graph) {
  auto gpu = resource_manager_->get_local_gpu(raw_device_id);
  // Phase 14s (perf-push, overlap-fix #1): see pipeline.cpp:91-101 comment.
  // NV upstream uses cudaEventWaitExternal here; same env knob gates restore.
  static const bool kRestoreWaitExternal = []() {
    const char* env = std::getenv("HCTR_RESTORE_WAIT_EXTERNAL");
    return env != nullptr && env[0] == '1';
  }();
  const auto flags =
      (kRestoreWaitExternal && from_graph) ? hipEventWaitExternal : 0;
  HCTR_LIB_THROW(hipStreamWaitEvent(stream, gpu->get_event("sparse_tensors_ready"), flags));
}

template <typename SparseType>
void AsyncDataReader<SparseType>::stream_wait_dense_tensors(hipStream_t stream, int raw_device_id,
                                                            bool from_graph) {
  auto gpu = resource_manager_->get_local_gpu(raw_device_id);
  // Phase 14s (perf-push, overlap-fix #1): see pipeline.cpp:91-101 comment.
  static const bool kRestoreWaitExternal = []() {
  ...
  const auto flags =
      (kRestoreWaitExternal && from_graph) ? hipEventWaitExternal : 0;
  HCTR_LIB_THROW(hipStreamWaitEvent(stream, gpu->get_event("bottom_MLP_tensors_ready"), flags));
}
...
template <typename SparseType>
void AsyncDataReader<SparseType>::schedule_split_3_way_here(hipStream_t stream, int raw_device_id,
                                                            bool from_graph) {
  unsigned int flags = from_graph ? hipEventRecordExternal : 0;
  HCTR_LIB_THROW(hipEventRecordWithFlags(split_schedule_events_[raw_device_id], stream, flags));
}

template <typename SparseType>
void AsyncDataReader<SparseType>::schedule_d2d_here(hipStream_t stream, int raw_device_id,
                                                    bool from_graph) {
  unsigned int flags = from_graph ? hipEventRecordExternal : 0;
  HCTR_LIB_THROW(hipEventRecordWithFlags(d2d_schedule_events_[raw_device_id], stream, flags));
}

template <typename SparseType>
void AsyncDataReader<SparseType>::schedule_here(hipStream_t stream, int raw_device_id) {
  reader_impl_->schedule_upload_here(raw_device_id, stream, false);
}

template <typename SparseType>
void AsyncDataReader<SparseType>::schedule_here_graph(hipStream_t stream, int raw_device_id) {
