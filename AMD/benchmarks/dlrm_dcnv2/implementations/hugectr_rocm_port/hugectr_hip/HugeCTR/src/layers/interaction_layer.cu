...
// HugeCTR ROCm port: hipify left an empty include here (was <cuda/std/array>);
// the standard <array> is sufficient for the use sites in this file.
#include <array>
// HugeCTR ROCm port: <mma.h> is the CUDA WMMA tensor-core API. AMD provides
// equivalent matrix-core (MFMA) intrinsics via __builtin_amdgcn_mfma_*
// (in <hip/amd_detail/amd_hip_runtime.h>) or via the rocWMMA header.
// For a baseline functional build we route the WMMA path through the
// non-tensor-core fallback already coded in this file.
// #include <mma.h>
...
void InteractionLayer<T>::fprop_generic(bool is_train) {
  ...
  const hipblasGemmAlgo_t algo =
      use_mixed_precision_ ? HIPBLAS_GEMM_DEFAULT : HIPBLAS_GEMM_DEFAULT;

  HCTR_LIB_THROW(hipblasGemmStridedBatchedEx(get_gpu().get_cublas_handle(), HIPBLAS_OP_T, HIPBLAS_OP_N,
...
void InteractionLayer<T>::bprop_generic() {
  ...
  HCTR_LIB_THROW(hipblasGemmStridedBatchedEx(get_gpu().get_cublas_handle(), HIPBLAS_OP_N, HIPBLAS_OP_N,
