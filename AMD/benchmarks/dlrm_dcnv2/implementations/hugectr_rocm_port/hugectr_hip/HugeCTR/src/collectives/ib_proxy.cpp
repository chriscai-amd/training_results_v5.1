...
void IbvProxy::HierA2ACollContext::init_buf(const M2PHierA2ABufInit& in, P2MHierA2ABufInit& out) {
  ...
  HCTR_LIB_THROW(hipHostMalloc(&send_sizes_, sizeof(size_t) * num_procs));
  HCTR_LIB_THROW(hipHostMalloc(&recv_sizes_, sizeof(size_t) * num_procs));
...
void IbvProxy::HierA2AvCollContext::init_buf(const M2PHierA2AvBufInit& in,
  ...
  HCTR_LIB_THROW(hipHostMalloc(&send_sizes_, sizeof(size_t) * num_procs_ * num_buffers_));
  HCTR_LIB_THROW(hipHostMalloc(&recv_sizes_, sizeof(size_t) * num_procs_ * num_buffers_));
...
IbvProxy::HierA2AvCollContext::~HierA2AvCollContext() {
  ...
  if (send_sizes_) {
    HCTR_LIB_CHECK_(hipHostFree(send_sizes_));
  }
  if (recv_sizes_) {
    HCTR_LIB_CHECK_(hipHostFree(recv_sizes_));
  }
  if (wr_) {
    for (size_t n = 0; n < num_procs_; n++) {
...
void IbvProxy::ARCollContext::init_gdr() {
  ...
  PROXY_ASSERT(gdr_);
  {
    auto ret = gdr_pin_buffer(gdr_, (hipDeviceptr_t)(d_ag_cmd_), gpu_page_size, 0, 0, &gdr_mh_);
    PROXY_ASSERT(ret == 0);
  }
  {
...
void IbvProxy::ARCollContext::init_buf(const M2PARBufInit& in, P2MARBufInit& out) {
  ...
  // Allocate host flags
  HCTR_LIB_THROW(hipHostMalloc(&h_rs_cmd_, sizeof(size_t)));
  *h_rs_cmd_ = 0;
  ...
  HCTR_LIB_THROW(hipSetDevice(proxy_ctx_->cfg_.device_id_));
  HCTR_LIB_THROW(hipMalloc(&d_ag_storage_, 2 * gpu_page_size));
  HCTR_LIB_THROW(hipMemset(d_ag_storage_, 0, 2 * gpu_page_size));
  d_ag_cmd_ = (size_t*)(((hipDeviceptr_t)d_ag_storage_ + gpu_page_size - 1) & gpu_page_mask);
...
IbvProxy::ARCollContext::~ARCollContext() {
  ...
  if (d_ag_storage_) {
    HCTR_LIB_CHECK_(hipFree(d_ag_storage_));
  }
  if (h_rs_cmd_) {
    HCTR_LIB_CHECK_(hipHostFree(h_rs_cmd_));
  }
  if (mem_mr) {
