...
GPUResource::GPUResource(int device_id, size_t local_id, size_t global_id,
  ...
  // ROCm port FIX: omitting hipStreamNonBlocking creates a BLOCKING stream
  // that implicitly synchronises with stream 0, which prevents this stream
  // (used for async-wgrad / compute-overlap) from running concurrently
  // with other streams (including RCCL). On NVIDIA / CUDA the original
  // HugeCTR code happens to work because cuStreamCreate w/ flags=0 still
  // allows concurrency on most setups; on AMD / HIP it strictly serialises.
  // Setting hipStreamNonBlocking is the canonical fix for this kind of
  // overlap regression and matches the other 2 explicit non-blocking
  // streams (memcpy_stream_, p2p_stream_) that already use the flag.
  computation_stream_2_ =
      stream_event_manager_.get_stream("computation_stream_2_", hipStreamNonBlocking);
