...
GpuLearningRateScheduler::GpuLearningRateScheduler(float base_lr, size_t warmup_steps,
  ...
  HCTR_LIB_THROW(hipMalloc(&step_, sizeof(size_t)));
  HCTR_LIB_THROW(hipMalloc(&current_lr_, sizeof(float)));
  HCTR_LIB_THROW(hipMalloc(&last_lr_, sizeof(float)));
...
GpuLearningRateScheduler::~GpuLearningRateScheduler() {
  HCTR_LIB_CHECK_(hipFree(step_));
  HCTR_LIB_CHECK_(hipFree(current_lr_));
  HCTR_LIB_CHECK_(hipFree(last_lr_));
}
