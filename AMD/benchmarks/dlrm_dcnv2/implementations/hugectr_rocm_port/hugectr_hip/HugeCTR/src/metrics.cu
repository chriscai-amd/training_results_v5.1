...
void copy_all<float>(float* y_pred, float* y_label, float* x_pred, float* x_label, int num_elems,
  ...
  HCTR_LIB_THROW(hipMemcpyAsync(y_label, x_label, num_elems * sizeof(float),
...
void all_to_all(T* srcptr, T* dstptr, const CountType* src_offsets, const CountType* dst_offsets,
                int num_global_gpus, const GPUResource* gpu_resource, const hipStream_t& stream) {
...
void send_halo_right(T* srcptr, T* dstptr, int count, int left_neighbor, int right_neighbor,
                     const GPUResource* gpu_resource, const hipStream_t& stream) {
  auto& comm = gpu_resource->get_nccl();
  auto type = get_nccl_type<T>();
...
void get_raw_metric_as_host_float_tensor(Core23RawMetricMap metric_map, RawType raw_type,
  ...
  int device_id;
  HCTR_LIB_THROW(hipGetDevice(&device_id));
  ...
  HCTR_LIB_THROW(hipMemcpy(rst, device_prediction_result.data(), num * sizeof(float),
...
AverageLoss<T>::AverageLoss(const std::shared_ptr<ResourceManager>& resource_manager)
  ...
  for (size_t local_gpu_id = 0; local_gpu_id < resource_manager_->get_local_gpu_count();
       ++local_gpu_id) {
    HCTR_LIB_THROW(hipHostMalloc((void**)&loss_local_[local_gpu_id], sizeof(float)));
  }
}
...
AverageLoss<T>::~AverageLoss() {
  for (size_t local_gpu_id = 0; local_gpu_id < resource_manager_->get_local_gpu_count();
       ++local_gpu_id) {
    HCTR_LIB_CHECK_(hipHostFree(loss_local_[local_gpu_id]));
  }
}
...
void AverageLoss<T>::local_reduce(int local_gpu_id, Core23RawMetricMap raw_metrics) {
  ...
  HCTR_LIB_THROW(hipMemcpy(loss_local_[local_gpu_id], loss_tensor.data(), sizeof(float),
                            hipMemcpyDeviceToHost));
}
...
void gen_access_desc(std::vector<hipMemAccessDesc>& access_desc, const std::vector<int>& peers) {
  access_desc.resize(peers.size());
  for (size_t i = 0; i < peers.size(); i++) {
    access_desc[i].flags = hipMemAccessFlagsProtReadWrite;
    access_desc[i].location.type = hipMemLocationTypeDevice;
...
void AUCStorage::alloc_main(size_t num_local_samples, size_t num_bins, size_t num_partitions,
                            ...
                            const std::vector<int>& peers, hipStream_t stream) {
  ...
  for (auto& st : finalize_storage_) {
    HCTR_LIB_THROW(hipMemset(st.d_pos_per_gpu(), 0, (num_global_gpus + 1) * sizeof(CountType)));
    HCTR_LIB_THROW(hipMemset(st.d_neg_per_gpu(), 0, (num_global_gpus + 1) * sizeof(CountType)));
  }
...
void AUCStorage::realloc_redistributed(size_t num_redistributed_samples, hipStream_t stream,
                                       size_t stream_id) {
...
AUC<T>::AUC(int batch_size_per_gpu, int n_batches, int label_dim,
  ...
  for (int i = 0; i < num_local_gpus_; i++) {
    ...
    for (auto& stream : streams_[i]) {
      HCTR_LIB_THROW(hipStreamCreate(&stream));
    }
  ...
  if (num_classes_ == 1) {
    ...
    for (size_t class_id = 0; class_id < num_classes_; class_id++) {
      ...
      HCTR_LIB_THROW(hipMemcpyAsync(
          ...
          num_valid_samples * sizeof(float), hipMemcpyDeviceToDevice, copy_stream));
      HCTR_LIB_THROW(hipMemcpyAsync(
...
float AUC<T>::finalize_class_metric_multi_stream(int local_id, int num_local_samples) {
  ...
  while (class_begin < num_classes_) {
    for (size_t step_id = 0; step_id < num_finalize_steps_; step_id++) {
      ...
      for (size_t stream_id = 0; stream_id < num_streams and class_begin + stream_id < num_classes_;
           stream_id++) {
        HCTR_LIB_THROW(hipStreamSynchronize(streams_[local_id][stream_id]));
...
void AUC<T>::run_finalize_step(float* d_preds, float* d_labels, int local_id,
  ...
  if (step_id == 0 or step_id == num_finalize_steps_) {
    ...
    hipcub::TransformInputIterator<float, decltype(clamp), float*> d_clamped_preds(d_preds, clamp);
    ...
    CUB_allocate_and_launch(st, stream_id, [&](void* workspace, size_t& size) {
      return hipcub::DeviceHistogram::HistogramEven(
    ...
    CUB_allocate_and_launch(st, stream_id, [&](void* workspace, size_t& size) {
      return hipcub::DeviceScan::InclusiveSum(workspace, size, st.fst(stream_id).d_global_bins(),
    ...
    CUB_allocate_and_launch(st, stream_id, [&](void* workspace, size_t& size) {
      return hipcub::DeviceScan::InclusiveSum(workspace, size, fst.d_local_bins(),
...
#pragma omp barrier
    ...
    if (fst.num_redistributed_samples > 0) {
      ...
      hipcub::TransformInputIterator<float, decltype(one_minus_val), float*> d_one_minus_labels(
          fst.d_sorted_labels(), one_minus_val);

      CUB_allocate_and_launch(st, stream_id, [&](void* workspace, size_t& size) {
        auto& fst = st.fst(stream_id);
        return hipcub::DeviceScan::InclusiveSum(workspace, size, fst.d_sorted_labels(), fst.d_tp(),
                                             fst.num_redistributed_samples, stream);
      });
      CUB_allocate_and_launch(st, stream_id, [&](void* workspace, size_t& size) {
        auto& fst = st.fst(stream_id);
        return hipcub::DeviceScan::InclusiveSum(workspace, size, d_one_minus_labels, fst.d_fp(),
                                             fst.num_redistributed_samples, stream);
      });
      ...
      CUB_allocate_and_launch(st, stream_id, [&](void* workspace, size_t& size) {
        auto& fst = st.fst(stream_id);
        return hipcub::DeviceRunLengthEncode::NonTrivialRuns(
      ...
      CUB_allocate_and_launch(st, stream_id, [&](void* workspace, size_t& size) {
        auto& fst = st.fst(stream_id);
        return hipcub::DeviceScan::ExclusiveSum(workspace, size, fst.d_pos_per_gpu(),
                                             fst.d_tp_offsets(), num_global_gpus_ + 1, stream);
      });

      CUB_allocate_and_launch(st, stream_id, [&](void* workspace, size_t& size) {
        auto& fst = st.fst(stream_id);
        return hipcub::DeviceScan::ExclusiveSum(workspace, size, fst.d_neg_per_gpu(),
...
void NDCGStorage::realloc_redistributed(size_t num_redistributed_samples, hipStream_t stream) {
...
float NDCG<T>::finalize_metric_per_gpu(int local_id) {
  ...
  hipcub::TransformInputIterator<float, decltype(clamp), float*> d_clamped_preds(st.d_preds(), clamp);

  CUB_allocate_and_launch(st, [&](void* workspace, size_t& size) {
    return hipcub::DeviceHistogram::HistogramEven(workspace, size, d_clamped_preds, st.d_local_bins(),
  ...
  // 3. Find num_global_gpus_-1 pivot points
  CUB_allocate_and_launch(st, [&](void* workspace, size_t& size) {
    return hipcub::DeviceScan::InclusiveSum(workspace, size, st.d_global_bins(),
  ...
  //    separated by pivot points.
  CUB_allocate_and_launch(st, [&](void* workspace, size_t& size) {
    return hipcub::DeviceScan::InclusiveSum(workspace, size, st.d_local_bins(), st.d_local_bins_sum(),
                                         num_bins_, stream);
  });
...
#pragma omp barrier
  ...
  if (st.num_redistributed_samples > 0) {
    ...
    CUB_allocate_and_launch(st, [&](void* workspace, size_t& size) {
      return hipcub::DeviceReduce::Sum(workspace, size, st.d_scaled_labels(), st.d_dcg(),
  ...
  CUB_allocate_and_launch(st, [&](void* workspace, size_t& size) {
    return hipcub::DeviceReduce::Sum(workspace, size, st.d_labels(), st.d_label_count(),
                                  num_local_samples, stream);
  });

  // Sort locally
  CUB_allocate_and_launch(st, [&](void* workspace, size_t& size) {
    return hipcub::DeviceRadixSort::SortKeys(workspace, size, st.d_labels(), st.d_sorted_labels(),
  ...
  CUB_allocate_and_launch(st, [&](void* workspace, size_t& size) {
    return hipcub::DeviceReduce::Sum(workspace, size, st.d_scaled_labels(), st.d_ideal_dcg(),
                                  num_local_samples, stream);
  });
...
float NDCG<T>::finalize_metric_single_gpu(int local_id) {
  ...
  // Compute DCG
  CUB_allocate_and_launch(st, [&](void* workspace, size_t& size) {
    return hipcub::DeviceRadixSort::SortPairs(workspace, size, st.d_preds(), st.d_sorted_preds(),
  ...
  // Compute Ideal DCG
  CUB_allocate_and_launch(st, [&](void* workspace, size_t& size) {
    return hipcub::DeviceRadixSort::SortKeys(workspace, size, st.d_sorted_labels(), st.d_labels(),
...
HitRate<T>::HitRate(int batch_size_per_gpu,
  ...
  for (int i = 0; i < num_local_gpus_; i++) {
    ...
    HCTR_LIB_THROW(hipMalloc((void**)(&(checked_count_[i])), sizeof(int)));
    HCTR_LIB_THROW(hipMalloc((void**)(&(hit_count_[i])), sizeof(int)));
  }
}
...
void HitRate<T>::free_all() {
  for (int i = 0; i < num_local_gpus_; i++) {
    int device_id = resource_manager_->get_local_gpu(i)->get_device_id();
    CudaDeviceContext context(device_id);
    HCTR_LIB_THROW(hipFree(checked_count_[i]));
    HCTR_LIB_THROW(hipFree(hit_count_[i]));
  }
}
...
void HitRate<T>::local_reduce(int local_gpu_id, Core23RawMetricMap raw_metrics) {
  ...
  HCTR_LIB_THROW(
      hipMemsetAsync(checked_count_[local_gpu_id], 0, sizeof(int), local_gpu->get_stream()));
  HCTR_LIB_THROW(
      hipMemsetAsync(hit_count_[local_gpu_id], 0, sizeof(int), local_gpu->get_stream()));
  ...
  int checked_host = 0;
  int hits_host = 0;
  HCTR_LIB_THROW(hipMemcpyAsync(&checked_host, checked_count_[local_gpu_id], sizeof(int),
                                 hipMemcpyDeviceToHost, local_gpu->get_stream()));
  checked_local_[local_gpu_id] = checked_host;
  HCTR_LIB_THROW(hipMemcpyAsync(&hits_host, hit_count_[local_gpu_id], sizeof(int),
                                 hipMemcpyDeviceToHost, local_gpu->get_stream()));
...
SMAPE<T>::SMAPE(int batch_size_per_gpu, const std::shared_ptr<ResourceManager>& resource_manager)
  ...
  for (int i = 0; i < num_local_gpus_; i++) {
    int device_id = resource_manager_->get_local_gpu(i)->get_device_id();
    CudaDeviceContext context(device_id);
    HCTR_LIB_THROW(hipMalloc((void**)(&(error_[i])), sizeof(float)));
  }
}
...
void SMAPE<T>::free_all() {
  for (int i = 0; i < num_local_gpus_; i++) {
    int device_id = resource_manager_->get_local_gpu(i)->get_device_id();
    CudaDeviceContext context(device_id);
    HCTR_LIB_THROW(hipFree(error_[i]));
  }
}
...
void SMAPE<T>::local_reduce(int local_gpu_id, Core23RawMetricMap raw_metrics) {
  ...
  hipMemsetAsync(error_[local_gpu_id], 0, sizeof(float), local_gpu->get_stream());
  ...
  HCTR_LIB_THROW(hipMemcpyAsync(&error_local_[local_gpu_id], error_[local_gpu_id], sizeof(float),
                                 hipMemcpyDeviceToHost, local_gpu->get_stream()));
}
...
ReallocBuffer<T, U>::ReallocBuffer() : num_elements_(0), ptr_(nullptr) {
  hipDevice_t device;
  HCTR_LIB_THROW(hipGetDevice(&device));

  prop_.type = hipMemAllocationTypePinned;
  prop_.location.type = hipMemLocationTypeDevice;
  prop_.location.id = device;
  HCTR_LIB_THROW(
      hipMemGetAllocationGranularity(&chunk_size_, &prop_, hipMemAllocationGranularityMinimum));
  access_desc_ = nullptr;
}

template <typename T, ReallocType_t U>
void ReallocBuffer<T, U>::init_access_desc(const std::vector<hipMemAccessDesc>* access_desc) {
...
template <typename T, ReallocType_t U>
void ReallocBuffer<T, U>::realloc(size_t new_num_elements, hipStream_t stream) {
  ...
  if (U == ReallocType_t::NO_COPY) {
    HCTR_LIB_THROW(hipFree(ptr_));
    HCTR_LIB_THROW(hipMalloc(&ptr_, new_size));
  } else if (U == ReallocType_t::DEFAULT) {
    void* tmp;
    HCTR_LIB_THROW(hipMalloc(&tmp, new_size));
    if (old_size) {
      HCTR_LIB_THROW(hipMemcpyAsync(tmp, ptr_, old_size, hipMemcpyDeviceToDevice, stream));
      HCTR_LIB_THROW(hipFree(ptr_));
    }
    ptr_ = (T*)tmp;
  } else {  // MMAP
...
void ReallocBuffer<T, U>::realloc_ptr_mmap(void** ptr, size_t old_size, size_t new_size) {
  ...
  hipDeviceptr_t new_ptr = 0;
  if (old_size == 0) {
    // Reserve a virtual address range
    HCTR_LIB_THROW(hipMemAddressReserve(&new_ptr, reserve_size, 0, 0, 0));
    vm_ranges_.push_back({new_ptr, reserve_size});
    *ptr = (void*)new_ptr;
  } else {
    // Try to reserve virtual memory at the end of old ptr
    const hipError_t status =
        hipMemAddressReserve(&new_ptr, reserve_size, 0, (hipDeviceptr_t)((uint64_t)*ptr + old_size), 0);

    if ((status != hipSuccess) || ((void*)new_ptr != (void*)((uint64_t)*ptr + old_size))) {
      ...
      if (new_ptr) {
        HCTR_LIB_THROW(hipMemAddressFree(new_ptr, reserve_size));
      }

      // Now reserve the new, bigger VA range
      HCTR_LIB_THROW(hipMemAddressReserve(&new_ptr, new_size, 0, 0, 0));

      // Map first part of new VA range to existing physical memory chunks, enabling their access
      std::vector<std::pair<hipDeviceptr_t, size_t>> new_mmap_ranges;
      // HugeCTR ROCm port: hipDeviceptr_t is `void*` on HIP (vs CUdeviceptr=size_t
      // on CUDA), so pointer arithmetic needs an explicit char* cast.
      hipDeviceptr_t tmp = new_ptr;
      MMAP_DEBUG("Remapping VM -> PM\n");
      for (auto handle : pm_handles_) {
        auto size = handle.second;
        HCTR_LIB_THROW(hipMemMap(tmp, size, 0, handle.first, 0));
        new_mmap_ranges.push_back({tmp, size});
        tmp = static_cast<hipDeviceptr_t>(static_cast<char*>(tmp) + size);
      }

      // Set access permissions
      HCTR_LIB_THROW(
          hipMemSetAccess(new_ptr, old_size, &(access_desc_->at(0)), access_desc_->size()));

      // Unmap old mappings
      for (auto range : mmap_ranges_) {
        HCTR_LIB_THROW(hipMemUnmap(range.first, range.second));
      }
      ...
      for (auto range : vm_ranges_) {
        HCTR_LIB_THROW(hipMemAddressFree(range.first, range.second));
      }
      ...
      *ptr = (void*)new_ptr;
      new_ptr = (hipDeviceptr_t)(((uint64_t)new_ptr) + old_size);
    } else {
      vm_ranges_.push_back({new_ptr, reserve_size});
    }
  }
  ...
  HCTR_LIB_THROW(hipMemCreate(&allocHandle, reserve_size, &prop_, 0));
  pm_handles_.push_back({allocHandle, reserve_size});

  // Map new_ptr to physical memory
  HCTR_LIB_THROW(hipMemMap(new_ptr, reserve_size, 0, allocHandle, 0));
  mmap_ranges_.push_back({new_ptr, reserve_size});

  // Set access permissions
  HCTR_LIB_THROW(
      hipMemSetAccess(new_ptr, reserve_size, &(access_desc_->at(0)), access_desc_->size()));
}
...
void ReallocBuffer<T, U>::release_mmap_memory() {
  ...
  for (auto handle : pm_handles_) {
    HCTR_LIB_THROW(hipMemRelease(handle.first));
  }
  pm_handles_.clear();
}
