...
CPUResource::CPUResource(unsigned long long replica_uniform_seed,
  ...
  for (size_t i = 0; i < replica_variant_seeds.size(); i++) {
    HCTR_LIB_THROW(hiprandCreateGeneratorHost(&replica_uniform_curand_generators_[i],
                                             HIPRAND_RNG_PSEUDO_DEFAULT));
    HCTR_LIB_THROW(hiprandSetPseudoRandomGeneratorSeed(replica_uniform_curand_generators_[i],
                                                      replica_uniform_seed));
    HCTR_LIB_THROW(hiprandCreateGeneratorHost(&replica_variant_curand_generators_[i],
                                             HIPRAND_RNG_PSEUDO_DEFAULT));
    HCTR_LIB_THROW(hiprandSetPseudoRandomGeneratorSeed(replica_variant_curand_generators_[i],
...
CPUResource::~CPUResource() {
  try {
    for (auto generator : replica_uniform_curand_generators_) {
      HCTR_LIB_THROW(hiprandDestroyGenerator(generator));
    }
