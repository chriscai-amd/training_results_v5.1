...
HashTable<KeyType, ValType>::HashTable(size_t capacity, size_t count) : capacity_(capacity) {
  ...
  HCTR_LIB_THROW(hipMalloc((void**)&d_counter_, sizeof(size_t)));
  HCTR_LIB_THROW(hipMalloc((void**)&d_container_size_, sizeof(size_t)));
  HCTR_LIB_THROW(hipMemcpy(d_counter_, &count, sizeof(size_t), hipMemcpyHostToDevice));
}
...
HashTable<KeyType, ValType>::~HashTable() {
  try {
    ...
    HCTR_LIB_THROW(hipFree(d_counter_));
    HCTR_LIB_THROW(hipFree(d_container_size_));
...
size_t HashTable<KeyType, ValType>::get_size(hipStream_t stream) const {
  ...
  HCTR_LIB_THROW(hipMemsetAsync(d_container_size_, 0, sizeof(size_t), stream));
  size_kernel<<<grid_size, BLOCK_SIZE_, 0, stream>>>(container_, hash_capacity, d_container_size_,
                                                     empty_key);
  HCTR_LIB_THROW(hipMemcpyAsync(&container_size, d_container_size_, sizeof(size_t),
...
size_t HashTable<KeyType, ValType>::get_value_head(hipStream_t stream) const {
  size_t counter;
  HCTR_LIB_THROW(
      hipMemcpyAsync(&counter, d_counter_, sizeof(size_t), hipMemcpyDeviceToHost, stream));
  HCTR_LIB_THROW(hipStreamSynchronize(stream));
  return counter;
}
...
void HashTable<KeyType, ValType>::dump(KeyType* d_key, ValType* d_val, size_t* d_dump_counter,
  ...
  HCTR_LIB_THROW(hipMemsetAsync(d_dump_counter, 0, sizeof(size_t), stream));
