...
void FusedReshapeConcatGeneralLayer<T>::initialize() {
  ...
  HCTR_LIB_THROW(hipMemcpyAsync((void*)vecs_size_.data(), (void*)h_vecs_size_.data(),
                                 num_ * sizeof(int64_t), hipMemcpyHostToDevice,
                                 get_gpu().get_stream()));

  HCTR_LIB_THROW(hipMemcpyAsync((void*)d_inputs_.data(), (void*)h_inputs_.data(),
...
void FusedReshapeConcatGeneralLayer<T>::fprop(bool is_train) {
  ...
  fused_reshape_concat_general_kernel<<<grid_size, block_size, 0, get_gpu().get_stream()>>>(
      true, d_inputs_.template data<T*>(), output, batch_size_, slot_num_, vecs_size_.template data<size_t>(),
      new_width_, num_);
}
...
void FusedReshapeConcatGeneralLayer<T>::bprop() {
  ...
  dim3 grid_size(n_sms * 8, 1, 1);
  fused_reshape_concat_general_kernel<<<grid_size, block_size, 0, get_gpu().get_stream()>>>(
      false, d_inputs_.template data<T*>(), output, batch_size_, slot_num_, vecs_size_.template data<size_t>(),
      new_width_, num_);
}
