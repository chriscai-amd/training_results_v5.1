...
#include <algorithm>
#include <prims/hip_utils.cuh>  // ROCm port
#include <functional>
...
void ScaleLayer<T>::fprop(bool is_train) {
  ...
  scale(out_tensor.template data<T>(), in_tensor.template data<T>(), in_tensor_dim[0], in_tensor_dim[1], axis, factor,
...
void ScaleLayer<T>::bprop() {
  ...
  scale(bottom_tensor.template data<T>(), top_tensor.template data<T>(), bottom_tensor_dim[0], bottom_tensor_dim[1],
