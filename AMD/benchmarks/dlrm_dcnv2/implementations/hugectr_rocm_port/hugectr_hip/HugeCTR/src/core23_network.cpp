...
float Network::get_loss() {
  ...
  for (auto& loss_tensor : train_loss_tensor_) {
    HCTR_LIB_THROW(hipMemcpyAsync(&loss_temp[i], loss_tensor.second.data(), sizeof(float),
                                   hipMemcpyDeviceToHost, gpu_resource_->get_stream()));
    ++i;
  }
  HCTR_LIB_THROW(hipStreamSynchronize(gpu_resource_->get_stream()));
...
void Network::download_params_to_host(const std::string& write_path) {
  ...
  HCTR_LIB_THROW(hipMemcpy(weight.get(), train_weight_tensor_->data(),
                            train_weight_tensor_->num_bytes(), hipMemcpyDeviceToHost));
...
void Network::download_opt_states_to_host(const std::string& write_path) {
  ...
  HCTR_LIB_THROW(hipMemcpy(h_opt_states.get(), src, dst_size_in_byte, hipMemcpyDeviceToHost));
...
void Network::upload_params_to_device(const std::string& model_file) {
  ...
  HCTR_LIB_THROW(hipMemcpy(train_weight_tensor_->data(), params.get(),
                            train_weight_tensor_->num_bytes(), hipMemcpyHostToDevice));
  return;
}
...
void Network::download_params_to_host(float* weight) {
  ...
  HCTR_LIB_THROW(hipMemcpy(weight, train_weight_tensor_->data(), train_weight_tensor_->num_bytes(),
...
void Network::upload_params_to_device(float* params) {
  ...
  HCTR_LIB_THROW(hipMemcpy(train_weight_tensor_->data(), params, train_weight_tensor_->num_bytes(),
...
void Network::upload_opt_states_to_device(char* h_opt_states) {
  ...
  HCTR_LIB_THROW(hipMemcpy(dst, h_opt_states, src_size_in_byte, hipMemcpyHostToDevice));
...
void Network::copy_weights_from_train_layers_to_evaluate_layers() {
  CudaDeviceContext context(get_device_id());
  HCTR_LIB_THROW(hipMemcpyAsync(evaluate_weight_tensor_->data(), train_weight_tensor_->data(),
                                 train_weight_tensor_->num_bytes(), hipMemcpyDeviceToDevice,
...
void Network::copy_non_trainable_params_from_train_layers_to_evaluate_layers() {
  ...
  for (size_t i{0}; i < train_layers_.size(); ++i) {
    ...
    for (size_t j{0}; j < tensors_in_train_layers.size(); ++j) {
      HCTR_LIB_THROW(hipMemcpyAsync(tensors_in_evaluate_layers[j].data(),
                                     tensors_in_train_layers[j].data(),
                                     tensors_in_train_layers[j].num_bytes(),
                                     hipMemcpyDeviceToDevice, gpu_resource_->get_stream()));
    }
  }
}
