...
DataReaderImpl::DataReaderImpl(const std::vector<FileSource>& source_files,
  ...
  for (size_t i = 0; i < batch_buffers_.size(); ++i) {
    ...
    for (auto& local_batch : batch->local_batches) {
      ...
      for (auto source : source_files) {
        ...
        HCTR_LIB_THROW(hipMalloc(&ptr, local_batch_size_bytes));
        HCTR_LIB_THROW(hipMemset(ptr, 0, local_batch_size_bytes));
  ...
  for (size_t i = 0; i < resource_manager->get_local_gpu_count(); ++i) {
    ...
    HCTR_LIB_THROW(hipStreamCreateWithPriority(&stream, hipStreamNonBlocking, -100));
    placement_streams_.push_back(stream);

    hipEvent_t event;
    HCTR_LIB_THROW(hipEventCreate(&event));
    placement_events_.push_back(event);

    hipStream_t cb_stream;
    hipEvent_t cb_event;
    HCTR_LIB_THROW(hipStreamCreateWithFlags(&cb_stream, hipStreamNonBlocking));
    HCTR_LIB_THROW(hipEventCreate(&cb_event));
...
void DataReaderImpl::device_release_last_batch_here(hipStream_t stream, int gpu_id) const {
  ...
  HCTR_LIB_THROW(hipEventRecord(callback_events_[gpu_id], stream));
  HCTR_LIB_THROW(hipStreamWaitEvent(cb_stream, callback_events_[gpu_id]));
  HCTR_LIB_THROW(hipStreamAddCallback(cb_stream, &DataReaderImpl::release_batch_callback,
                                       (void*)last_batch_, 0));
}

void DataReaderImpl::schedule_upload_here(int raw_device_id, hipStream_t stream, bool from_graph) {
  if (schedule_uploads_) {
    unsigned int flags = from_graph ? hipEventRecordExternal : 0;
    HCTR_LIB_THROW(hipEventRecordWithFlags(placement_events_[raw_device_id], stream, flags));
  }
}
...
void DataReaderImpl::upload_batches(size_t device_id) {
  ...
  hipStream_t& stream = placement_streams_[device_id];
  ...
  while (running_) {
    ...
    if (batch->state.load(std::memory_order_acquire) == BatchState::READY_TO_UPLOAD &&
      ...
      if (schedule_uploads_) {
        ...
        pending_transfers_[device_id].raw--;
        HCTR_LIB_THROW(hipStreamWaitEvent(stream, placement_events_[device_id]));
      }
  ...
  HCTR_LIB_THROW(hipStreamDestroy(stream));
}

void DataReaderImpl::release_batch_callback(hipStream_t stream, hipError_t status,
                                            void* user_data) {
