...
void FtrlOptimizer<T>::initialize() {
  HCTR_LIB_THROW(
      hipMemsetAsync(n_tensor_.data(), 0, n_tensor_.num_bytes(), gpu_resource_->get_stream()));
  HCTR_LIB_THROW(
      hipMemsetAsync(z_tensor_.data(), 0, z_tensor_.num_bytes(), gpu_resource_->get_stream()));
}
