...
IbComm::HierA2ACollContext::HierA2ACollContext(IbComm* comm, size_t num_buffers = 0) {
  HCTR_LIB_THROW(hipHostMalloc(&cmd_storage_, 2 * sizeof(size_t)));
  ...
  *h_recv_cmd_ptr_ = 1;
  ...
  for (size_t g = 0; g < num_gpus; g++) {
    HCTR_LIB_THROW(hipSetDevice(comm->device_list_[g]));
    HCTR_LIB_THROW(hipEventCreate(&ctx_[g]->event_));

    // TODO: collate all storage
    HCTR_LIB_THROW(hipMalloc((void**)&d_send_cmd_[g], sizeof(size_t)));
    size_t init_value = 2;
    HCTR_LIB_THROW(hipMemcpy(d_send_cmd_[g], &init_value, sizeof(size_t), hipMemcpyHostToDevice));

    HCTR_LIB_THROW(hipMalloc((void**)&d_ibv_atomic_[g], MAX_IBV_DEST * sizeof(size_t)));
    size_t atomic_init_values[MAX_IBV_DEST];
    std::fill_n(atomic_init_values, MAX_IBV_DEST, 1);
    HCTR_LIB_THROW(hipMemcpy(d_ibv_atomic_[g], atomic_init_values, MAX_IBV_DEST * sizeof(size_t),
                              hipMemcpyHostToDevice));

    HCTR_LIB_THROW(hipMalloc((void**)&d_ibv_atomic_recv_[g], MAX_IBV_DEST * sizeof(size_t)));
    std::fill_n(atomic_init_values, MAX_IBV_DEST, 0);
    HCTR_LIB_THROW(hipMemcpy(d_ibv_atomic_recv_[g], atomic_init_values,
                              MAX_IBV_DEST * sizeof(size_t), hipMemcpyHostToDevice));
  }
...
IbComm::HierA2ACollContext::~HierA2ACollContext() {
  ...
  if (d_ibv_atomic_recv_) {
    for (size_t g = 0; g < num_gpus; g++) {
      HCTR_LIB_CHECK_(hipFree(d_ibv_atomic_recv_[g]));
    }
    delete[] d_ibv_atomic_recv_;
  }
  if (d_ibv_atomic_) {
    for (size_t g = 0; g < num_gpus; g++) {
      HCTR_LIB_CHECK_(hipFree(d_ibv_atomic_[g]));
    }
    delete[] d_ibv_atomic_;
  }
  if (d_send_cmd_) {
    for (size_t g = 0; g < num_gpus; g++) {
      HCTR_LIB_CHECK_(hipFree(d_send_cmd_[g]));
    }
    delete[] d_send_cmd_;
  }
  if (cmd_storage_) {
    HCTR_LIB_CHECK_(hipHostFree(cmd_storage_));
  }
}
...
IbComm::HierA2ACollContextPerGPU::~HierA2ACollContextPerGPU() {
  ...
  if (d_send_sizes_copy_) {
    HCTR_LIB_CHECK_(hipFree(d_send_sizes_copy_));
  }
}
...
void IbComm::set_a2a_coll_stream(HierA2ACollHandle coll, hipStream_t stream, size_t device_id) {
  hier_a2a_coll_ctx_[coll]->ctx_[device_id]->stream_ = stream;
}

void IbComm::set_a2a_coll_stream(HierA2AvCollHandle coll, hipStream_t stream, size_t device_id) {
...
void IbComm::set_a2a_coll_buf(HierA2AvCollHandle coll, void* send_ptrs, const size_t send_max_size,
  ...
  HCTR_LIB_THROW(hipSetDevice(device_list_[device_id]));

  // Allocate A2Av send size copy storage
  HCTR_LIB_THROW(hipMalloc((void**)(&gpu_ctx.d_send_sizes_copy_),
  ...
  HCTR_LIB_THROW(hipMemcpy(gpu_ctx.d_send_sizes_copy_, send_sizes.data(),
                            sizeof(size_t) * coll_ctx.num_buffers_ * num_procs_,
                            hipMemcpyHostToDevice));
...
void IbComm::update_a2a_coll_sizes(HierA2AvCollHandle coll, const size_t* d_send_sizes,
                                   const size_t* d_recv_sizes, hipStream_t dep_stream,
                                   size_t device_id) {
  auto& ctx = *hier_a2a_v_coll_ctx_[coll];
  auto& gpu_ctx = *ctx.ctx_[device_id];
  HCTR_LIB_THROW(hipEventRecord(gpu_ctx.event_, dep_stream));
  HCTR_LIB_THROW(hipStreamWaitEvent(gpu_ctx.stream_, gpu_ctx.event_));
...
void IbComm::pre_intra_update_a2a_coll_sizes(HierA2AvCollHandle coll,
                                             ...
                                             hipStream_t dep_stream, size_t device_id) {
  auto& ctx = *hier_a2a_v_coll_ctx_[coll];
  auto& gpu_ctx = *ctx.ctx_[device_id];
  HCTR_LIB_THROW(hipEventRecord(gpu_ctx.event_, dep_stream));
  HCTR_LIB_THROW(hipStreamWaitEvent(gpu_ctx.stream_, gpu_ctx.event_));
...
template <typename T>
void IbComm::post_send_command_a2a<T>(HierA2ACollHandle coll, hipStream_t dep_stream,
                                      size_t device_id) {
...
template <typename T>
void IbComm::post_send_command_a2a<T>(HierA2AvCollHandle coll, hipStream_t dep_stream,
                                      size_t device_id) {
  auto& ctx = *hier_a2a_v_coll_ctx_[coll];
  auto& gpu_ctx = *ctx.ctx_[device_id];
  HCTR_LIB_THROW(hipEventRecord(gpu_ctx.event_, dep_stream));
...
template <typename T>
void IbComm::post_a2a_send_command<T>(HierA2AvCollHandle coll, hipStream_t dep_stream,
                                      size_t device_id) {
  auto& ctx = *hier_a2a_v_coll_ctx_[coll];
  auto& gpu_ctx = *ctx.ctx_[device_id];
  HCTR_LIB_THROW(hipEventRecord(gpu_ctx.event_, dep_stream));
...
void IbComm::blocking_wait(HierA2AvCollHandle coll, hipStream_t dep_stream, size_t device_id) {
  auto& ctx = *hier_a2a_v_coll_ctx_[coll];
  auto& gpu_ctx = *ctx.ctx_[device_id];
  HCTR_LIB_THROW(hipEventRecord(gpu_ctx.event_, dep_stream));
  HCTR_LIB_THROW(hipStreamWaitEvent(gpu_ctx.stream_, gpu_ctx.event_));
...
template void IbComm::post_send_command_a2a<__half>(HierA2ACollHandle coll, hipStream_t dep_stream,
                                                    size_t device_id);
template void IbComm::post_send_command_a2a<float>(HierA2ACollHandle coll, hipStream_t dep_stream,
                                                   size_t device_id);
...
template void IbComm::post_send_command_a2a<__half>(HierA2AvCollHandle coll,
                                                    hipStream_t dep_stream, size_t device_id);
template void IbComm::post_send_command_a2a<float>(HierA2AvCollHandle coll, hipStream_t dep_stream,
                                                   size_t device_id);
...
template void IbComm::post_a2a_send_command<__half>(HierA2AvCollHandle coll,
                                                    hipStream_t dep_stream, size_t device_id);
template void IbComm::post_a2a_send_command<float>(HierA2AvCollHandle coll, hipStream_t dep_stream,
                                                   size_t device_id);
