...
class DynamicEmbeddingTableCPU final : public IDynamicEmbeddingTable {
 ...
 public:
  ...
  void lookup(const core23::Tensor& keys, size_t num_keys,
    ...
    w_dev.resize(k.size());
    HCTR_LIB_THROW(hipMemcpy(w_dev.data(), embedding_vec.data<float*>(),
                              w_dev.size() * sizeof(float*), hipMemcpyDeviceToHost));
    ...
    for (size_t i = 0; i < is.size(); ++i) {
      ...
      for (uint32_t off = is_off[i]; off < next_off; ++off) {
        ...
        HCTR_LIB_THROW(
            hipMemcpy(w_dev[off], w->data(), w->size() * sizeof(float), hipMemcpyHostToDevice));
      }
    }
  }
