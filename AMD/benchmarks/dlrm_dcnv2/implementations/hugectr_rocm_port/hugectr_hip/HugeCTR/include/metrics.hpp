...
class ReallocBuffer {
 public:
  ...
  void realloc(size_t new_num_elements, hipStream_t stream = 0);
  T* get_ptr() { return ptr_; }
  void init_access_desc(const std::vector<hipMemAccessDesc>* access_desc);
 ...
 private:
  ...
  hipMemAllocationProp prop_;
  const std::vector<hipMemAccessDesc>* access_desc_;  // Virtual memory access descriptor
  std::vector<std::pair<hipMemGenericAllocationHandle_t, size_t>> pm_handles_;
  std::vector<std::pair<hipDeviceptr_t, size_t>> vm_ranges_;    // Virtual allocation info
  std::vector<std::pair<hipDeviceptr_t, size_t>> mmap_ranges_;  // Memmap VA->PA info
...
class AUCStorage {
 public:
  ...
  void realloc_redistributed(size_t num_redistributed_samples, hipStream_t stream,
                             size_t stream_id);
 ...
 private:
  ...
  std::vector<hipMemAccessDesc> access_desc_;  // Access descriptors used by ReallocBuffers
...
class AUC : public Metric {
 ...
 private:
  ...
  std::vector<std::vector<hipStream_t>> streams_;
