...
class DataReaderImpl {
 public:
  ...
  void device_release_last_batch_here(hipStream_t stream, int gpu_id) const;

  void schedule_upload_here(int device_id, hipStream_t stream, bool from_graph);
 ...
 private:
  ...
  static void release_batch_callback(hipStream_t stream, hipError_t status,
                                               void* user_data);
  ...
  std::vector<hipStream_t> placement_streams_;
  std::vector<hipEvent_t> placement_events_;

  std::vector<hipStream_t> callback_streams_;
  std::vector<hipEvent_t> callback_events_;
};
