...
class MLPLayer : public TrainableLayer<T> {
  ...
  // Phase 14n.2 weight pre-transpose path: per-layer scratch buffers for
  // K×N col-major transposed weight, used by CK-Tile fused MLP kernel.
  std::vector<__half*> cktile_kernel_T_buffers_;
  std::vector<bool>    cktile_kernel_T_allocated_;
 ...
 public:
  ...
  ~MLPLayer() {
    ...
    if (event_overlap_created_) {
      hipEventDestroy(event_overlap_);
    }
    for (size_t i = 0; i < cktile_kernel_T_buffers_.size(); ++i) {
      if (cktile_kernel_T_allocated_[i] && cktile_kernel_T_buffers_[i] != nullptr) {
        hipFree(cktile_kernel_T_buffers_[i]);
      }
    }
  };
};
