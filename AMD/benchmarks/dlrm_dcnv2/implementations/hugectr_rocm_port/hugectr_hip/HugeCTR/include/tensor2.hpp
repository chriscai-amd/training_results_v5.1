...
namespace cuda {
template <typename T>
void copy_async(SparseTensor<T> &dst, const SparseTensor<T> &src, hipMemcpyKind kind,
                hipStream_t stream) {
  HCTR_LIB_THROW(hipMemcpyAsync(dst.get_value_ptr(), src.get_value_ptr(), src.nnz() * sizeof(T),
                                 kind, stream));

  HCTR_LIB_THROW(hipMemcpyAsync(dst.get_rowoffset_ptr(), src.get_rowoffset_ptr(),
...
template <typename T>
void copy_async(SparseTensor<T> &dst, const CSR<T> &src, hipStream_t stream) {
  HCTR_LIB_THROW(hipMemcpyAsync(dst.get_value_ptr(), src.get_value_tensor().get_ptr(),
                                 src.get_num_values() * sizeof(T), hipMemcpyHostToDevice, stream));

  HCTR_LIB_THROW(hipMemcpyAsync(dst.get_rowoffset_ptr(), src.get_row_offset_tensor().get_ptr(),
