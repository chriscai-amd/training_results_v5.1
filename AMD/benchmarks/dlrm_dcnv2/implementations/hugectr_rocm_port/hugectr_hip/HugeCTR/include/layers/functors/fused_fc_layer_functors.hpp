...
class FusedFCLayerFunctors {
 ...
 public:
  void fprop(const T* kernel, const T* bottom, T* top,
             ...
             const CublasFusedFCLayerAlgo<T>& cublas_layer_algo, hipblasLtHandle_t cublaslt_handle,
             hipStream_t stream);
  ...
  void bprop(const T* kernel, const T* bottom, const T* train_top, const T* mask_aux,
             ...
             const CublasFusedFCLayerDesc<T>& cublas_layer_desc,
             const CublasFusedFCLayerAlgo<T>& cublas_layer_algo, hipblasLtHandle_t cublaslt_handle,
             hipStream_t stream, hipStream_t overlap_stream, hipEvent_t& event_overlap,
...
// HugeCTR ROCm port: change explicit instantiation *definitions* to
// *declarations*, so only the dedicated TU (fused_fc_layer_functors.cu) emits
// the definitions. Without this the symbols vanish under amdclang's -O3 + HIP
// pipeline (the explicit-definition copy in the header gets compiled but never
// actually emitted to the .o for reasons we don't fully understand).
extern template class CublasFusedFCLayerDesc<float>;
extern template class CublasFusedFCLayerDesc<__half>;

extern template class CublasFusedFCLayerAlgo<float>;
extern template class CublasFusedFCLayerAlgo<__half>;

extern template class FusedFCLayerFunctors<float>;
extern template class FusedFCLayerFunctors<__half>;
