...
class BatchNormLayer : public TrainableLayer<T, true> {
 ...
 private:
  ...
  const hipdnnBatchNormMode_t mode_;
  hipdnnTensorDescriptor_t in_out_desc_;
  hipdnnTensorDescriptor_t gamma_beta_desc_;
#else
  // ROCm port: cuDNN/MIOpen-based BN implementation excluded; placeholder.
  int mode_ = 0;
  void* in_out_desc_ = nullptr;
  void* gamma_beta_desc_ = nullptr;
#endif
