...
#define HCTR_CUDA_KERNEL_SUMMARY(MODE, FUNC, BLOCKSIZE, DSHMEM)                                 \
  do {                                                                                          \
    ...
    int max_blocks = 0;                                                                         \
    HCTR_CUDA_CHECK(ASYNC, hipFuncGetAttributes(&func_attr, reinterpret_cast<const void*>(FUNC)));                            \
    HCTR_CUDA_CHECK(ASYNC, hipOccupancyMaxActiveBlocksPerMultiprocessor(&max_blocks, FUNC,     \
                                                                         BLOCKSIZE, DSHMEM));   \
    HCTR_LOG(MODE, WORLD,                                                                       \
             #FUNC                                                                              \
             " Summary:\n"                                                                      \
...
#define HCTR_CUDA_KERNEL_TIME(MODE, KERNEL_LAUNCH, ITER)              \
  do {                                                                \
    hipEvent_t st_time, ed_time;                                     \
    hipEventCreate(&st_time);                                        \
    hipEventCreate(&ed_time);                                        \
    hipEventRecord(st_time, stream);                                 \
    for (size_t i = 0; i < ITER; ++i) {                               \
      KERNEL_LAUNCH;                                                  \
    }                                                                 \
    hipEventRecord(ed_time, stream);                                 \
    hipEventSynchronize(ed_time);                                    \
    HCTR_CUDA_CHECK(ASYNC, hipGetLastError());                       \
    float elapsed_ms = 0.0f;                                          \
    hipEventElapsedTime(&elapsed_ms, st_time, ed_time);              \
    HCTR_LOG(MODE, WORLD,                                             \
             #KERNEL_LAUNCH                                           \
             ":\n"                                                    \
             "\tAvg. (%zu iterations) elapsed time in usec = %.0f\n", \
             ITER, elapsed_ms * 1000 / ITER);                         \
    hipEventDestroy(st_time);                                        \
    hipEventDestroy(ed_time);                                        \
  } while (0)
