...
SparseMPDataDistributionOp::MPTempStorage::MPTempStorage(
  ...
  }
  {
    ...
    DISPATCH_INTEGRAL_FUNCTION_CORE23(offset_type.type(), BucketRangeType, [&] {
      hipcub::DeviceScan::InclusiveSum(nullptr, temp_bytes, (BucketRangeType*)nullptr,
  ...
  DISPATCH_INTEGRAL_FUNCTION_CORE23(offset_type.type(), offset_t, [&] {
    HCTR_LIB_THROW(hipHostMalloc((void**)&this->h_send_k_per_g,
                                  core->get_global_gpu_count() * sizeof(offset_t)));
    HCTR_LIB_THROW(hipHostMalloc((void**)&this->h_recv_k_per_g,
                                  core->get_global_gpu_count() * sizeof(offset_t)));
  });
}
...
void SparseMPDataDistributionOp::all2all_keys(embedding::EmbeddingInput& output,
  ...
  // TODO: compare with hipcub::DeviceSegmentedReduce::Sum
  {
  ...
  DISPATCH_INTEGRAL_FUNCTION_CORE23(ebc_param_.key_type.type(), KeyType, [&] {
    ...
    hipcub::DeviceRadixSort::SortPairs(temp_ptr, temp_bytes, labels_in, labels_out, keys_in, keys_out,
  ...
  HCTR_LIB_THROW(hipMemcpyAsync(h_send_k_per_g, send_k_per_g.data(), send_k_per_g.num_bytes(),
                                 hipMemcpyDeviceToHost, stream));
  HCTR_LIB_THROW(hipMemcpyAsync(h_recv_k_per_g, recv_k_per_g.data(), recv_k_per_g.num_bytes(),
                                 hipMemcpyDeviceToHost, stream));
  HCTR_LIB_THROW(hipStreamSynchronize(stream));
...
void SparseMPDataDistributionOp::filter_after_all2all(embedding::EmbeddingInput& output,
  ...
  DISPATCH_INTEGRAL_FUNCTION_CORE23(ebc_param_.offset_type.type(), BucketRangeType, [&] {
    ...
    // computes in-place!
    HCTR_LIB_THROW(hipMemsetAsync(bucket_range_gpu_major.data<BucketRangeType>(), 0,
                                   sizeof(BucketRangeType), stream));
    hipcub::DeviceScan::InclusiveSum(temp_ptr, temp_bytes, k_per_b_gpu_major.data<BucketRangeType>(),
  ...
  DISPATCH_INTEGRAL_FUNCTION_CORE23(ebc_param_.offset_type.type(), BucketRangeType, [&] {
    ...
    HCTR_LIB_THROW(hipMemsetAsync(output.bucket_range.data<BucketRangeType>(), 0,
                                   sizeof(BucketRangeType), stream));
    hipcub::DeviceScan::InclusiveSum(temp_ptr, temp_bytes, k_per_b_feat_major.data<BucketRangeType>(),
