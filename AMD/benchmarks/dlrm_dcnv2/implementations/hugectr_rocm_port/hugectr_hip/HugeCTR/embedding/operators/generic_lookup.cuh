  ...
  if (i_ev < copy_desc.num_vec_) {
    ...
    for (int r = 0; r < L; r += kWarpSize) {
      ...
      for (int j = 0; j < kWarpSize && r + j < L; ++j) {
        int j_ev = __shfl_sync(0xFFFFFFFFFFFFFFFFULL, l, j);
...
void copy_one_to_one(CopyDesc copy_desc, int max_ev_size, hipStream_t stream,
...
void copy_one_to_one(CopyDesc copy_desc, const HugeCTR::core23::KernelParams &kernel_params,
                     int max_ev_size, hipStream_t stream, bool only_reset = false) {
  if (only_reset) {
    int grid_size = (copy_desc.num_vec_ - 1) / 2 + 1;
    dim3 block_size{32, 2};
    reset_warp_per_ev_vec4_kernel<CopyDesc, 1><<<grid_size, block_size, 0, stream>>>(copy_desc);
...
template <typename CopyDesc>
void copy_multi_to_one(CopyDesc copy_desc, int max_ev_size, hipStream_t stream) {
  // ROCm port: HCTR_LOOKUP_WARPS_PER_BLOCK env knob lets us A/B test wave64
  // efficiency. Default = 2 (matches NV's CUDA-warp-32 split-warp tiling).
  // With value = 1, we use one logical CUDA warp per block, doubling the
  // grid; this proves whether the second warp on wave64 is doing useful
  // work or duplicating warp 0's cross-warp __shfl_sync(mask=0xFF...FFULL,
  // src_lane in [0,32)) reads.
  static const int kWarpsPerBlock = []() {
    const char* env = getenv("HCTR_LOOKUP_WARPS_PER_BLOCK");
    return env ? std::max(1, std::atoi(env)) : 2;
  }();
  if (max_ev_size <= 128) {
    int grid_size = (copy_desc.num_vec_ - 1) / kWarpsPerBlock + 1;
    dim3 block_size{32, (unsigned int)kWarpsPerBlock};
...
template <typename CopyDesc>
void copy_multi_to_one_weight(CopyDesc copy_desc, int max_ev_size, hipStream_t stream) {
  if (max_ev_size <= 128) {
    int grid_size = (copy_desc.num_vec_ - 1) / 2 + 1;
    dim3 block_size{32, 2};
...
template <typename CopyDesc>
void copy_one_to_multi(CopyDesc copy_desc, int max_ev_size, hipStream_t stream) {
  if (max_ev_size <= 128) {
    int grid_size = (copy_desc.num_vec_ - 1) / 2 + 1;
    dim3 block_size{32, 2};
...
template <typename CopyDesc>
void copy_one_to_multi_weight(CopyDesc copy_desc, int max_ev_size, hipStream_t stream) {
  if (max_ev_size <= 128) {
    int grid_size = (copy_desc.num_vec_ - 1) / 2 + 1;
    dim3 block_size{32, 2};
...
void one_to_one_atomic(CopyDesc desc, const HugeCTR::core23::KernelParams &kernel_params,
                       int ev_size, int key_num, hipStream_t stream) {
