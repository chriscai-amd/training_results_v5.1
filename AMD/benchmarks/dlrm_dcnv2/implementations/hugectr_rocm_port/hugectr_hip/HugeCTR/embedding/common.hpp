...
core23::Tensor init_tensor_list(const std::vector<core23::Tensor> &tensor_vec, int device_id,
  ...
  if (stream != 0) {
    HCTR_LIB_THROW(hipMemcpyAsync(tensor_list.data(), data_vec.data(),
                                   data_vec.size() * sizeof(BuiltInType *), hipMemcpyHostToDevice,
                                   stream));
  } else {
    HCTR_LIB_THROW(hipMemcpy(tensor_list.data(), data_vec.data(),
                              data_vec.size() * sizeof(BuiltInType *), hipMemcpyHostToDevice));
  }
...
static void init_tensor_list(core23::Tensor &tensor_list,
  ...
  if (tensor_list.device() == Device(DeviceType::CPU)) {
    ...
    if (stream != 0) {
      HCTR_LIB_THROW(hipMemcpyAsync(tensor_list.data(), data_vec.data(),
                                     data_vec.size() * sizeof(void *), hipMemcpyHostToDevice,
                                     stream));
    } else {
      HCTR_LIB_THROW(hipMemcpy(tensor_list.data(), data_vec.data(),
                                data_vec.size() * sizeof(void *), hipMemcpyHostToDevice));
...
struct DataDistributionInput {
  ...
  void copy_tensor_vec(const std::vector<core23::Tensor> &dp_keys,
                       const std::vector<core23::Tensor> &dp_bucket_range, hipStream_t stream);
