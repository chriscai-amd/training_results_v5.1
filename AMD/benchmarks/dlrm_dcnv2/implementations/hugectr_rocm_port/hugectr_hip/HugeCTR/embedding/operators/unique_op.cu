...
// HIP/ROCm: native atomicCAS for unsigned long/long long exists but ADL/name
// lookup behavior in templated kernels can fail to find them when local
// overloads are present at file scope. Provide all 64-bit signed/unsigned
// passthroughs explicitly to avoid surprises.
...
#ifdef __HIPCC__
// On AMD64, uint64_t aliases unsigned long, so this single overload covers
// both. (HIP's built-in for unsigned long exists but isn't always picked up
// by template ADL when other user-defined atomicCAS overloads sit at file scope.)
...
unique_op<KeyType, CounterType, empty_key, empty_val, hasher>::unique_op(
  ...
  HCTR_LIB_THROW(hipStreamSynchronize(0));
...
void unique_op<KeyType, CounterType, empty_key, empty_val, hasher>::unique(
    ...
    size_t* d_output_counter, hipStream_t stream) {
  // Set the d_output_counter to 0
  HCTR_LIB_THROW(hipMemsetAsync(d_output_counter, 0, sizeof(size_t), stream));
  ...
  hipMemcpyAsync(d_output_counter, counter_.data<CounterType>(), sizeof(CounterType),
