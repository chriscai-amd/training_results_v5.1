    ...
    auto get_bucket_range = [&] {
      DISPATCH_INTEGRAL_FUNCTION_CORE23(
          row_lengths_all_gather_recv_buffer.data_type().type(), offset_t, [&] {
            ...
            size_t temp_bytes = 0;
            hipcub::DeviceScan::InclusiveSum(nullptr, temp_bytes, (offset_t *)nullptr,
    ...
    auto scan_row_lengths = [&] {
      DISPATCH_INTEGRAL_FUNCTION_CORE23(
          row_lengths_all_gather_recv_buffer.data_type().type(), offset_t, [&] {
            ...
            hipcub::DeviceScan::InclusiveSum(temp_scan_storage.data(), temp_bytes,
                                          row_lengths_all_gather_recv_buffer.data<offset_t>(),
...
void filter(std::shared_ptr<CoreResourceManager> core,
  ...
  DISPATCH_INTEGRAL_FUNCTION_CORE23(bucket_range.data_type().type(), offset_t, [&] {
    DISPATCH_INTEGRAL_FUNCTION_CORE23(keys_after_filter.data_type().type(), key_t, [&] {
      ...
      hipcub::DeviceSelect::Flagged(temp_select_storage.data(), temp_select_bytes, keys_ptr,
    ...
    DISPATCH_INTEGRAL_FUNCTION_CORE23(
        embedding_input.bucket_range.data_type().type(), offset_t, [&] {
          DISPATCH_INTEGRAL_FUNCTION_CORE23(embedding_input.keys.data_type().type(), key_t, [&] {
            hipcub::DeviceScan::InclusiveSum(nullptr, temp_scan_bytes, (offset_t *)nullptr,
                                          (offset_t *)nullptr,
            ...
            hipcub::DeviceSelect::Flagged(nullptr, temp_select_bytes, (key_t *)nullptr,
...
void weighted_copy_model_keys_and_offsets(
  ...
  HCTR_LIB_THROW(hipMemcpyAsync(tf_model_key.data(), model_key.data(), tf_model_key.num_bytes(),
                                 hipMemcpyDeviceToDevice, core->get_local_gpu()->get_stream()));
  HCTR_LIB_THROW(hipMemcpyAsync(tf_model_offsets.data(), model_offset.data(),
                                 tf_model_offsets.num_bytes(), hipMemcpyDeviceToDevice,
                                 core->get_local_gpu()->get_stream()));
  HCTR_LIB_THROW(hipMemcpyAsync(tf_sp_weight.data(), model_sp_weight.data(),
                                 tf_sp_weight.num_bytes(), hipMemcpyDeviceToDevice,
                                 core->get_local_gpu()->get_stream()));
}
...
void cal_unique_key_table_range(const std::shared_ptr<CoreResourceManager> &core,
  ...
  const int grid_size =
      core->get_kernel_param().num_sms * core->get_kernel_param().max_thread_per_block / block_size;
  HCTR_LIB_THROW(
      hipMemsetAsync(unique_table_ranges.data(), 0, unique_table_ranges.num_bytes(), stream));
...
void sparse_backward_per_gpu(std::shared_ptr<CoreResourceManager> core,
  ...
  *ret_continous_emb_vec = wgrad.data;
  ...
  HCTR_LIB_THROW(hipStreamSynchronize(core->get_local_gpu()->get_stream()));
...
void copy_backward_key_and_emb_vec(std::shared_ptr<CoreResourceManager> core,
  ...
  for (size_t i = 0; i < unique_key.size(); ++i) {
    ...
    HCTR_LIB_THROW(hipMemcpyAsync(
        ...
        unique_key[i].num_bytes(), hipMemcpyDeviceToDevice, core->get_local_gpu()->get_stream()));
    HCTR_LIB_THROW(hipMemcpyAsync(
        emb_vec[i].data(),
        reinterpret_cast<char *>(continous_emb_vec.data()) + nbytes_emb_vec_offsets,
        emb_vec[i].num_bytes(), hipMemcpyDeviceToDevice, core->get_local_gpu()->get_stream()));
