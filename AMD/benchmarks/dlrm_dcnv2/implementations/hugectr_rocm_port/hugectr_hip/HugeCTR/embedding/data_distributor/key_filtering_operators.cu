...
// HugeCTR ROCm port: ROCm cooperative_groups doesn't expose `cg::reduce`,
// `cg::inclusive_scan`, or `cg::plus`. Provide HIP-native equivalents that
// operate on a `cg::thread_block_tile<32>` using shfl primitives.
//
// On AMD (wave64), tiled_partition<32> still uses 32-thread tiles. Mask is
// 64-bit; HugeCTR's __shfl_*_sync compat shim handles the int variant via the
// header force-included by the top-level CMakeLists.txt.
namespace hctr_rocm_cg {

template <unsigned TileSize, typename T>
__device__ inline T tile_reduce_sum(cg::thread_block_tile<TileSize> tile, T val) {
  // Butterfly reduction across the tile.
  for (unsigned offset = TileSize / 2; offset > 0; offset >>= 1) {
...
template <unsigned TileSize, typename T>
__device__ inline T tile_inclusive_scan(cg::thread_block_tile<TileSize> tile, T val) {
  // Hillis-Steele inclusive scan (T(n log n) work, log(n) depth).
  T x = val;
  for (unsigned offset = 1; offset < TileSize; offset <<= 1) {
    T t = tile.shfl_up(x, offset);
...
__global__ void count_keys_per_gpu(const offset_t* __restrict k_per_b_gpu_major,
  ...
  for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < num_buckets_per_dev_padded;
    ...
    int sum = hctr_rocm_cg::tile_reduce_sum<32>(tile, bucket_num_keys);
...
__global__ void compute_shard_ranges(uint32_t* shard_ranges,
  ...
  for (int i = tid; i < num_shards_padded; i += tile.size()) {
    ...
    uint32_t scan_out = scan_start + hctr_rocm_cg::tile_inclusive_scan<32>(tile, num_keys);
...
void LabelAndCountKeysOperator::operator()(const DataDistributionInput& input,
                                           ...
                                           hipStream_t stream) {
  HCTR_LIB_THROW(hipMemsetAsync(output.keys_per_bucket.data(), 0,
                                 output.keys_per_bucket.num_bytes(), stream));
  HCTR_LIB_THROW(
      hipMemsetAsync(output.keys_per_gpu.data(), 0, output.keys_per_gpu.num_bytes(), stream));
  ...
  HCTR_LIB_THROW(
      hipMemsetAsync(output.local_labels.data(), 0xFF, output.local_labels.num_bytes(), stream));
...
void CountKeysOperator::operator()(core23::Tensor keys_per_bucket_gpu_major,
                                   core23::Tensor result_keys_per_gpu, hipStream_t stream) {
  dim3 block(128);
  ...
  HCTR_LIB_THROW(
      hipMemsetAsync(result_keys_per_gpu.data(), 0, result_keys_per_gpu.num_bytes(), stream));
...
void TransposeBucketsOperator::operator()(core23::Tensor buckets_gpu_major,
                                          core23::Tensor buckets_feat_major, hipStream_t stream) {
  if (num_shards_ == 0) return;
...
void SwizzleKeysOperator::operator()(core23::Tensor src_bucket_range,
                                     ...
                                     core23::Tensor result_keys, hipStream_t stream) {
  if (num_shards_ == 0) return;
...
void ComputeDPBucketRangeOperator::operator()(std::vector<core23::Tensor> dp_bucket_ranges,
                                              ...
                                              int current_batch_size, hipStream_t stream) {
