...
void fill_cpu(Type* data, int64_t num_elements, const Type val, const Device& device,
  ...
  if (stream_or) {
    ...
    HCTR_LIB_THROW(hipLaunchHostFunc(static_cast<hipStream_t>(stream_or.value()),
                                      static_cast<hipHostFn_t>(fill_wrapper<Type>), params));
  } else {
...
void transform_async_common(DstType* dst, const SrcType* src, int64_t num_elements,
  ...
  if (dst_device == src_device) {
    if (src_device.type() == DeviceType::CPU) {
      ...
      HCTR_LIB_THROW(hipLaunchHostFunc(
          stream(), static_cast<hipHostFn_t>(transform_wrapper<DstType, SrcType, Op>), params));
    } else {
      dim3 block(1024);
      dim3 grid((num_elements + block.x - 1) / block.x);
...
void uniform_async(Type* data, int64_t num_elements, const Type a, const Type b,
  ...
  if constexpr (ToScalarType<Type>::value == ScalarType::Float) {
    HCTR_LIB_THROW(hiprandGenerateUniform(generator(), data, num_elements));
  } else {
    static_assert(std::is_same<Type, double>::value);
    HCTR_LIB_THROW(hiprandGenerateUniformDouble(generator(), data, num_elements));
  }
...
void normal_async(Type* data, int64_t num_elements, const Type mean, const Type stddev,
  ...
  if (num_elements & 1) {
    if constexpr (ToScalarType<Type>::value == ScalarType::Float) {
      HCTR_LIB_THROW(hiprandGenerateNormal(generator(), tmp, 2, mean, stddev));
    } else {
      static_assert(std::is_same<Type, double>::value);
      HCTR_LIB_THROW(hiprandGenerateNormalDouble(generator(), tmp, 2, mean, stddev));
    }
  ...
  if constexpr (ToScalarType<Type>::value == ScalarType::Float) {
    HCTR_LIB_THROW(hiprandGenerateNormal(generator(), data, even_length, mean, stddev));
  } else {
    static_assert(std::is_same<Type, double>::value);
    HCTR_LIB_THROW(hiprandGenerateNormalDouble(generator(), data, even_length, mean, stddev));
  }
}
