 ...
 * 3.2.1. Examples:
     HCTR_CUDA_CHECK(BLOCKING, hipLaunchKernel(...));
     HCTR_CUDA_CHECK(BLOCKING, hipMemcpyAsync(...));
     HCTR_CUDA_CHECK(ASYNC, hipGetDevice(...));

 * If you specify its first argument as 'BLOCKING', it will insert a hipDeviceSynchronize() for
 you,
...
#define HCTR_CUDA_CHECK(SYNC_MODE, FUNC)                                       \
  do {                                                                         \
    ...
    if (CHECK_CALL(SYNC_MODE)) {                                               \
      ret_err = hipDeviceSynchronize();                                       \
    }                                                                          \
    if (ret_err != hipSuccess) {                                              \
      HugeCTR::Logger::get().check(ret_err == hipSuccess, CUR_SRC_LOC(EXPR)); \
    }                                                                          \
  } while (0)
