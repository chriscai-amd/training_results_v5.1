...
class LowLevelCUDAAllocator : public Allocator {
 ...
 private:
  ...
  hipDeviceptr_t reserve(hipDeviceptr_t old_ptr, int64_t new_size);
  hipDeviceptr_t allocate_common(hipDeviceptr_t ptr, int64_t new_size);
  void deallocate_common(hipDeviceptr_t ptr);
  ...
  struct AllocInfo {
    size_t reserve_size;
    size_t alloc_size;
    std::vector<hipMemGenericAllocationHandle_t> alloc_handles;
    std::vector<size_t> alloc_handle_sizes;
    std::vector<Range> va_ranges;
  };

  hipMemAllocationProp prop_;
  hipMemAccessDesc access_desc_;
  size_t granularity_;
  std::unordered_map<hipDeviceptr_t, AllocInfo> alloc_info_;
};
