...
void copy_cpu_to_cpu(void* dst_data, const void* src_data, int64_t num_bytes,
  ...
  if (stream_or) {
    ...
    HCTR_LIB_THROW(hipLaunchHostFunc(static_cast<hipStream_t>(stream_or.value()),
                                      static_cast<hipHostFn_t>(copy_wrapper), params));
  } else {
    std::copy(src, src + num_bytes, dst);
  }
}
void copy_gpu_common(void* dst_data, const void* src_data, int64_t num_bytes, hipMemcpyKind kind,
                     std::optional<CUDAStream> stream_or) {
  if (stream_or) {
    HCTR_LIB_THROW(hipMemcpyAsync(dst_data, src_data, num_bytes, kind,
                                   static_cast<hipStream_t>(stream_or.value())));
  } else {
    HCTR_LIB_THROW(hipMemcpy(dst_data, src_data, num_bytes, kind));
  }
}

void copy_gpu_to_gpu(void* dst_data, const void* src_data, int64_t num_bytes,
                     std::optional<CUDAStream> stream_or) {
  copy_gpu_common(dst_data, src_data, num_bytes, hipMemcpyDeviceToDevice, stream_or);
}

void copy_cpu_to_gpu(void* dst_data, const void* src_data, int64_t num_bytes,
                     std::optional<CUDAStream> stream_or) {
  copy_gpu_common(dst_data, src_data, num_bytes, hipMemcpyHostToDevice, stream_or);
}

void copy_gpu_to_cpu(void* dst_data, const void* src_data, int64_t num_bytes,
                     std::optional<CUDAStream> stream_or) {
  copy_gpu_common(dst_data, src_data, num_bytes, hipMemcpyDeviceToHost, stream_or);
}
