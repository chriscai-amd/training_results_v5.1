...
class TrainableLayer : public Layer {
 ...
 public:
  ...
  void init_params(const hiprandGenerator_t& generator) override;
...
void TrainableLayer<DType, use_FP32_weight>::init_params(const hiprandGenerator_t& generator) {
  ...
  for (size_t i = 0; i < weights.size(); ++i) {
    ...
    HCTR_LIB_THROW(hipMemcpyAsync(weights[i].data(), weight_cpu_tensors[i].data(),
                                   weights[i].num_bytes(), hipMemcpyHostToDevice,
                                   get_gpu().get_stream()));
  }
}
