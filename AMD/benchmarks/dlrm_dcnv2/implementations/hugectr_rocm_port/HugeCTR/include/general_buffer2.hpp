...
class CudaHostAllocator {
 public:
  void *allocate(size_t size) const {
    void *ptr;
    HCTR_LIB_THROW(hipHostAlloc(&ptr, size, hipHostMallocDefault));
    return ptr;
  }
  void deallocate(void *ptr) const { HCTR_LIB_THROW(hipHostFree(ptr)); }
};
...
class CudaManagedAllocator {
 public:
  void *allocate(size_t size) const {
    void *ptr;
    HCTR_LIB_THROW(hipMallocManaged(&ptr, size));
    return ptr;
  }
  void deallocate(void *ptr) const { HCTR_LIB_THROW(hipFree(ptr)); }
};
...
class CudaAllocator {
 public:
  void *allocate(size_t size) const {
    void *ptr;
    HCTR_LIB_THROW(hipMalloc(&ptr, size));
    size_t cap{};
    size_t ava{};
    int dev_id{};
    HCTR_LIB_THROW(hipGetDevice(&dev_id));
    HCTR_LIB_THROW(hipMemGetInfo(&ava, &cap));
