...
// do all-to-all
    ...
    if (embedding_data_.get_resource_manager().get_global_gpu_count() > 1) {
      ...
      HCTR_LIB_THROW(hipMemcpyAsync(
          ...
          hipMemcpyDeviceToDevice, embedding_data_.get_local_gpu(0).get_stream()));
    }
#else
    if (embedding_data_.get_resource_manager().get_global_gpu_count() > 1) {
...
#pragma omp parallel for num_threads(num_local_gpu_count)
   ...
   */
  hipError_t update_top_gradients(const bool on_gpu, const void *const top_gradients) override {
    ...
    hipMemcpyKind direction = (on_gpu ? hipMemcpyDeviceToDevice : hipMemcpyHostToDevice);
    ...
    for (size_t dev_id = 0; dev_id < embedding_data_.get_resource_manager().get_local_gpu_count();
      ...
      if (error != hipError_t::hipSuccess) return error;
    }

    for (size_t dev_id = 0; dev_id < embedding_data_.get_resource_manager().get_local_gpu_count();
         ++dev_id) {
      context.set_device(embedding_data_.get_local_gpu(dev_id).get_device_id());
      error = hipStreamSynchronize(embedding_data_.get_local_gpu(dev_id).get_stream());
      if (error != hipError_t::hipSuccess) return error;
    }
