...
class AsyncDataReader : public SchedulableDataReader {
 ...
 public:
  ...
  hipStream_t get_split_3_way_stream(int raw_device_id) const {
    return s3w_streams_.at(raw_device_id);
  }
  hipStream_t get_d2d_stream(int raw_device_id) const { return d2d_streams_.at(raw_device_id); }

  void set_schedule_streams(hipStream_t s3w_stream, hipStream_t d2d_stream,
                            int raw_device_id) override;

  void stream_wait_sparse_tensors(hipStream_t stream, int raw_device_id, bool from_graph) override;
  void stream_wait_dense_tensors(hipStream_t stream, int raw_device_id, bool from_graph) override;
  ...
  void schedule_split_3_way_here(hipStream_t stream, int raw_device_id, bool from_graph) override;
  void schedule_d2d_here(hipStream_t stream, int raw_device_id, bool from_graph) override;
  void schedule_here(hipStream_t stream, int raw_device_id) override;
  void schedule_here_graph(hipStream_t stream, int raw_device_id) override;
 ...
 private:
  ...
  void assign_dense_and_label_tensors(core23::Tensor label_tensor, core23::Tensor dense_tensor,
                                      int raw_device_id, hipStream_t stream);
  ...
  std::vector<hipEvent_t> schedule_events_;
  std::vector<hipEvent_t> split_schedule_events_;
  std::vector<hipEvent_t> d2d_schedule_events_;

  std::vector<hipStream_t> s3w_streams_;  // split_3_way streams
  std::vector<hipStream_t> d2d_streams_;  // d2d copy streams
