...
// HugeCTR ROCm port: Boost.Serialization not required on this build host. The
// only Boost facility used in this header is BOOST_STRONG_TYPEDEF, replicated
// inline below to avoid a libboost-dev dependency.
#ifndef BOOST_STRONG_TYPEDEF
...
class AllReduceInPlaceComm {
 public:
  ...
  virtual void all_reduce(Handle coll, hipStream_t stream, size_t device_id) = 0;
#ifdef ENABLE_MPI
...
class OneshotMultiARInplaceComm : public AllReduceInPlaceComm {
 public:
  ...
  virtual void all_reduce(Handle coll, hipStream_t stream, size_t device_id) final;
