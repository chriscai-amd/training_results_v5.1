...
class GRULayer : public TrainableLayer<T> {
  hipblasGemmAlgo_t falgo_{HIPBLAS_GEMM_DEFAULT};
 ...
 private:
  ...
  hipdnnTensorDescriptor_t hDesc;
  hipdnnDropoutDescriptor_t dropoutDesc;
  hipdnnDataType_t data_type;
#else
  // ROCm port: cuDNN RNN/Dropout types replaced with opaque handles.
  void* cudnnHandle = nullptr;
  void* rnnDesc = nullptr;
  void* in_Desc = nullptr;
  void* out_Desc = nullptr;
  void* cDesc = nullptr;
  void* hDesc = nullptr;
  void* dropoutDesc = nullptr;
  int data_type = 0;
#endif
