...
class HostUniformGenerator {
 public:
  template <typename T>
  static void fill(Tensor2<T>& tensor, T a, T b, const hiprandGenerator_t& gen);
  template <typename T>
  static void fill(core23::Tensor& tensor, T a, T b, const hiprandGenerator_t& gen);
};
...
class HostNormalGenerator {
 public:
  template <typename T>
  static void fill(Tensor2<T>& tensor, float mean, float stddev, const hiprandGenerator_t& gen);
  template <typename T>
  static void fill(core23::Tensor& tensor, float mean, float stddev, const hiprandGenerator_t& gen);
};

class DataSimulator {
 public:
  virtual ~DataSimulator() {}
  virtual void fill(Tensor2<float>& tensor, const hiprandGenerator_t& gen) = 0;
  virtual void fill(core23::Tensor& tensor, const hiprandGenerator_t& gen) = 0;
};
...
class ConstantDataSimulator : public DataSimulator {
 public:
  ...
  void fill(Tensor2<float>& tensor, const hiprandGenerator_t& gen) override;
  void fill(core23::Tensor& tensor, const hiprandGenerator_t& gen) override;
