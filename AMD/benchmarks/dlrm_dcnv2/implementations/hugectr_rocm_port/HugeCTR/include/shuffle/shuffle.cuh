...
template <typename Config, typename CopyDesc>
void __shuffle_dispatch_aligned(CopyDesc copy_desc, size_t expected_elements, hipStream_t stream) {
...
template <typename Config, typename CopyDesc>
void __shuffle_dispatch(CopyDesc copy_desc, size_t expected_elements, hipStream_t stream) {
  dim3 block = Config::block;
  size_t grid = std::max((size_t)1, std::min((size_t)100000, expected_elements / block.y / 2 + 1));
...
void shuffle(CopyDesc copy_desc, hipStream_t stream, size_t expected_elements = 10000) {
