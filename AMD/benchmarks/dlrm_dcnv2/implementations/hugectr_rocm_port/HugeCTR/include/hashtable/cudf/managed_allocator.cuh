...
struct managed_allocator {
  ...
  T* allocate(std::size_t n) const {
    T* ptr = nullptr;
    hipError_t result = hipMallocManaged(&ptr, n * sizeof(T));
    if (hipSuccess != result || nullptr == ptr) {
      HCTR_LOG_S(ERROR, WORLD) << "CUDA Runtime call in line " << __LINE__ << " of file "
                               << __FILE__ << " failed with " << hipGetErrorString(result) << " ("
  ...
  void deallocate(T* p, std::size_t) const { hipFree(p); }
};
