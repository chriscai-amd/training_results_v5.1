  ...
  do {                                                                                           \
    ...
    if (hipSuccess != status) {                                                                 \
      HCTR_LOG(ERROR, WORLD, "CUDA RT call \"%s\" in line %d of file %s failed with %s (%d).\n", \
               #EXPR, __LINE__, __FILE__, hipGetErrorString(status), status);                   \
      exit(1);                                                                                   \
    }                                                                                            \
  } while (0)
#endif
...
// HIP/ROCm: native overloads exist for unsigned long/long long/double, but
// signed `int64_t` and `long long int` overloads are not provided. Keep the
// signed variants outside the HIPCC guard so they're available on AMD.
...
// HIP/ROCm: native atomicCAS(float*) and atomicAdd(unsigned long*/unsigned long long*)
// exist; uint64_t aliases unsigned long on AMD64 so user overload would conflict.
#ifndef __HIPCC__
...
// signed int64_t / long long: HIP doesn't ship a native signed overload.
...
__forceinline__ __device__ pair_type load_pair_vectorized(const pair_type* __restrict__ const ptr) {
  if (sizeof(uint4) == sizeof(pair_type)) {
    ...
    // HIP/ROCm: HIP_vector_type<uint,4> is a class type that doesn't accept
    // brace-enclosed scalar init {0,0,0,0}. Use make_uint4 to default-init
    // the active union member.
    pair_type2vec_type converter{make_uint4(0u, 0u, 0u, 0u)};
    ...
    pair_type2vec_type converter{make_uint2(0u, 0u)};
 ...
 public:
  ...
  explicit concurrent_unordered_map(size_type n, const mapped_type unused_element,
    ...
    constexpr int block_size = 128;
    {
      ...
      if (hipSuccess == status && hashtbl_values_ptr_attributes.type == hipMemoryTypeManaged)
#else
      if (hipSuccess == status && hashtbl_values_ptr_attributes.isManaged)
#endif
      {
        int dev_id = 0;
        CUDA_RT_CALL(hipGetDevice(&dev_id));
        CUDA_RT_CALL(
            hipMemPrefetchAsync(m_hashtbl_values, m_hashtbl_size * sizeof(value_type), dev_id, 0));
      }
    }
...
int*>(tmp_it), unused, value ); if ( old_val == unused ) { it = tmp_it;
  ...
  */
  ...
  int assign_async(const concurrent_unordered_map& other, hipStream_t stream = 0) {
    ...
    CUDA_RT_CALL(hipMemcpyAsync(m_hashtbl_values, other.m_hashtbl_values,
                                 m_hashtbl_size * sizeof(value_type), hipMemcpyDefault, stream));
    return 0;
  }

  void clear_async(hipStream_t stream = 0) {
    constexpr int block_size = 128;
  ...
  */
  ...
  int prefetch(const int dev_id, hipStream_t stream = 0) {
    hipPointerAttribute_t hashtbl_values_ptr_attributes;
...
#endif
    {
      CUDA_RT_CALL(hipMemPrefetchAsync(m_hashtbl_values, m_hashtbl_size * sizeof(value_type),
                                        dev_id, stream));
    }
    CUDA_RT_CALL(hipMemPrefetchAsync(this, sizeof(*this), dev_id, stream));
