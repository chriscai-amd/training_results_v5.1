...
__device__ char* kernel_printf_info(char* dst) {
  // HugeCTR ROCm port: PTX intrinsics (%gridid/%smid/%warpid/%laneid) and
  // host-side hipGetDevice in __device__ context don't work on AMD. Stub these
  // debug fields to 0 -- the kernel still prints block/thread coords correctly.
  unsigned grid_id = 0;
  int device_id = 0;
  unsigned sm_id = 0;
  unsigned warp_id = 0;
