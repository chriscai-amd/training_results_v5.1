...
DataReader<TypeKey>::DataReader(int batchsize, size_t label_dim, int dense_dim,
  ...
  for (size_t local_id = 0; local_id < local_gpu_count; ++local_id) {
    ...
    HCTR_LIB_THROW(hipEventCreateWithFlags(&broadcast_buffer_->finish_broadcast_events[local_id],
...
DataReader<TypeKey>::~DataReader() {
  try {
    ...
    for (size_t i = 0; i < local_gpu_count; ++i) {
      HCTR_LIB_THROW(hipEventDestroy(broadcast_buffer_->finish_broadcast_events[i]));
    }
