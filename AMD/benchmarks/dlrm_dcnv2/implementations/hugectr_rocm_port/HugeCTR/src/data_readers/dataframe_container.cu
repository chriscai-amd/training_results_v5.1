...
static void vector_dec(int32_t* dev_array, size_t len, int32_t decrements, hipStream_t stream) {
...
static void concat_offset(int32_t* lhs_offset, size_t len_l, const int32_t* rhs_offset,
                          size_t len_r, hipStream_t stream) {
  const uint32_t block_size = 256;
  dim3 block(block_size, 1, 1);
...
DFContainer<T>::DFContainer(int dev, size_t max_rows, std::vector<size_t> max_dense_size,
  ...
  allocated_ = true;
  HCTR_LIB_THROW(hipMalloc(&raw_dense_, dense_size_bytes_total));
  HCTR_LIB_THROW(hipMalloc(&raw_sparse_, sparse_size_bytes_total));
  HCTR_LIB_THROW(
      hipHostAlloc(&h_copy_helper_buffer_, num_sparse_ * sizeof(int32_t), hipHostMallocDefault));
  raw_dense_offset_ = nullptr;
  HCTR_LIB_THROW(hipMalloc(&raw_sparse_offset_, sizeof(int32_t) * (max_rows + 1) * num_sparse));
  this->max_rows_ = max_rows;
  HCTR_LIB_THROW(hipStreamCreateWithFlags(&copy_stream_, hipStreamNonBlocking));
};
template <typename T>
DFContainer<T>::~DFContainer() {
  ...
  if (allocated_) {
    CudaDeviceContext context(device_id_);
    HCTR_LIB_CHECK_(hipFree(raw_dense_));
    HCTR_LIB_CHECK_(hipFree(raw_sparse_offset_));
    HCTR_LIB_CHECK_(hipFree(raw_sparse_));
    HCTR_LIB_CHECK_(hipHostFree(h_copy_helper_buffer_));
    HCTR_LIB_CHECK_(hipStreamDestroy(copy_stream_));
  }
}
...
DFContainer<T>& DFContainer<T>::operator=(const DFContainer& rhs) {
  ...
  for (size_t i = 0; i < num_dense_; i++) {
    ...
    HCTR_LIB_THROW(hipMemcpyAsync(reinterpret_cast<void*>(data_to_ptr),
  ...
  this->current_row_ = 0;
  HCTR_LIB_THROW(hipStreamSynchronize(copy_stream_));
  return *this;
}
...
DFContainer<T>& DFContainer<T>::operator+=(const DFContainer& rhs) {
  ...
  for (size_t i = 0; i < num_sparse_; i++) {
    ...
    HCTR_LIB_THROW(hipMemcpyAsync(
        ...
        (len_r) * sizeof(int32_t), hipMemcpyDeviceToDevice, copy_stream_));
...
template <typename T>
hipStream_t& DFContainer<T>::get_copy_stream() {
  return copy_stream_;
}
...
void DFContainer<T>::erase_front(size_t erase_rows) {
  ...
  for (size_t i = 0; i < num_sparse_; i++) {
    ...
    num_variable_sparse++;
    HCTR_LIB_THROW(
        hipMemcpyAsync(reinterpret_cast<void*>(h_copy_helper_buffer_ + i),
                        reinterpret_cast<void*>(this->sparse_offset_ptr_[i] + erase_rows),
                        sizeof(int32_t), hipMemcpyDeviceToHost, copy_stream_));
  }
  // no need to sync if all sparse fixed..
  if (num_variable_sparse > 0) HCTR_LIB_THROW(hipStreamSynchronize(copy_stream_));
...
void dump_table_data_to(cudf::table_view& table_view, std::map<int, int>& dense_idx_to_parquet_col_,
  ...
  HCTR_LIB_THROW(hipStreamSynchronize(df_ptrs_dst->get_copy_stream()));
}
