...
DropoutLayer<T>::DropoutLayer(const core23::Tensor& input_tensor,
  ...
  HCTR_LIB_THROW(hipdnnDropoutGetStatesSize(gpu_resource->get_cudnn_handle(), &size_in_bytes));
  ...
  HCTR_LIB_THROW(hipMalloc(&cudnn_status_, size_in_bytes));
...
void DropoutLayer<T>::fprop(bool is_train) {
  ...
  if (is_train) {
    ...
    HCTR_LIB_THROW(hipMemcpyAsync(output_tensors_[0].data(), input_tensors_[0].data(),
