...
void CublasFusedFCLayerDesc<T>::set_fprop_attr(const T* bias_ptr, Activation_t act, T* mask_out_ptr,
  ...
  fprop_desc.set_fprop_attr({batch_size, bottom_size}, {bottom_size, top_size}, HIPBLAS_OP_N,
                            HIPBLAS_OP_N, HIPBLASLT_ORDER_ROW, enable_tf32_compute, bias_ptr, act,
                            mask_out_ptr);
}
...
void CublasFusedFCLayerDesc<T>::set_bprop_attr(T* dbias_bottom_ptr, T* dbias_top_ptr,
  ...
  bprop_wgrad_desc.set_bprop_attr({batch_size, bottom_size}, {batch_size, top_size}, HIPBLAS_OP_T,
                                  HIPBLAS_OP_N, HIPBLASLT_ORDER_ROW, enable_tf32_compute,
                                  dbias_top_ptr, nullptr);
  bprop_dgrad_desc.set_bprop_attr({batch_size, top_size}, {bottom_size, top_size}, HIPBLAS_OP_N,
                                  HIPBLAS_OP_T, HIPBLASLT_ORDER_ROW, enable_tf32_compute,
...
void FusedFCLayerFunctors<T>::bprop(const T* kernel, const T* bottom, const T* train_top,
                                    ...
                                    const CublasFusedFCLayerAlgo<T>& cublas_layer_algo,
                                    hipblasLtHandle_t cublaslt_handle, hipStream_t stream,
                                    hipStream_t overlap_stream, hipEvent_t& event_overlap,
  ...
  // wait for dact
  if (async_wgrad) {
    HCTR_LIB_THROW(hipEventRecord(event_overlap, stream));
    HCTR_LIB_THROW(hipStreamWaitEvent(overlap_stream, event_overlap));
  }
...
// HugeCTR ROCm port: force explicit instantiation in this TU so the symbols
// land in fused_fc_layer_functors.cu.o regardless of whether other TUs see the
// header's instantiation declarations first.
