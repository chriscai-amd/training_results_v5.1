...
void FullyConnectedLayer<float>::fprop(bool is_train) {
  ...
  HCTR_LIB_THROW(hipblasGemmEx(get_gpu().get_cublas_handle(), HIPBLAS_OP_N, HIPBLAS_OP_N, output_size,
                              in_batch_size, input_size, &alpha, weight, HIP_R_32F, output_size,
                              in, HIP_R_32F, input_size, &beta, out, HIP_R_32F, output_size,
                              compute_type, falgo_));
...
void FullyConnectedLayer<float>::bprop() {
  ...
  HCTR_LIB_THROW(hipblasGemmEx(get_gpu().get_cublas_handle(), HIPBLAS_OP_N, HIPBLAS_OP_T, output_size,
                              input_size, in_batch_size, &alpha, out, HIP_R_32F, output_size, in,
                              HIP_R_32F, input_size, &beta_w, wgrad, HIP_R_32F, output_size,
  ...
  HCTR_LIB_THROW(hipblasGemmEx(get_gpu().get_cublas_handle(), HIPBLAS_OP_T, HIPBLAS_OP_N, input_size,
                              in_batch_size, output_size, &alpha, weight, HIP_R_32F, output_size,
                              out, HIP_R_32F, output_size, &beta_x, in, HIP_R_32F, input_size,
...
void FullyConnectedLayer<float>::search_algorithm() {
  ...
  if (use_mixed_precision_) {
    startAlgo = HIPBLAS_GEMM_DEFAULT;
    endAlgo = HIPBLAS_GEMM_DEFAULT;
  } else {
    startAlgo = HIPBLAS_GEMM_DEFAULT;
    endAlgo = HIPBLAS_GEMM_DEFAULT;
  }
  ...
  for (int testAlgo = startAlgo; testAlgo <= endAlgo; testAlgo++) {
    ...
    for (int i = 0; i < repeat_num; ++i) {
      status = hipblasGemmEx(get_gpu().get_cublas_handle(), HIPBLAS_OP_N, HIPBLAS_OP_N, output_size,
                            in_batch_size, input_size, &alpha, weight, HIP_R_32F, output_size, in,
                            HIP_R_32F, input_size, &beta, out, HIP_R_32F, output_size,
                            compute_type, static_cast<hipblasGemmAlgo_t>(testAlgo));
    }
