...
void SoftmaxLayer<T>::fprop(bool is_train) {
  ...
  MLCommon::LinAlg::reduce(workspace23_.template data<T>(), output_tensor.template data<T>(), hidden_size_, n_rows_,
                           T(0), true, true, get_gpu().get_stream());
  // Softmax exp(x_i) / sum(exp)(x_i)) i=[0, embedding_vector_size-1].
  Softmax_fprop(output_tensor.template data<T>(), workspace23_.template data<T>(), n_rows_, hidden_size_,
                get_gpu().get_stream());
  HCTR_LIB_THROW(hipMemcpyAsync(softmax_out23_.data(), output_tensor.data(),
                                 output_tensor.num_bytes(), hipMemcpyDeviceToDevice,
                                 get_gpu().get_stream()));
}
...
void SoftmaxLayer<__half>::fprop(bool is_train) {
  ...
  HCTR_LIB_THROW(hipblasGemmEx(
      ...
      output_tensor.data(), HIP_R_16F, hidden_size_, identity23_.data(), HIP_R_16F, hidden_size_,
      &beta, workspace23_.data(), HIP_R_16F, n_rows_, HIPBLAS_COMPUTE_16F, HIPBLAS_GEMM_DEFAULT));
  // Softmax exp(x_i) / sum(exp)(x_i)) i=[0, embedding_vector_size-1]
  Softmax_fprop(output_tensor.template data<__half>(), workspace23_.template data<__half>(), n_rows_, hidden_size_,
                get_gpu().get_stream());
  HCTR_LIB_THROW(hipMemcpyAsync(softmax_out23_.data(), output_tensor.data(),
                                 output_tensor.num_bytes(), hipMemcpyDeviceToDevice,
                                 get_gpu().get_stream()));
...
void SoftmaxLayer<T>::bprop() {
  ...
  Softmax_bprop(top_tensor.template data<T>(), bottom_tensor.template data<T>(), softmax_out23_.template data<T>(), n_rows_,
...
void SoftmaxLayer<__half>::bprop() {
  ...
  Softmax_bprop(top_tensor.template data<__half>(), bottom_tensor.template data<__half>(),
                softmax_out23_.template data<__half>(), n_rows_, hidden_size_, get_gpu().get_stream());
}
