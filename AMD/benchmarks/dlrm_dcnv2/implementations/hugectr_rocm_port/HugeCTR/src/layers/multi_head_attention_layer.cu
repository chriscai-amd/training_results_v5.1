...
void MultiHeadAttentionLayer<T>::fprop(bool is_train) {
  ...
  */
  ...
  hipDataType a_type = use_mixed_precision_ ? HIP_R_16F : HIP_R_32F;
  hipDataType b_type = use_mixed_precision_ ? HIP_R_16F : HIP_R_32F;
  hipDataType c_type = use_mixed_precision_ ? HIP_R_16F : HIP_R_32F;

  hipblasComputeType_t compute_type =
      enable_tf32_compute_ ? HIPBLAS_COMPUTE_32F_FAST_TF32 : HIPBLAS_COMPUTE_32F;

  hipblasGemmAlgo_t algo =
      use_mixed_precision_ ? HIPBLAS_GEMM_DEFAULT : HIPBLAS_GEMM_DEFAULT;
  /* 2. bgemm
  ...
  */

  {
    ...
    HCTR_LIB_THROW(hipblasGemmStridedBatchedEx(
        get_gpu().get_cublas_handle(), HIPBLAS_OP_T, HIPBLAS_OP_N, n, m, k, &alpha, key, a_type, k,
  ...
  */
  {
    ...
    HCTR_LIB_THROW(hipblasGemmStridedBatchedEx(
        get_gpu().get_cublas_handle(), HIPBLAS_OP_N, HIPBLAS_OP_N, n, m, k, &alpha, value_4d, b_type,
  ...
  HCTR_LIB_THROW(hipMemcpyAsync(fprop_query_tensor_.data(), (void*)query_buf,
                                 input_tensors_[0].num_bytes(), hipMemcpyDeviceToDevice,
                                 get_gpu().get_stream()));
  HCTR_LIB_THROW(hipMemcpyAsync(fprop_softmax_tensor_.data(), attention_softmax_4d_.data(),
                                 attention_softmax_4d_.num_bytes(), hipMemcpyDeviceToDevice,
                                 get_gpu().get_stream()));
}
  ...
  */
  {
    ...
    HCTR_LIB_THROW(hipblasGemmStridedBatchedEx(
        get_gpu().get_cublas_handle(), HIPBLAS_OP_T, HIPBLAS_OP_N, n, m, k, &alpha, value_4d, b_type,
    ...
    HCTR_LIB_THROW(hipblasGemmStridedBatchedEx(
        get_gpu().get_cublas_handle(), HIPBLAS_OP_N, HIPBLAS_OP_T, k, n, m, &alpha, attention_out_tmp,
  ...
  */
  {
    ...
    HCTR_LIB_THROW(hipblasGemmStridedBatchedEx(
        get_gpu().get_cublas_handle(), HIPBLAS_OP_N, HIPBLAS_OP_N, n, m, k, &alpha, key_buf, a_type,
    ...
    HCTR_LIB_THROW(hipblasGemmStridedBatchedEx(
        get_gpu().get_cublas_handle(), HIPBLAS_OP_N, HIPBLAS_OP_T, n, k, m, &alpha, cur_Q, a_type, n,
