...
#include <algorithm>
// ROCm port: cuml/src_prims/cuda_utils.cuh removed (unused after shim)
#include <functional>
...
void PRelu_Dice_Layer<T>::fprop(bool is_train) {
  ...
  MLCommon::LinAlg::reduce(
      E_x_.template data<T>(), input_tensor.template data<T>(), hiddensize, batchsize, T(0), true, false,
  ...
  // E(x^2);
  MLCommon::LinAlg::reduce(
      E_x2_.template data<T>(), input_tensor.template data<T>(), hiddensize, batchsize, T(0), true, false,
      get_gpu().get_stream(), false, [] __device__(T in, int i) { return pow(in, 2.0); },
      [] __device__(T a, T b) { return a + b; },
      [batchsize] __device__(T out) { return out / batchsize; });
  // E(x)^2;
  MLCommon::LinAlg::unaryOp(
      Var_x_.template data<T>(), E_x_.template data<T>(), hiddensize, [] __device__(T in) { return pow(in, 2.0); },
      get_gpu().get_stream());
  // Var_x = E(x^2) - E(x)^2;
  ...
  Dice_fprop(output_tensor.template data<T>(), input_tensor.template data<T>(), E_x_.template data<T>(), Var_x_.template data<T>(),
...
void PRelu_Dice_Layer<T>::bprop() {
  ...
  Dice_bprop(top_tensor.template data<T>(), bottom_tensor.template data<T>(), E_x_.template data<T>(), Var_x_.template data<T>(), alpha,
