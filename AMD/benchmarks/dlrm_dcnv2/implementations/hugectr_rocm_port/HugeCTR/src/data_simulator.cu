...
void UniformGenerator::fill<float>(float* ptr, size_t num_elements, float a, float b,
                                   size_t sm_count, const hiprandGenerator_t& generator,
                                   const hipStream_t& stream) {
  if (a >= b) {
    HCTR_OWN_THROW(Error_t::WrongInput, "a must be smaller than b");
  }

  HCTR_LIB_THROW(hiprandGenerateUniform(generator, ptr, num_elements));
...
void HostUniformGenerator::fill<float>(Tensor2<float>& tensor, float a, float b,
  ...
  HCTR_LIB_THROW(hiprandGenerateUniform(generator, tensor.get_ptr(),
...
void NormalGenerator::fill<float>(Tensor2<float>& tensor, float mean, float stddev, size_t sm_count,
                                  const hiprandGenerator_t& generator, const hipStream_t& stream) {
  HCTR_LIB_THROW(
      hiprandGenerateNormal(generator, tensor.get_ptr(), tensor.get_num_elements(), mean, stddev));
}
...
void HostNormalGenerator::fill<float>(Tensor2<float>& tensor, float mean, float stddev,
  ...
  HCTR_LIB_THROW(hiprandGenerateNormal(gen, tensor.get_ptr(),
                                      tensor.get_num_elements() % 2 != 0
                                          ? tensor.get_num_elements() + 1
                                          : tensor.get_num_elements(),
                                      mean, stddev));
...
void HostUniformGenerator::fill<float>(core23::Tensor& tensor, float a, float b,
  ...
  if (num_elements & 1) {
    ...
    HCTR_LIB_THROW(hiprandGenerateUniform(generator, tmp, 2));
    tmp[0] = tmp[0] * (b - a) + a;
    *last_element_ptr = tmp[0];
  }
  // even_length=0 is allowed
  HCTR_LIB_THROW(hiprandGenerateUniform(generator, tensor.data<float>(), even_length));
...
void NormalGenerator::fill<float>(core23::Tensor& tensor, float mean, float stddev, size_t sm_count,
                                  const hiprandGenerator_t& generator, const hipStream_t& stream) {
  HCTR_LIB_THROW(
      hiprandGenerateNormal(generator, tensor.data<float>(), tensor.num_elements(), mean, stddev));
}
...
void HostNormalGenerator::fill<float>(core23::Tensor& tensor, float mean, float stddev,
  ...
  if (num_elements & 1) {
    ...
    HCTR_LIB_THROW(hiprandGenerateNormal(gen, tmp, 2, mean, stddev));
    *last_element_ptr = tmp[0];
  }

  HCTR_LIB_THROW(hiprandGenerateNormal(gen, p, even_length, mean, stddev));
}

void ConstantDataSimulator::fill(core23::Tensor& tensor, const hiprandGenerator_t& gen) {
  float* p = tensor.data<float>();
  for (int64_t i = 0; i < tensor.num_elements(); i++) {
    p[i] = value_;
  }
...
void UniformDataSimulator::fill(core23::Tensor& tensor, const hiprandGenerator_t& gen) {
  HostUniformGenerator::fill<float>(tensor, min_, max_, gen);
}

void GaussianDataSimulator::fill(core23::Tensor& tensor, const hiprandGenerator_t& gen) {
