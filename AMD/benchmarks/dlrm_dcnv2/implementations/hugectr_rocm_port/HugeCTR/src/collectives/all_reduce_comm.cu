...
void OneshotSingleARInplaceComm<T>::register_coll_buf(Handle coll) {
  ...
  for (size_t g = 0; g < num_gpus_; g++) {
    HCTR_LIB_THROW(hipSetDevice(gpu_resources_[g]->get_device_id()));
...
template <typename T>
void NCCLARInplaceComm<T>::all_reduce(AllReduceInPlaceComm::Handle coll, hipStream_t stream,
                                      size_t g) {
  auto& ctx = ar_ctx_[coll];
  auto& ctx_g = ctx->ctx_[g];
