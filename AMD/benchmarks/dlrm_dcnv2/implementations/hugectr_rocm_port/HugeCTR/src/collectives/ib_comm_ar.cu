...
void IbComm::register_ar_coll_buf(ARCollHandle coll) {
  ...
  // Allocations
  for (size_t g = 0; g < num_gpus_; g++) {
    HCTR_LIB_THROW(hipSetDevice(device_list_[g]));
    ...
    HCTR_LIB_THROW(hipMemset(gpu_ctx.buf_->get_ptr(), 0, gpu_ctx.buf_->get_size_in_bytes()));
  }
  ...
  for (size_t g = 0; g < num_gpus_; g++) {
    auto& gpu_ctx = *coll_ctx->ctx_[g];
    HCTR_LIB_THROW(hipSetDevice(device_list_[g]));
    HCTR_LIB_THROW(hipMemcpy(gpu_ctx.d_peer_ptrs_.get_ptr(), h_peer_ptrs.data(),
                              num_gpus_ * sizeof(void*), hipMemcpyHostToDevice));
    HCTR_LIB_THROW(hipMemcpy(gpu_ctx.d_flags_ptrs_.get_ptr(), h_peer_flag_ptrs.data(),
                              num_gpus_ * sizeof(size_t*), hipMemcpyHostToDevice));
  }
...
template <int RANKS, typename T>
void IbComm::all_reduce(ARCollHandle coll, hipStream_t stream, size_t device_id) const {
  auto& ctx = ar_coll_ctx_[coll];
...
template <typename T>
void IbComm::all_reduce(ARCollHandle coll, hipStream_t stream, size_t device_id) {
...
#define AR_METHOD(r, data, p)                                                            \
  template void IbComm::all_reduce<p, __half>(ARCollHandle, hipStream_t, size_t) const; \
  template void IbComm::all_reduce<p, float>(ARCollHandle, hipStream_t, size_t) const;  \
  template void IbComm::all_reduce<p, uint32_t>(ARCollHandle, hipStream_t, size_t) const;

BOOST_PP_SEQ_FOR_EACH(AR_METHOD, "", SUPPORTED_AR_RANKS)
#undef AR_METHOD

template void IbComm::all_reduce<__half>(ARCollHandle, hipStream_t, size_t);
template void IbComm::all_reduce<float>(ARCollHandle, hipStream_t, size_t);
template void IbComm::all_reduce<uint32_t>(ARCollHandle, hipStream_t, size_t);
}  // namespace HugeCTR
#endif
