...
void EmbeddingOptimizer<TypeHashKey, TypeEmbeddingComp>::initialize(const GPUResource &local_gpu) {
  switch (param.opt_params.optimizer) {
    case Optimizer_t::Ftrl:
      HCTR_LIB_THROW(hipMemsetAsync(opt_tensors_.opt_n_tensors_.get_ptr(), 0,
                                     opt_tensors_.opt_n_tensors_.get_size_in_bytes(),
                                     local_gpu.get_stream()));
      HCTR_LIB_THROW(hipMemsetAsync(opt_tensors_.opt_z_tensors_.get_ptr(), 0,
                                     opt_tensors_.opt_z_tensors_.get_size_in_bytes(),
                                     local_gpu.get_stream()));
      break;

    case Optimizer_t::Adam:
      HCTR_LIB_THROW(hipMemsetAsync(opt_tensors_.opt_m_tensors_.get_ptr(), 0,
                                     opt_tensors_.opt_m_tensors_.get_size_in_bytes(),
                                     local_gpu.get_stream()));
      HCTR_LIB_THROW(hipMemsetAsync(opt_tensors_.opt_v_tensors_.get_ptr(), 0,
    ...
    case Optimizer_t::AdaGrad:
      HCTR_LIB_THROW(hipMemsetAsync(opt_tensors_.opt_accm_tensors_.get_ptr(),
    ...
    case Optimizer_t::MomentumSGD:
      HCTR_LIB_THROW(hipMemsetAsync(opt_tensors_.opt_momentum_tensors_.get_ptr(), 0,
                                     opt_tensors_.opt_momentum_tensors_.get_size_in_bytes(),
                                     local_gpu.get_stream()));
      break;

    case Optimizer_t::Nesterov:
      HCTR_LIB_THROW(hipMemsetAsync(opt_tensors_.opt_accm_tensors_.get_ptr(), 0,
                                     opt_tensors_.opt_accm_tensors_.get_size_in_bytes(),
                                     local_gpu.get_stream()));
      break;
...
// lib)
//*          step3: sort by value_index (will call hipcub::DeviceRadixSort::SortPairs in cub lib)
...
void EmbeddingOptimizer<TypeHashKey, TypeEmbeddingComp>::update(
  ...
  try {
    ...
    if (opt_params.optimizer == Optimizer_t::SGD &&
      ...
      HCTR_LIB_THROW(hipcub::DeviceRadixSort::SortPairs(
      ...
      HCTR_LIB_THROW(hipcub::DeviceScan::InclusiveSum(
      ...
      // a pinned memory
      HCTR_LIB_THROW(hipMemcpyAsync(&hash_hash_value_index_count_num,
