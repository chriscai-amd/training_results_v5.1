...
void SparseEmbeddingFunctors::get_forward_results(
  ...
  if (total_gpu_count > 1) {
    ...
    HCTR_LIB_THROW(hipMemcpyAsync(embedding_feature.get_ptr(), temp_tensors[0].get_ptr(),
...
void SparseEmbeddingFunctors::get_forward_results(
  ...
  if (total_gpu_count > 1) {
    ...
    CudaDeviceContext context;
    context.set_device(local_gpu->get_device_id());
    HCTR_LIB_THROW(hipMemcpyAsync(embedding_feature, temp_tensors[0].get_ptr(),
                                   total_gpu_count * memcpy_size * sizeof(TypeEmbeddingComp),
    ...
    CudaDeviceContext context;
    context.set_device(local_gpu->get_device_id());
    HCTR_LIB_THROW(hipMemcpyAsync(embedding_feature, embedding_feature_tensors[0].get_ptr(),
