...
#pragma omp parallel for num_threads(local_gpu_count)
  for (size_t id = 0; id < local_gpu_count; id++) {
    ...
    HCTR_LIB_THROW(hipMemcpyAsync(embedding_tensors[id].get_ptr(), h_value_per_gpu[id],
...
void DistributedSlotSparseEmbeddingHash<TypeHashKey, TypeEmbeddingComp>::load_parameters(
  ...
  for (size_t i = 0; i < loop_num; i++) {
    ...
    for (size_t id = 0; id < local_gpu_count; id++) {
      ...
      HCTR_LIB_THROW(hipMemcpyAsync(dst_buf, src_buf, value_chunk_size * sizeof(float),
