...
void DynamicEmbeddingTable::update(const core23::Tensor &unique_keys,
  ...
  HCTR_LIB_THROW(hipMemcpyAsync(&num_unique_keys_cpu, num_unique_keys.data(),
                                 num_unique_keys.num_bytes(), hipMemcpyDeviceToHost, stream));
...
void DynamicEmbeddingTable::dump_by_id(core23::Tensor *h_keys_tensor,
  ...
  DISPATCH_INTEGRAL_FUNCTION_CORE23(key_type.type(), key_t, [&] {
    ...
    HCTR_LIB_THROW(hipMalloc(&d_keys, sizeof(key_t) * key_num));
    HCTR_LIB_THROW(hipMalloc(&d_values, sizeof(float) * values_size));
    ...
    HCTR_LIB_THROW(
        hipMemcpyAsync(h_keys, d_keys, sizeof(key_t) * key_num, hipMemcpyDeviceToHost, stream));

    HCTR_LIB_THROW(hipMemcpyAsync(h_values, d_values, sizeof(float) * values_size,
                                   hipMemcpyDeviceToHost, stream));
    HCTR_LIB_THROW(hipStreamSynchronize(stream));
    HCTR_LIB_THROW(hipFree(d_keys));
    HCTR_LIB_THROW(hipFree(d_values));
  });
}
...
void DynamicEmbeddingTable::load_by_id(core23::Tensor *h_keys_tensor,
  ...
  DISPATCH_INTEGRAL_FUNCTION_CORE23(key_type.type(), key_t, [&] {
    ...
    key_t *h_keys = (key_t *)h_keys_tensor->data();
    HCTR_LIB_THROW(
        hipMemcpyAsync(d_keys, h_keys, sizeof(key_t) * key_num, hipMemcpyHostToDevice, stream));
    ...
    HCTR_LIB_THROW(hipMemcpyAsync(d_values, h_values, sizeof(float) * values_size,
