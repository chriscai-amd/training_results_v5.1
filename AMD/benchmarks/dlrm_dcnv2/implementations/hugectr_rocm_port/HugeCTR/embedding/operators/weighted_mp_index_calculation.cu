...
WeightedModelIndexCalculation::WeightedModelIndexCalculation(
  ...
  {
    size_t temp_bytes = 0;
    hipcub::DeviceScan::InclusiveSum(nullptr, temp_bytes, (uint64_t*)nullptr, (uint64_t*)nullptr,
  ...
  size_t temp_bytes = 0;
  hipcub::DeviceSelect::Flagged(nullptr, temp_bytes, (size_t*)nullptr, (char*)nullptr,
  ...
  if (num_local_embedding_ > 0) {
    DISPATCH_INTEGRAL_FUNCTION_CORE23(key.data_type().type(), key_t, [&] {
      DISPATCH_INTEGRAL_FUNCTION_CORE23(bucket_range.data_type().type(), offset_t, [&] {
        ...
        HCTR_LIB_THROW(hipMemsetAsync(model_key_.data(), 0, model_key_.num_bytes(), stream));
        HCTR_LIB_THROW(
            hipMemsetAsync(model_idx_offsets_.data(), 0, model_idx_offsets_.num_bytes(), stream));
        HCTR_LIB_THROW(hipMemsetAsync(flag_.data(), 0, flag_.num_bytes(), stream));
        ...
        hipcub::DeviceScan::InclusiveSum(d_temp_scan_storage_.data(), d_temp_scan_storage_nbytes,
        ...
        hipcub::DeviceSelect::Flagged(d_temp_select_storage_.data(), d_temp_select_storage_nbytes,
        ...
        DISPATCH_FLOAT_AND_HALF_FUNCTION_CORE23(reorder_sp_weight.data_type().type(), dtype_t, [&] {
          ...
          hipcub::DeviceSelect::Flagged(d_temp_select_weight_storage_.data(),
...
WeightedModelBackwardIndexCalculation::WeightedModelBackwardIndexCalculation(
  ...
  {
    size_t temp_bytes = 0;
    hipcub::DeviceRadixSort::SortPairs(nullptr, temp_bytes, (uint64_t*)nullptr, (uint64_t*)nullptr,
...
void WeightedModelBackwardIndexCalculation::compute(
  ...
  DISPATCH_INTEGRAL_FUNCTION_CORE23(model_key.data_type().type(), key_t, [&] {
    DISPATCH_INTEGRAL_FUNCTION_CORE23(model_offset.data_type().type(), offset_t, [&] {
      ...
      HCTR_LIB_THROW(hipMemsetAsync(bucket_id_list_.data<uint32_t>(), 0,
                                     bucket_id_list_.num_bytes(), stream));
      HCTR_LIB_THROW(hipMemsetAsync(sorted_local_index_.data<uint32_t>(), 0,
                                     sorted_local_index_.num_bytes(), stream));
      HCTR_LIB_THROW(hipMemsetAsync(unique_local_index_.data<uint32_t>(), 0,
                                     unique_local_index_.num_bytes(), stream));
      HCTR_LIB_THROW(
          hipMemsetAsync(unique_key_.data<key_t>(), 0, unique_key_.num_bytes(), stream));
      HCTR_LIB_THROW(hipMemsetAsync(unique_dst_idx_.data<uint32_t>(), 0,
                                     unique_dst_idx_.num_bytes(), stream));
      HCTR_LIB_THROW(hipMemsetAsync(coordinate_wgrad_dst_idx_.data<uint32_t>(), 0,
                                     coordinate_wgrad_dst_idx_.num_bytes(), stream));
      HCTR_LIB_THROW(hipMemsetAsync(sorted_bucket_id_list_.data<uint32_t>(), 0,
                                     sorted_bucket_id_list_.num_bytes(), stream));
      HCTR_LIB_THROW(hipMemsetAsync(sorted_bucket_id_offset_.data<uint32_t>(), 0,
                                     sorted_bucket_id_offset_.num_bytes(), stream));
      // TODO:: need to fix  a flexsible type
      HCTR_LIB_THROW(hipMemsetAsync(sorted_sp_weight_list_.data<float>(), 0,
      ...
      if (num_local_embedding_ > 0 && num_model_key > 0ul) {
        ...
        }
        {
          ...
          HCTR_LIB_THROW(hipStreamSynchronize(stream));  // to sync num_unique_key to host
        }
        ...
        HCTR_LIB_THROW(hipStreamSynchronize(stream));
        {
          ...
          hipcub::DeviceScan::InclusiveSum(d_temp_scan_encode_storage_.data(), nbytes,
                                        unique_id_space_offset_.data<uint32_t>(),
                                        unique_id_space_offset_.data<uint32_t>(),
