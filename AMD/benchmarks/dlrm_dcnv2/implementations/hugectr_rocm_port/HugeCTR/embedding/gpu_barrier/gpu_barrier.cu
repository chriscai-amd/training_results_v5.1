...
GPUBarrier::GPUBarrier(size_t num_gpus, const std::vector<int>& dev_list, bool enforce_order)
  ...
  for (size_t g = 0; g < num_gpus_; g++) {
    HCTR_LIB_THROW(hipSetDevice(dev_list_[g]));
    HCTR_LIB_THROW(hipMalloc(&d_barrier_flags_[g], num_gpus_ * sizeof(size_t)));
    HCTR_LIB_THROW(hipMemset(d_barrier_flags_[g], 0, num_gpus_ * sizeof(size_t)));
  }

  for (size_t g = 0; g < num_gpus_; g++) {
    HCTR_LIB_THROW(hipSetDevice(dev_list_[g]));
    HCTR_LIB_THROW(hipMalloc(&d_rem_barrier_flags_[g], num_gpus_ * sizeof(size_t*)));
    HCTR_LIB_THROW(hipMemcpy(d_rem_barrier_flags_[g], d_barrier_flags_,
                              num_gpus_ * sizeof(size_t*), hipMemcpyHostToDevice));
  }

  for (size_t g = 0; g < num_gpus_; g++) {
    HCTR_LIB_THROW(hipSetDevice(dev_list_[g]));
    HCTR_LIB_THROW(hipMalloc(&d_global_barrier_store_[g], sizeof(float)));
  }
}

void GPUBarrier::sync_all_gpus(const hipStream_t* streams) {
...
void GPUBarrier::sync_all_gpus(const hipStream_t stream, size_t device_id) {
  constexpr size_t MAX_TPB = 256;
  size_t n_blocks = ceildiv<size_t>(num_gpus_, MAX_TPB);
  HCTR_LIB_THROW(hipSetDevice(dev_list_[device_id]));
...
void GPUBarrier::sync_all_gpus_report_host(size_t* d_report_count, size_t* h_report_ptr,
                                           const hipStream_t stream, size_t device_id) {
  HCTR_LIB_THROW(hipSetDevice(dev_list_[device_id]));
...
GPUBarrier::~GPUBarrier() {
  for (size_t g = 0; g < num_gpus_; g++) {
    HCTR_LIB_CHECK_(hipSetDevice(dev_list_[g]));
    HCTR_LIB_CHECK_(hipFree(d_rem_barrier_flags_[g]));
    HCTR_LIB_CHECK_(hipFree(d_barrier_flags_[g]));
    HCTR_LIB_CHECK_(hipFree(d_global_barrier_store_[g]));
  }
