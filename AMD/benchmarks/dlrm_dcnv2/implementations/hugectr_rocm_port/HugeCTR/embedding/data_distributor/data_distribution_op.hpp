...
class IDataDistributionOp {
 public:
  ...
  virtual void distribute(const DataDistributionInput &input, embedding::EmbeddingInput &output,
                          int batch_size, hipStream_t stream) = 0;
};
...
class DenseMPDataDistributionOp final : public IDataDistributionOp {
 public:
  ...
  void distribute(const DataDistributionInput &input, embedding::EmbeddingInput &output,
                  int batch_size, hipStream_t stream) override;
  ...
  void all2all_keys_per_bucket(embedding::EmbeddingInput &output, hipStream_t stream);

  void all2all_keys(embedding::EmbeddingInput &output, hipStream_t stream);

  void filter_after_all2all(embedding::EmbeddingInput &output, hipStream_t stream);
