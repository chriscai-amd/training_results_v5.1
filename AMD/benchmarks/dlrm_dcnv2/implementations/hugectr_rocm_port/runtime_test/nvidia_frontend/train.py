...
# HugeCTR ROCm port: vendor the NVIDIA train.py and shim out missing modules.
...
try:
    ...
    from mlperf_common_stub import HCTRCommunicationHandler, MLLoggerWrapper
try:
    from mpi4py import MPI
except ImportError:
    from mpi4py_stub import MPI

import hugectr
import mlperf_logger
import sharding

# HugeCTR ROCm port: scaled-down values for our subsampled Criteo day_0 + the
# fact that our preprocessing produced single-hot per slot (not the multi-hot
# shape the MLPerf reference dataset has). We override via env vars so the
# original published MLPerf numbers are still visible in source.
def _i(env, default):
    return int(os.environ.get(env, default))

TRAIN_NUM_SAMPLES = _i("HCTR_TRAIN_NUM_SAMPLES", 4195197692)
EVAL_NUM_SAMPLES  = _i("HCTR_EVAL_NUM_SAMPLES",  89137319)
_DEFAULT_TABLE_SIZE_ARRAY = [
...
if os.environ.get("HCTR_USE_SUBSAMPLED_CRITEO", "0") == "1":
    ...
    TABLE_SIZE_ARRAY = _DEFAULT_TABLE_SIZE_ARRAY
    MULTI_HOT_SIZES  = _DEFAULT_MULTI_HOT_SIZES
...
parser.add_argument(
    ...
    type=float,
    # Our subsampled Criteo can't reach the full 0.80275 AUC; relax for smoke.
    default=float(os.environ.get("HCTR_AUC_THRESHOLD", "0.80275")),
)
parser.add_argument(
    "--sharding_plan",
...
reader = hugectr.DataReaderParams(
    ...
    async_param=hugectr.AsyncParam(
        num_threads=int(os.environ.get("HCTR_READER_THREADS", "4")),
        num_batches_per_thread=int(os.environ.get("HCTR_READER_BATCHES", "16")),
        shuffle=False,
...
def _add_mlp_stack(top, bot, hidden_dims, last_act=True):
    ...
    for i, h in enumerate(hidden_dims):
        ...
        model.add(hugectr.DenseLayer(
            ...
            bottom_names=[cur], top_names=[fc],
            num_output=h,
        ))
        if last_act or i < len(hidden_dims) - 1:
            relu = f"{top}_relu{i+1}"
            model.add(hugectr.DenseLayer(
...
if os.environ.get("HCTR_USE_INNERPRODUCT_INSTEAD", "0") == "1":
    ...
    model.add(hugectr.DenseLayer(
        layer_type=hugectr.Layer_t.InnerProduct,
        bottom_names=["concat1"], top_names=["interaction_fc1"],
        num_output=_concat_dim,
    ))
    model.add(hugectr.DenseLayer(
        layer_type=hugectr.Layer_t.ReLU,
        bottom_names=["interaction_fc1"], top_names=["interaction_relu"],
    ))
    model.add(hugectr.DenseLayer(
        layer_type=hugectr.Layer_t.InnerProduct,
        bottom_names=["interaction_relu"], top_names=["interaction1"],
        num_output=_concat_dim,
    ))
else:
    model.add(hugectr.DenseLayer(
