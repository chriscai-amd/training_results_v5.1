...
NATIVE_DIR=${1:?"usage: $0 <native_dir> <cold_prefix> [cold_iter] [n_gpus]"}
COLD_PREFIX=${2:?"cold-pass prefix required"}
COLD_ITER=${3:-5}
NGPU=${4:-8}
...
python3 "$JOINER" \
    ...
    --cold-prefix "$COLD_PREFIX" \
    --gpu 0 \
    --cold-iter "$COLD_ITER" \
    --out "$NATIVE_DIR/overlay_rank0.json" \
    --dump-map "$MAP_JSON"

# Apply to remaining ranks. Reuse the cached mapping by passing the same
# cold prefix; the joiner regenerates it cheaply (it's a few-thousand-row
# CSV scan) and per-rank application is the slow part.
...
echo
echo "Drag any overlay_rank*.json into ui.perfetto.dev to view."
