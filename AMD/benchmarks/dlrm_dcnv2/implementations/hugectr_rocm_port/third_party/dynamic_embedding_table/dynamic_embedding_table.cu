...
__global__ void setup_kernel(Seed seed, hiprandState *state) {
  auto grid = cooperative_groups::this_grid();
  hiprand_init(seed, grid.thread_rank(), 0, &state[grid.thread_rank()]);
}
}  // namespace
...
template <typename KeyType, typename ValueType>
void DynamicEmbeddingTable<KeyType, ValueType>::initialize(hipStream_t stream) {
  std::random_device rd;
  auto seed = rd();

  int device;
  CUCO_CUDA_TRY(hipGetDevice(&device));
  hipDeviceProp_t deviceProp;
  CUCO_CUDA_TRY(hipGetDeviceProperties(&deviceProp, device));
  CUCO_CUDA_TRY(hipMallocAsync(
      &curand_states_, sizeof(hiprandState) * deviceProp.multiProcessorCount * 2048, stream));
  ...
  for (size_t i = 0; i < num_classes_; i++) {
    hipEvent_t event;
    CUCO_CUDA_TRY(hipEventCreate(&event));
    event_per_class_.push_back(event);
  }

  CUCO_CUDA_TRY(hipEventCreate(&primary_event_));
  ...
  CUCO_CUDA_TRY(hipGetLastError());
}

template <typename KeyType, typename ElementType>
void DynamicEmbeddingTable<KeyType, ElementType>::uninitialize(hipStream_t stream) {
  for (size_t i = 0; i < num_classes_; i++) {
    maps_[i]->uninitialize(stream);
  }

  CUCO_CUDA_TRY(hipEventDestroy(primary_event_));

  for (size_t i = 1; i < num_classes_; i++) {
    CUCO_CUDA_TRY(hipEventDestroy(event_per_class_[i]));
  }

  for (size_t i = 1; i < num_classes_; i++) {
    CUCO_CUDA_TRY(hipStreamDestroy(stream_per_class_[i]));
  }

  CUCO_CUDA_TRY(hipFreeAsync(curand_states_, stream));
  CUCO_CUDA_TRY(hipGetLastError());
}
...
void DynamicEmbeddingTable<KeyType, ElementType>::lookup(
    ...
    size_t const *id_space_offsets, size_t num_id_spaces, hipStream_t stream) {
  CUCO_CUDA_TRY(hipEventRecord(primary_event_, stream));
  ...
  for (size_t i = 0; i < num_id_spaces; ++i) {
    ...
    CUCO_CUDA_TRY(hipStreamWaitEvent(stream_per_class_[id_space], primary_event_));
    ...
    CUCO_CUDA_TRY(hipEventRecord(event_per_class_[id_space], stream_per_class_[id_space]));
  }

  for (size_t i = 0; i < num_classes_; ++i) {
    CUCO_CUDA_TRY(hipStreamWaitEvent(stream, event_per_class_[i]));
  }
...
template <typename KeyType, typename ElementType>
void DynamicEmbeddingTable<KeyType, ElementType>::clear(hipStream_t stream) {
  for (auto &map : maps_) {
    map->clear(stream);
    CUCO_CUDA_TRY(hipGetLastError());
  }
