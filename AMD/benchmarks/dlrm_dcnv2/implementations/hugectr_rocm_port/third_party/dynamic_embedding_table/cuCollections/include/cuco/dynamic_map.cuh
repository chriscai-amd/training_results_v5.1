...
class dynamic_map {
 ...
 public:
  ...
  void initialize(hipStream_t stream = 0);
  void uninitialize(hipStream_t stream = 0);
   ...
   */
  void reserve(size_t n, hipStream_t stream = 0);

  template <typename Hash = cuco::detail::MurmurHash3_32<key_type>>
  void lookup(key_type const *keys, element_type *values, size_t num_keys, hipStream_t stream = 0,
              Hash hash = Hash{});

  template <typename Hash = cuco::detail::MurmurHash3_32<key_type>>
  void lookup_unsafe(key_type const *keys, element_type **values, size_t num_keys,
                     hipStream_t stream = 0, Hash hash = Hash{});
  ...
  template <typename Hash = cuco::detail::MurmurHash3_32<key_type>>
  void remove(key_type const *keys, size_t num_keys, hipStream_t stream = 0, Hash hash = Hash{});

  void eXport(key_type *keys, element_type *values, size_t num_keys, hipStream_t stream = 0);
