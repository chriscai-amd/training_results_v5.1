...
template <typename Key, typename Element, typename Initializer>
void static_map<Key, Element, Initializer>::initialize(hipStream_t stream) {
  CUCO_CUDA_TRY(hipMallocAsync(&slots_, sizeof(pair_atomic_type) * capacity_, stream));
  CUCO_CUDA_TRY(hipMallocAsync(&elements_, sizeof(element_type) * dimension_ * capacity_, stream));
...
template <typename Key, typename Element, typename Initializer>
void static_map<Key, Element, Initializer>::uninitialize(hipStream_t stream) {
  hipFreeAsync(slots_, stream);
  hipFreeAsync(elements_, stream);
}
