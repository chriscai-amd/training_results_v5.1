...
template <typename Key, typename Element, typename Initializer>
void dynamic_map<Key, Element, Initializer>::initialize(hipStream_t stream) {
  void *ptr;
  CUCO_CUDA_TRY(hipMalloc(&ptr, sizeof(atomic_ctr_type) * max_num_submaps * 3));
  ...
  void *h_ptr;
  CUCO_CUDA_TRY(hipHostMalloc(&h_ptr, sizeof(atomic_ctr_type) * max_num_submaps * 3));
  ...
  CUCO_CUDA_TRY(hipMemcpyAsync(ptr, h_ptr, sizeof(atomic_ctr_type) * 2 * max_num_submaps,
                                hipMemcpyHostToDevice, stream));
}

template <typename Key, typename Element, typename Initializer>
void dynamic_map<Key, Element, Initializer>::uninitialize(hipStream_t stream) {
  for (auto &submap : submaps_) {
    submap->uninitialize(stream);
  }
  CUCO_ASSERT_CUDA_SUCCESS(hipFree(reinterpret_cast<void *>(occupied_size_per_submap_)));
  CUCO_ASSERT_CUDA_SUCCESS(hipHostFree(reinterpret_cast<void *>(h_occupied_size_per_submap_)));
}

template <typename Key, typename Element, typename Initializer>
void dynamic_map<Key, Element, Initializer>::reserve(size_t n, hipStream_t stream) {
...
void dynamic_map<Key, Element, Initializer>::lookup(key_type const *keys, element_type *values,
  ...
  CUCO_CUDA_TRY(hipMemcpyAsync(h_occupied_size_per_submap_, occupied_size_per_submap_,
...
void dynamic_map<Key, Element, Initializer>::remove(key_type const *keys, size_t num_keys,
  ...
  CUCO_CUDA_TRY(hipMemcpyAsync(h_reclaimed_size_per_submap_, reclaimed_size_per_submap_,
                                sizeof(atomic_ctr_type) * max_num_submaps, hipMemcpyDeviceToHost,
                                stream));
...
void dynamic_map<Key, Element, Initializer>::eXport(key_type *keys, element_type *values,
  ...
  CUCO_ASSERT_CUDA_SUCCESS(hipMemcpyAsync(
      num_successes_, h_num_successes_, sizeof(atomic_ctr_type), hipMemcpyHostToDevice, stream));
  ...
  CUCO_CUDA_TRY(hipMemcpyAsync(h_num_successes_, num_successes_,
                                sizeof(atomic_ctr_type) * max_num_submaps, hipMemcpyDeviceToHost,
                                stream));
...
template <typename Key, typename Element, typename Initializer>
void dynamic_map<Key, Element, Initializer>::clear(hipStream_t stream) {
