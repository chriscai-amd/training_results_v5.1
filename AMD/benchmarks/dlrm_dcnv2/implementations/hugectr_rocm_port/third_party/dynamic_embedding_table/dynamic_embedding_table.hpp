...
class DynamicEmbeddingTable {
  ...
  std::vector<hipStream_t> stream_per_class_;
  std::vector<hipEvent_t> event_per_class_;
  hipEvent_t primary_event_;
 ...
 public:
  ...
  void lookup(KeyType const *keys, ElementType *elements, size_t num_keys, size_t const *id_spaces,
              size_t const *id_space_offsets, size_t num_id_spaces, hipStream_t stream = 0);
  ...
  void lookup_by_index(size_t class_index, KeyType const *d_keys, ElementType *d_values,
                       size_t num_keys, hipStream_t stream = 0);
