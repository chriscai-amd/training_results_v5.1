...
void test_basic(size_t max_hbm_for_vectors) {
  ...
  CUDA_CHECK(hipMalloc(&d_new_vectors, KEY_NUM * sizeof(V) * options.dim));
  ...
  for (int i = 0; i < TEST_TIMES; i++) {
    ...
    CUDA_CHECK(hipMemset(d_new_vectors, 2, KEY_NUM * sizeof(V) * options.dim));
    ...
    CUDA_CHECK(hipMemset(d_found, 0, KEY_NUM * sizeof(bool)));
    CUDA_CHECK(hipMemset(d_new_vectors, 0, KEY_NUM * sizeof(V) * options.dim));
    {
      V** d_vectors_ptr = nullptr;
      CUDA_CHECK(hipMalloc(&d_vectors_ptr, KEY_NUM * sizeof(V*)));
    ...
    CUDA_CHECK(hipMemcpy(h_found, d_found, KEY_NUM * sizeof(bool),
                          hipMemcpyDeviceToHost));
    CUDA_CHECK(hipMemcpy(h_vectors, d_new_vectors,
                          KEY_NUM * sizeof(V) * options.dim,
                          hipMemcpyDeviceToHost));
    found_num = 0;
    ...
    CUDA_CHECK(hipMemset(d_keys, 0, KEY_NUM * sizeof(K)));
    CUDA_CHECK(hipMemset(d_scores, 0, KEY_NUM * sizeof(S)));
    CUDA_CHECK(hipMemset(d_vectors, 0, KEY_NUM * sizeof(V) * options.dim));
  ...
  CUDA_CHECK(hipFree(d_new_vectors));
  CUDA_CHECK(hipFree(d_found));
...
void test_rehash(size_t max_hbm_for_vectors) {
  ...
  for (int i = 0; i < TEST_TIMES; i++) {
    ...
    {
      V** d_vectors_ptr = nullptr;
      CUDA_CHECK(hipMalloc(&d_vectors_ptr, BUCKET_MAX_SIZE * sizeof(V*)));
...
void test_rehash_on_big_batch(size_t max_hbm_for_vectors) {
...

  {
    V** d_vectors_ptr = nullptr;
    bool* d_found;
    CUDA_CHECK(hipMalloc(&d_found, INIT_KEY_NUM * sizeof(bool)));
    CUDA_CHECK(hipMalloc(&d_vectors_ptr, INIT_KEY_NUM * sizeof(V*)));
...
void test_export_batch_if(size_t max_hbm_for_vectors) {
  ...
  for (int i = 0; i < TEST_TIMES; i++) {
    ...
    CUDA_CHECK(hipMemset(h_keys, 0, KEY_NUM * sizeof(K)));
    CUDA_CHECK(hipMemset(h_scores, 0, KEY_NUM * sizeof(S)));
    CUDA_CHECK(hipMemset(h_vectors, 0, KEY_NUM * sizeof(V) * options.dim));
...
void test_basic_for_cpu_io() {
  ...
  for (int i = 0; i < TEST_TIMES; i++) {
    ...
    CUDA_CHECK(hipMemset(d_vectors, 2, KEY_NUM * sizeof(V) * options.dim));
  ...
  for (int i = 0; i < TEST_TIMES; i++) {
...

    {
...

      {
        V** d_vectors_ptr = nullptr;
        bool* d_found;
        CUDA_CHECK(hipMalloc(&d_found, BASE_KEY_NUM * sizeof(bool)));
        CUDA_CHECK(hipMalloc(&d_vectors_ptr, BASE_KEY_NUM * sizeof(V*)));
...

    {
...

      {
        V** d_vectors_ptr = nullptr;
        bool* d_found;
        CUDA_CHECK(hipMalloc(&d_found, TEST_KEY_NUM * sizeof(bool)));
        CUDA_CHECK(hipMalloc(&d_vectors_ptr, TEST_KEY_NUM * sizeof(V*)));
        test_util::array2ptr(d_vectors_ptr, d_vectors_temp, options.dim,
                             TEST_KEY_NUM, stream);
    ...
    for (int r = 0; r < rounds; r++) {
      ...
      for (int s = 0; s < STEPS; s++) {
                      ...
                      stream);
        {
          ...
          CUDA_CHECK(hipMalloc(&d_vectors_ptr, BATCH_SIZE * sizeof(V*)));
          test_util::array2ptr(d_vectors_ptr, d_vectors_temp, options.dim,
                               BATCH_SIZE, stream);
...
void CheckFindOrInsertValues(Table* table, K* keys, V* values, S* scores,
...

  {
    V** d_vectors_ptr = nullptr;
    bool* d_found;
    CUDA_CHECK(hipMalloc(&d_found, len * sizeof(bool)));
    CUDA_CHECK(hipMalloc(&d_vectors_ptr, len * sizeof(V*)));
...
void test_duplicated_keys(size_t max_hbm_for_vectors) {
  ...
  CUDA_CHECK(hipMemset(h_keys, 1, KEY_NUM * sizeof(K)));
  CUDA_CHECK(hipMemset(h_vectors, 0, KEY_NUM * sizeof(V) * options.dim));
