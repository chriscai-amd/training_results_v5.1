...
void CheckInsertAndEvict(Table* table, K* keys, V* values, S* scores,
...

  {
    ...
    CUDA_CHECK(hipMemcpyAsync(h_tmp_founds, d_tmp_founds, len * sizeof(bool),
                               hipMemcpyDeviceToHost, stream));
    CUDA_CHECK(hipStreamSynchronize(stream));
...
void BatchCheckInsertAndEvict(Table* table, K* keys, V* values, S* scores,
  ...
  while (step->load() < total_step) {
    ...
    if (if_check) {
      ...
      CUDA_CHECK(hipMemcpyAsync(h_tmp_keys + table_size_before, keys + len * s,
                                 len * sizeof(K), hipMemcpyDeviceToHost,
                                 stream));
      CUDA_CHECK(hipMemcpyAsync(h_tmp_values + table_size_before * dim,
                                 values + len * s * dim, len * dim * sizeof(V),
                                 hipMemcpyDeviceToHost, stream));
      CUDA_CHECK(hipMemcpyAsync(h_tmp_scores + table_size_before,
    ...
    if (if_check) {
      ...
      CUDA_CHECK(hipMemcpyAsync(h_tmp_values + table_size_after * dim,
                                 evicted_values, filtered_len * dim * sizeof(V),
                                 hipMemcpyDeviceToHost, stream));
      CUDA_CHECK(hipMemcpyAsync(h_tmp_scores + table_size_after,
...
void BatchCheckFind(Table* table, K* keys, V* values, S* scores, size_t len,
  ...
  while (step->load() < total_step) {
    ...
    CUDA_CHECK(hipMemcpyAsync(d_tmp_keys, keys + cap * find_step,
    ...
    if (if_check) {
      CUDA_CHECK(hipMemcpyAsync(h_tmp_keys, d_tmp_keys, cap * sizeof(K),
                                 hipMemcpyDeviceToHost, stream));
      CUDA_CHECK(hipMemcpyAsync(h_tmp_values, d_tmp_values,
                                 cap * dim * sizeof(V), hipMemcpyDeviceToHost,
                                 stream));
      CUDA_CHECK(hipMemcpyAsync(h_tmp_scores, d_tmp_scores, cap * sizeof(S),
                                 hipMemcpyDeviceToHost, stream));
      CUDA_CHECK(hipMemcpyAsync(h_tmp_founds, d_tmp_founds, cap * sizeof(bool),
                                 hipMemcpyDeviceToHost, stream));
...
void test_insert_and_evict_run_with_batch_find() {
  ...
  CUDA_CHECK(hipStreamCreate(&insert_stream));
  CUDA_CHECK(hipStreamCreate(&find_stream));
  ...
  CUDA_CHECK(hipStreamSynchronize(insert_stream));
  ...
  CUDA_CHECK(hipStreamDestroy(insert_stream));
  CUDA_CHECK(hipStreamDestroy(find_stream));
}
