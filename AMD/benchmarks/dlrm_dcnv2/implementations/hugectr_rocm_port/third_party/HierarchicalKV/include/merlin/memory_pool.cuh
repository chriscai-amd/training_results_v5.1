...
class MemoryPool final {
 public:
  ...
  MemoryPool(const MemoryPoolOptions& options, BaseAllocator* allocator)
    ...
    for (auto& ready_event : ready_events_) {
      CUDA_CHECK(hipEventCreate(&ready_event));
    }
  ...
  ~MemoryPool() {
    ...
    for (auto& ready_event : ready_events_) {
      CUDA_CHECK(hipEventDestroy(ready_event));
    }
 ...
 private:
  ...
  inline void put_raw(Iterator first, Iterator const last,
    ...
    // we return the buffer ownershup. This `hipStreamQuery` will prevent that.
    if (stream && hipStreamQuery(stream) != hipErrorInvalidHandle) {
      for (; first != last; ++first) {
        ...
        CUDA_CHECK(hipEventRecord(ready_event, stream));
