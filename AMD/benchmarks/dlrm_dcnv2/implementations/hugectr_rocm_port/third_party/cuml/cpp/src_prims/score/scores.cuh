...
double trustworthiness_score(math_t *X, math_t *X_embedded, int n, int m, int d,
                             int n_neighbors,
                             std::shared_ptr<deviceAllocator> d_alloc,
                             hipStream_t stream, int batchSize = 512) {
...
math_t r2_score(math_t *y, math_t *y_hat, int n, hipStream_t stream) {
  ...
  CUDA_CHECK(hipFree(y_bar));
  CUDA_CHECK(hipFree(sse_arr));
  CUDA_CHECK(hipFree(ssto_arr));
...
void regression_metrics(const T *predictions, const T *ref_predictions, int n,
                        std::shared_ptr<deviceAllocator> d_alloc,
                        hipStream_t stream, double &mean_abs_error,
  ...
  CUDA_CHECK(hipMemsetAsync(tmp_sums, 0, 2 * sizeof(double), stream));
