...
// TODO: couldn't template this function due to hipsolverDnSgesvd and
...
void svdQR(T *in, int n_rows, int n_cols, T *sing_vals, T *left_sing_vecs,
           ...
           bool gen_right_vec, hipsolverHandle_t cusolverH,
           hipblasHandle_t cublasH, std::shared_ptr<deviceAllocator> allocator,
           hipStream_t stream) {
...
void svdEig(T *in, int n_rows, int n_cols, T *S, T *U, T *V, bool gen_left_vec,
            hipblasHandle_t cublasH, hipsolverHandle_t cusolverH,
            hipStream_t stream, std::shared_ptr<deviceAllocator> allocator) {
  ...
  gemm(in, n_rows, n_cols, in, in_cross_mult.data(), n_cols, n_cols,
       HIPBLAS_OP_T, HIPBLAS_OP_N, alpha, beta, cublasH, stream);
  ...
  if (gen_left_vec) {
    gemm(in, n_rows, n_cols, V, U, n_rows, n_cols, HIPBLAS_OP_N, HIPBLAS_OP_N,
...
void svdJacobi(math_t *in, int n_rows, int n_cols, math_t *sing_vals,
               ...
               int max_sweeps, hipsolverHandle_t cusolverH,
               hipStream_t stream,
  ...
  CUSOLVER_CHECK(cusolverDngesvdj_bufferSize(
    cusolverH, HIPSOLVER_EIG_MODE_VECTOR, econ, m, n, in, m, sing_vals,
    left_sing_vecs, m, right_sing_vecs, n, &lwork, gesvdj_params));

  device_buffer<math_t> d_work(allocator, stream, lwork);

  CUSOLVER_CHECK(cusolverDngesvdj(cusolverH, HIPSOLVER_EIG_MODE_VECTOR, econ, m,
...
void svdReconstruction(math_t *U, math_t *S, math_t *V, math_t *out, int n_rows,
                       int n_cols, int k, hipblasHandle_t cublasH,
                       hipStream_t stream,
                       std::shared_ptr<deviceAllocator> allocator) {
  const math_t alpha = 1.0, beta = 0.0;
  device_buffer<math_t> SVT(allocator, stream, k * n_cols);

  gemm(S, k, k, V, SVT.data(), k, n_cols, HIPBLAS_OP_N, HIPBLAS_OP_T, alpha, beta,
       cublasH, stream);
  gemm(U, n_rows, k, SVT.data(), out, n_rows, n_cols, HIPBLAS_OP_N, HIPBLAS_OP_N,
       alpha, beta, cublasH, stream);
}
...
bool evaluateSVDByL2Norm(math_t *A_d, math_t *U, math_t *S_vec, math_t *V,
  ...
  CUDA_CHECK(hipMemsetAsync(P_d.data(), 0, sizeof(math_t) * m * n, stream));
  CUDA_CHECK(hipMemsetAsync(S_mat.data(), 0, sizeof(math_t) * k * k, stream));
  ...
  CUDA_CHECK(
    hipMemsetAsync(A_minus_P.data(), 0, sizeof(math_t) * m * n, stream));

  CUBLAS_CHECK(cublasgeam(cublasH, HIPBLAS_OP_N, HIPBLAS_OP_N, m, n, &alpha, A_d,
