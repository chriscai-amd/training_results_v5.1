...
struct PhiloxGenerator {
   ...
   * @{
   */
  DI void next(float& ret) { ret = hiprand_uniform(&(this->state)); }
  DI void next(double& ret) { ret = hiprand_uniform_double(&(this->state)); }
  DI void next(uint32_t& ret) { ret = hiprand(&(this->state)); }
