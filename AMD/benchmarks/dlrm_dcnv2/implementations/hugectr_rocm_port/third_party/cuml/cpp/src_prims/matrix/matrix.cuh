...
void copyRows(const m_t *in, int n_rows, int n_cols, m_t *out,
              const int *indices, int n_rows_indices, hipStream_t stream,
              bool rowMajor = false) {
  if (rowMajor) {
...
void truncZeroOrigin(m_t *in, int in_n_rows, m_t *out, int out_n_rows,
                     int out_n_cols, hipStream_t stream) {
  auto m = out_n_rows;
  auto k = in_n_rows;
 ...
 */
template <typename m_t>
void colReverse(m_t *inout, int n_rows, int n_cols, hipStream_t stream) {
  auto n = n_cols;
  auto m = n_rows;
 ...
 */
template <typename m_t>
void rowReverse(m_t *inout, int n_rows, int n_cols, hipStream_t stream) {
  auto m = n_rows;
  auto size = n_rows * n_cols;
  auto d_q = inout;
  auto d_q_reversed = inout;
...
void print(const m_t *in, int n_rows, int n_cols, char h_separator = ' ',
  ...
  CUDA_CHECK(hipMemcpy(h_matrix.data(), in, n_cols * n_rows * sizeof(m_t),
...
void sliceMatrix(m_t *in, int n_rows, int n_cols, m_t *out, int x1, int y1,
                 int x2, int y2, hipStream_t stream) {
  // Slicing
  dim3 block(64);
 ...
 */
template <typename m_t>
void getDiagonalInverseMatrix(m_t *in, int len, hipStream_t stream) {
  dim3 block(64);
 ...
 */
template <typename m_t>
m_t getL2Norm(m_t *in, int size, hipblasHandle_t cublasH, hipStream_t stream) {
  m_t normval = 0;
