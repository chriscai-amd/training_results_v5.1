...
void cov(Type *covar, Type *data, const Type *mu, int D, int N, bool sample,
         bool rowMajor, bool stable, hipblasHandle_t handle,
         hipStream_t stream) {
  if (stable) {
    ...
    Type beta = Type(0);
    if (rowMajor) {
      CUBLAS_CHECK(LinAlg::cublasgemm(handle, HIPBLAS_OP_N, HIPBLAS_OP_T, D, D, N,
                                      &alpha, data, D, data, D, &beta, covar, D,
                                      stream));
    } else {
      LinAlg::gemm(data, N, D, data, covar, D, D, HIPBLAS_OP_T, HIPBLAS_OP_N,
