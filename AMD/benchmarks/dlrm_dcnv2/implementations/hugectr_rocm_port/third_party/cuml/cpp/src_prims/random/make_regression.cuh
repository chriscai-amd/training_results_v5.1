...
static void _make_low_rank_matrix(DataT* out, IdxT n_rows, IdxT n_cols,
                                  ...
                                  Rng& r, hipblasHandle_t cublas_handle,
  ...
  CUDA_CHECK(
    hipMemsetAsync(singular_mat.data(), 0, n * n * sizeof(DataT), stream));
  ...
  LinAlg::cublasgemm(cublas_handle, HIPBLAS_OP_N, HIPBLAS_OP_N, n_rows, n, n,
                     &alpha, q0.data(), n_rows, singular_mat.data(), n, &beta,
                     temp_q0s.data(), n_rows, stream);
  LinAlg::cublasgemm(cublas_handle, HIPBLAS_OP_N, HIPBLAS_OP_T, n_rows, n_cols, n,
...
void make_regression(DataT* out, DataT* values, IdxT n_rows, IdxT n_cols,
                     IdxT n_informative, hipblasHandle_t cublas_handle,
                     hipsolverHandle_t cusolver_handle,
                     std::shared_ptr<deviceAllocator> allocator,
                     hipStream_t stream, DataT* coef = nullptr,
  ...
  CUBLAS_CHECK(LinAlg::cublasgemm(
    cublas_handle, HIPBLAS_OP_T, HIPBLAS_OP_T, n_rows, n_targets, n_informative,
