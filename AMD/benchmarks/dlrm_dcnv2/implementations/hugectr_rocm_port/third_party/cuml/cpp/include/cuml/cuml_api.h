...
typedef hipError_t (*cuml_allocate)(void** p, size_t n, hipStream_t stream);
typedef hipError_t (*cuml_deallocate)(void* p, size_t n, hipStream_t stream);
 ...
 * @return CUML_SUCCESS on success, @todo: add more error codes
 */
cumlError_t cumlSetStream(cumlHandle_t handle, hipStream_t stream);
/**
 ...
 * @return CUML_SUCCESS on success, @todo: add more error codes
 */
cumlError_t cumlGetStream(cumlHandle_t handle, hipStream_t* stream);
 ...
 * Example use:
 * @code{.c}
 * hipError_t device_allocate(void** p,size_t n, hipStream_t)
 * {
 *     return hipMalloc(p,n);
 * }
 * 
 * hipError_t device_deallocate(void* p, size_t, hipStream_t)
 * {
 *     return hipFree(p);
 * }
 * 
 ...
 * Example use:
 * @code{.c}
 * hipError_t host_allocate(void** p,size_t n, hipStream_t)
 * {
 *     *p = malloc(n);
 *     return NULL != *p ? hipSuccess : hipErrorUnknown;
 * }
 * 
 * hipError_t host_deallocate(void* p, size_t, hipStream_t stream)
 * {
 *     free(p);
 *     return hipSuccess;
 * }
