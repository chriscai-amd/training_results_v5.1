...
void countLabels(LabelT *labels, DataT *binCountArray, int nRows,
  ...
  CUDA_CHECK(hipcub::DeviceHistogram::HistogramEven(
...
DataT silhouetteScore(DataT *X_in, int nRows, int nCols, LabelT *labels,
                      ...
                      std::shared_ptr<MLCommon::deviceAllocator> allocator,
                      hipStream_t stream, int metric = 4) {
  ...
  CUDA_CHECK(
    hipMemsetAsync(perSampleSilScore, 0, nRows * sizeof(DataT), stream));

  //getting the sample count per cluster
  MLCommon::device_buffer<DataT> binCountArray(allocator, stream, nLabels);
  CUDA_CHECK(
    hipMemsetAsync(binCountArray.data(), 0, nLabels * sizeof(DataT), stream));
  ...
  device_buffer<DataT> sampleToClusterSumOfDistances(allocator, stream,
                                                     nRows * nLabels);
  CUDA_CHECK(hipMemsetAsync(sampleToClusterSumOfDistances.data(), 0,
  ...
  CUDA_CHECK(
    hipMemsetAsync(d_aArray.data(), 0, nRows * sizeof(DataT), stream));
  CUDA_CHECK(
    hipMemsetAsync(d_bArray.data(), 0, nRows * sizeof(DataT), stream));
  ...
  device_buffer<DataT> averageDistanceBetweenSampleAndCluster(allocator, stream,
                                                              nRows * nLabels);
  CUDA_CHECK(hipMemsetAsync(averageDistanceBetweenSampleAndCluster.data(), 0,
                             nRows * nLabels * sizeof(DataT), stream));
  ...
  CUDA_CHECK(
    hipMemsetAsync(d_avgSilhouetteScore.data(), 0, sizeof(DataT), stream));
