...
dim3 computeGridDim(IdxT nrows, IdxT ncols, const void* kernel) {
  int occupancy;
  CUDA_CHECK(hipOccupancyMaxActiveBlocksPerMultiprocessor(&occupancy, kernel,
                                                           ThreadsPerBlock, 0));
...
void gmemHist(int* bins, IdxT nbins, const DataT* data, IdxT nrows, IdxT ncols,
              BinnerOp binner, hipStream_t stream) {
...
void smemBitsHist(int* bins, IdxT nbins, const DataT* data, IdxT nrows,
                  IdxT ncols, BinnerOp binner, hipStream_t stream) {
  typedef BitsInfo<BIN_BITS> Bits;
  auto blks = computeGridDim<IdxT, VecLen>(
    nrows, ncols,
    (const void*)
...
void histogramVecLen(HistType type, int* bins, IdxT nbins, const DataT* data,
                     IdxT nrows, IdxT ncols, hipStream_t stream,
                     BinnerOp binner) {
  CUDA_CHECK(hipMemsetAsync(bins, 0, ncols * nbins * sizeof(int), stream));
  switch (type) {
    case HistTypeGmem:
