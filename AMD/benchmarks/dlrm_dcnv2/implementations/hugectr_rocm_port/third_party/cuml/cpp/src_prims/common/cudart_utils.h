...
inline int getSharedMemPerBlock() {
  ...
  int smemPerBlk;
  CUDA_CHECK(hipDeviceGetAttribute(&smemPerBlk,
                                    hipDeviceAttributeMaxSharedMemoryPerBlock, devId));
  return smemPerBlk;
}
...
void copy(Type* dst, const Type* src, size_t len, hipStream_t stream) {
  CUDA_CHECK(
    hipMemcpyAsync(dst, src, len * sizeof(Type), hipMemcpyDefault, stream));
}
...
void copyAsync(Type* dPtr1, const Type* dPtr2, size_t len,
               hipStream_t stream) {
  CUDA_CHECK(hipMemcpyAsync(dPtr1, dPtr2, len * sizeof(Type),
...
std::string arr2Str(const T* arr, int size, std::string name,
                    hipStream_t stream, int width = 4) {
  std::stringstream ss;
...
void allocate(Type*& ptr, size_t len, bool setZero = false) {
  CUDA_CHECK(hipMalloc((void**)&ptr, sizeof(Type) * len));
  if (setZero) CUDA_CHECK(hipMemset(ptr, 0, sizeof(Type) * len));
}
...
void myPrintDevVector(const char* variableName, const T* devMem,
  ...
  CUDA_CHECK(hipMemcpy(hostMem, devMem, componentsCount * sizeof(T),
