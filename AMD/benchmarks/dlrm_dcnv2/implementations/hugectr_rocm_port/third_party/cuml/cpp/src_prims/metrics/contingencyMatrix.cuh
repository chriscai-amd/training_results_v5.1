...
void computeCMatWAtomics(const T *groundTruth, const T *predictedLabel,
                         ...
                         int outDimN, hipStream_t stream) {
  CUDA_CHECK(hipFuncSetCacheConfig(reinterpret_cast<const void*>(devConstructContingencyMatrix<T), OutT>,
...
ContingencyMatrixImplType getImplVersion(OutT outDimN) {
  ...
  CUDA_CHECK(
    hipDeviceGetAttribute(&l2CacheSize, hipDeviceAttributeL2CacheSize, currDevice));
...
void getInputClassCardinality(const T *groundTruth, const int nSamples,
                              hipStream_t stream, T &minLabel, T &maxLabel) {
...
size_t getContingencyMatrixWorkspaceSize(
  int nSamples, const T *groundTruth, hipStream_t stream,
  ...
  if (implVersion == SORT_AND_GATOMICS) {
    ...
    CUDA_CHECK(hipcub::DeviceRadixSort::SortPairs(pWorkspaceCub, tmpStorageBytes,
...
void contingencyMatrix(const T *groundTruth, const T *predictedLabel,
                       int nSamples, OutT *outMat, hipStream_t stream,
  ...
  CUDA_CHECK(
    hipMemsetAsync(outMat, 0, sizeof(OutT) * outDimM_N * outDimM_N, stream));
