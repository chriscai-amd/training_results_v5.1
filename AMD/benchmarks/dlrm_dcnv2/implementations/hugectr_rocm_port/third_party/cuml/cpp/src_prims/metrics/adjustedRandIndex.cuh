...
double computeAdjustedRandIndex(const T* firstClusterArray,
  ...
  CUDA_CHECK(hipMemsetAsync(dContingencyMatrix.data(), 0,
  ...
  CUDA_CHECK(
    hipMemsetAsync(a.data(), 0, nUniqClasses * sizeof(MathT), stream));
  CUDA_CHECK(
    hipMemsetAsync(b.data(), 0, nUniqClasses * sizeof(MathT), stream));
  CUDA_CHECK(hipMemsetAsync(d_aCTwoSum.data(), 0, sizeof(MathT), stream));
  CUDA_CHECK(hipMemsetAsync(d_bCTwoSum.data(), 0, sizeof(MathT), stream));
  CUDA_CHECK(hipMemsetAsync(d_nChooseTwoSum.data(), 0, sizeof(MathT), stream));
