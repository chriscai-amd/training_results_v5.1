...
class Rng {
 public:
  ...
  void normalTable(Type *ptr, LenType n_rows, LenType n_cols, const Type *mu,
                   const Type *sigma_vec, Type sigma, hipStream_t stream) {
    rand2Impl(
   ...
   */
  template <typename Type, typename LenType = int>
  void fill(Type *ptr, LenType len, Type val, hipStream_t stream) {
   ...
   */
  template <typename Type, typename LenType = int>
  void gumbel(Type *ptr, LenType len, Type mu, Type beta, hipStream_t stream) {
    custom_distribution(
      ptr, len,
      [=] __device__(Type val, LenType idx) {
   ...
   */
  template <typename Type, typename LenType = int>
  void exponential(Type *ptr, LenType len, Type lambda, hipStream_t stream) {
    custom_distribution(
      ptr, len,
      [=] __device__(Type val, LenType idx) {
        constexpr Type one = (Type)1.0;
   ...
   */
  template <typename Type, typename LenType = int>
  void rayleigh(Type *ptr, LenType len, Type sigma, hipStream_t stream) {
    custom_distribution(
      ptr, len,
      [=] __device__(Type val, LenType idx) {
        constexpr Type one = (Type)1.0;
  ...
  void sampleWithoutReplacement(DataT *out, IdxT *outIdx, const DataT *in,
    ...
    sortPairs(workspace, expWts.data(), sortedWts.data(), inIdxPtr, outIdxPtr,
              (int)len, stream);
    if (outIdx != nullptr) {
      CUDA_CHECK(hipMemcpyAsync(outIdx, outIdxPtr, sizeof(IdxT) * sampledLen,
 ...
 private:
  ...
  GeneratorType type;
  /**
   * offset is also used to initialize hiprand state.
  ...
  uint64_t _setupSeeds(uint64_t &seed, uint64_t &offset, LenType len,
    ...
    // hiprand uses 2 32b uint's to generate one double
