...
void rsvdFixedRank(math_t *M, int n_rows, int n_cols, math_t *&S_vec,
                   ...
                   math_t tol, int max_sweeps, hipsolverHandle_t cusolverH,
                   hipblasHandle_t cublasH, hipStream_t stream,
  ...
  CUDA_CHECK(hipMemsetAsync(S_vec_tmp.data(), 0, sizeof(math_t) * l, stream));
  ...
  gemm(M, m, n, RN.data(), Y.data(), m, l, HIPBLAS_OP_N, HIPBLAS_OP_N, alpha,
       beta, cublasH, stream);
  ...
  CUDA_CHECK(hipMemsetAsync(Z.data(), 0, sizeof(math_t) * n * l, stream));
  CUDA_CHECK(hipMemsetAsync(Yorth.data(), 0, sizeof(math_t) * m * l, stream));
  CUDA_CHECK(hipMemsetAsync(Zorth.data(), 0, sizeof(math_t) * n * l, stream));

  // power sampling scheme
  for (int j = 1; j < q; j++) {
    if ((2 * j - 2) % s == 0) {
      qrGetQ(Y.data(), Yorth.data(), m, l, cusolverH, stream, allocator);
      gemm(M, m, n, Yorth.data(), Z.data(), n, l, HIPBLAS_OP_T, HIPBLAS_OP_N,
           alpha, beta, cublasH, stream);
    } else {
      gemm(M, m, n, Y.data(), Z.data(), n, l, HIPBLAS_OP_T, HIPBLAS_OP_N, alpha,
           beta, cublasH, stream);
    }

    if ((2 * j - 1) % s == 0) {
      qrGetQ(Z.data(), Zorth.data(), n, l, cusolverH, stream, allocator);
      gemm(M, m, n, Zorth.data(), Y.data(), m, l, HIPBLAS_OP_N, HIPBLAS_OP_N,
           alpha, beta, cublasH, stream);
    } else {
      gemm(M, m, n, Z.data(), Y.data(), m, l, HIPBLAS_OP_N, HIPBLAS_OP_N, alpha,
           beta, cublasH, stream);
    }
  }
  ...
  CUDA_CHECK(hipMemsetAsync(Q.data(), 0, sizeof(math_t) * m * l, stream));
  ...
  if (!use_bbt) {
    ...
    CUDA_CHECK(hipMemsetAsync(Bt.data(), 0, sizeof(math_t) * n * l, stream));
    gemm(M, m, n, Q.data(), Bt.data(), n, l, HIPBLAS_OP_T, HIPBLAS_OP_N, alpha,
         beta, cublasH, stream);

    // compute QR factorization of Bt
    // M is mxn ; Q is mxn ; R is min(m,n) x min(m,n) */
    device_buffer<math_t> Qhat(allocator, stream, n * l);
    CUDA_CHECK(hipMemsetAsync(Qhat.data(), 0, sizeof(math_t) * n * l, stream));
    device_buffer<math_t> Rhat(allocator, stream, l * l);
    CUDA_CHECK(hipMemsetAsync(Rhat.data(), 0, sizeof(math_t) * l * l, stream));
    ...
    CUDA_CHECK(hipMemsetAsync(Uhat.data(), 0, sizeof(math_t) * l * l, stream));
    device_buffer<math_t> Vhat(allocator, stream, l * l);
    CUDA_CHECK(hipMemsetAsync(Vhat.data(), 0, sizeof(math_t) * l * l, stream));
    if (use_jacobi)
    ...
    if (gen_left_vec) {
      gemm(Q.data(), m, l, Vhat.data(), U, m,
           k /*used to be l and needs slicing*/, HIPBLAS_OP_N, HIPBLAS_OP_N,
           alpha, beta, cublasH, stream);
    }
    ...
    gemm(Q.data(), m, l, M, B.data(), l, n, HIPBLAS_OP_T, HIPBLAS_OP_N, alpha,
         beta, cublasH, stream);

    device_buffer<math_t> BBt(allocator, stream, l * l);
    gemm(B.data(), l, n, B.data(), BBt.data(), l, l, HIPBLAS_OP_N, HIPBLAS_OP_T,
         alpha, beta, cublasH, stream);
    ...
    CUDA_CHECK(
      hipMemsetAsync(Uhat_dup.data(), 0, sizeof(math_t) * l * l, stream));
    ...
    if (gen_left_vec) {
      gemm(Q.data(), m, l, Uhat.data() + p * l, U, m, k, HIPBLAS_OP_N,
           HIPBLAS_OP_N, alpha, beta, cublasH, stream);
    ...
    if (gen_right_vec) {
      ...
      CUDA_CHECK(
        hipMemsetAsync(Sinv.data(), 0, sizeof(math_t) * k * k, stream));
      device_buffer<math_t> UhatSinv(allocator, stream, l * k);
      CUDA_CHECK(
        hipMemsetAsync(UhatSinv.data(), 0, sizeof(math_t) * l * k, stream));
      ...
      gemm(Uhat.data() + p * l, l, k, Sinv.data(), UhatSinv.data(), l, k,
           HIPBLAS_OP_N, HIPBLAS_OP_N, alpha, beta, cublasH, stream);
      gemm(B.data(), l, n, UhatSinv.data(), V, n, k, HIPBLAS_OP_T, HIPBLAS_OP_N,
           alpha, beta, cublasH, stream);
