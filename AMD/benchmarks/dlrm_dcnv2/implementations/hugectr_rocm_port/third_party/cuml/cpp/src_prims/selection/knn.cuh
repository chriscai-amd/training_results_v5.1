...
inline void knn_merge_parts(float *inK, int64_t *inV, float *outK,
                            ...
                            hipStream_t stream, int64_t *translations) {
  if (k == 1)
...
void brute_force_knn(std::vector<float *> &input, std::vector<int> &sizes,
                     ...
                     hipStream_t *internalStreams = nullptr,
  ...
  if (n_int_streams > 0) CUDA_CHECK(hipStreamSynchronize(userStream));
  ...
  for (int i = 0; i < input.size(); i++) {
     ...
     * avoiding device-wide synchronizations from hipMalloc/hipFree
     */
    bfKnn(&gpu_res, args);
  ...
  for (int i = 0; i < n_int_streams; i++) {
    CUDA_CHECK(hipStreamSynchronize(internalStreams[i]));
  }
...
void class_probs(std::vector<float *> &out, const int64_t *knn_indices,
                 ...
                 hipStream_t user_stream, hipStream_t *int_streams = nullptr,
                 int n_int_streams = 0) {
  for (int i = 0; i < y.size(); i++) {
    ...
    CUDA_CHECK(hipMemsetAsync(out[i], 0, cur_size * sizeof(float), stream));
...
void knn_regress(ValType *out, const int64_t *knn_indices,
                 ...
                 size_t n_query_rows, int k, hipStream_t user_stream,
                 hipStream_t *int_streams = nullptr, int n_int_streams = 0) {
  /**
