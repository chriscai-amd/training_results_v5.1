...
class GramMatrixBase {
 ...
 public:
  GramMatrixBase(hipblasHandle_t cublas_handle) : cublas_handle(cublas_handle){};
  ...
  */
  virtual void operator()(const math_t *x1, int n1, int n_cols,
                          ...
                          hipStream_t stream, int ld1 = 0, int ld2 = 0,
                          int ld_out = 0) {
    if (ld1 <= 0) {
      ld1 = n1;
    }
  ...
  */
  virtual void evaluate(const math_t *x1, int n1, int n_cols, const math_t *x2,
                        int n2, math_t *out, hipStream_t stream, int ld1,
                        int ld2, int ld_out) {
  ...
  void linear(const math_t *x1, int n1, int n_cols, const math_t *x2, int n2,
              math_t *out, hipStream_t stream, int ld1, int ld2, int ld_out) {
    math_t alpha = 1.0;
    math_t beta = 0.0;
    CUBLAS_CHECK(LinAlg::cublasgemm(cublas_handle, HIPBLAS_OP_N, HIPBLAS_OP_T, n1,
