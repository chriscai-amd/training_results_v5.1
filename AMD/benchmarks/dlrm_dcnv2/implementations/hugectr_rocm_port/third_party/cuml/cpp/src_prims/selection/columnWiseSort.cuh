...
struct SmemPerBlock {
  typedef hipcub::BlockLoad<InType, BLOCK_SIZE, ITEMS_PER_THREAD,
                         hipcub::BLOCK_LOAD_WARP_TRANSPOSE>
    BlockLoadTypeKey;

  typedef hipcub::BlockRadixSort<InType, BLOCK_SIZE, ITEMS_PER_THREAD, OutType>
    BlockRadixSortType;
...
__global__ void __launch_bounds__(1024, 1)
  ...
  hipcub::StoreDirectStriped<BLOCK_SIZE>(threadIdx.x, inputVals + blockOffset,
                                      threadValues, n_cols);

  if (outputKeys) {
    hipcub::StoreDirectStriped<BLOCK_SIZE>(threadIdx.x, outputKeys + blockOffset,
                                        threadKeys, n_cols);
  }
}
...
hipError_t layoutIdx(OutType *in, int n_rows, int n_columns,
                      hipStream_t stream) {
...
void sortColumnsPerRow(const InType *in, OutType *out, int n_rows,
  ...
  // CUDA_CHECK(hipGetDevice(&currDevice));
  // CUDA_CHECK(hipDeviceGetAttribute(&smemLimit, hipDeviceAttributeMaxSharedMemoryPerBlock, currDevice));
  ...
  if (dtypeToColumnMap.count(perElementSmemUsage) != 0 &&
    ...
    if (workspaceSize == 0 || !workspacePtr) {
      ...
      CUDA_CHECK(hipcub::DeviceSegmentedRadixSort::SortPairs(
    ...
    if (workspaceSize == 0 || !workspacePtr) {
      ...
      for (int i = 0; i < n_rows; i++) {
        ...
        CUDA_CHECK(hipcub::DeviceRadixSort::SortPairs(workspacePtr, workspaceSize,
