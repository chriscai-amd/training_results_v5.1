...
class COO {
 ...
 public:
  ...
  /**
    ...
    */
  COO(std::shared_ptr<deviceAllocator> d_alloc, hipStream_t stream)
  ...
  void init_arrays(hipStream_t stream) {
    CUDA_CHECK(hipMemsetAsync(this->rows_arr.data(), 0,
                               this->nnz * sizeof(Index_Type), stream));
    CUDA_CHECK(hipMemsetAsync(this->cols_arr.data(), 0,
                               this->nnz * sizeof(Index_Type), stream));
    CUDA_CHECK(
      hipMemsetAsync(this->vals_arr.data(), 0, this->nnz * sizeof(T), stream));
  }
  ...
  /**
    ...
    * @param stream: CUDA stream to use
    */
  void allocate(int nnz, bool init, hipStream_t stream) {
...
void coo_sort(int m, int n, int nnz, int *rows, int *cols, T *vals,
              std::shared_ptr<deviceAllocator> d_alloc, hipStream_t stream) {
  ...
  CUSPARSE_CHECK(hipsparseXcoosort_bufferSizeExt(handle, m, n, nnz, rows, cols,
                                                &pBufferSizeInBytes));

  device_buffer<int> d_P(d_alloc, stream, nnz);
  device_buffer<char> pBuffer(d_alloc, stream, pBufferSizeInBytes);

  CUSPARSE_CHECK(hipsparseCreateIdentityPermutation(handle, nnz, d_P.data()));

  CUSPARSE_CHECK(hipsparseXcoosortByRow(handle, m, n, nnz, rows, cols,
 ...
 */
template <int TPB_X, typename T>
void coo_row_count(COO<T> *in, int *results, hipStream_t stream) {
...
void coo_row_count_scalar(const int *rows, const T *vals, int nnz, T scalar,
                          int *results, hipStream_t stream = 0) {
  dim3 grid_rc(MLCommon::ceildiv(nnz, TPB_X), 1, 1);
  dim3 blk_rc(TPB_X, 1, 1);
 ...
 */
template <int TPB_X, typename T>
void coo_row_count_nz(COO<T> *in, int *results, hipStream_t stream) {
  dim3 grid_rc(MLCommon::ceildiv(in->nnz, TPB_X), 1, 1);
  dim3 blk_rc(TPB_X, 1, 1);
...
void coo_remove_scalar(const int *rows, const int *cols, const T *vals, int nnz,
  ...
  CUDA_CHECK(hipMemsetAsync(ex_scan.data(), 0, n * sizeof(int), stream));
  CUDA_CHECK(hipMemsetAsync(cur_ex_scan.data(), 0, n * sizeof(int), stream));
...
void coo_remove_scalar(COO<T> *in, COO<T> *out, T scalar,
  ...
  CUDA_CHECK(
    hipMemsetAsync(row_count_nz.data(), 0, in->n_rows * sizeof(int), stream));
  CUDA_CHECK(
    hipMemsetAsync(row_count.data(), 0, in->n_rows * sizeof(int), stream));
...
void sorted_coo_to_csr(const T *rows, int nnz, T *row_ind, int m,
  ...
  CUDA_CHECK(hipMemsetAsync(row_counts.data(), 0, m * sizeof(T), stream));
...
void from_knn_symmetrize_matrix(const long *restrict knn_indices,
  ...
  CUDA_CHECK(hipMemsetAsync(row_sizes.data(), 0, sizeof(int) * n, stream));

  device_buffer<int> row_sizes2(d_alloc, stream, n);
  CUDA_CHECK(hipMemsetAsync(row_sizes2.data(), 0, sizeof(int) * n, stream));
