...
template <typename Type, typename MapOp, int TPB, typename... Args>
void mapThenSumReduceImpl(Type *out, size_t len, MapOp map, hipStream_t stream,
                          const Type *in, Args... args) {
  CUDA_CHECK(hipMemsetAsync(out, 0, sizeof(Type), stream));
...
void mapThenSumReduce(Type *out, size_t len, MapOp map, hipStream_t stream,
                      const Type *in, Args... args) {
