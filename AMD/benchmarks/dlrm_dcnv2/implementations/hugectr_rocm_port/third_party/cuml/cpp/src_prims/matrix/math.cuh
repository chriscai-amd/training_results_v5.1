...
void seqRoot(math_t *in, math_t *out, math_t scalar, IdxType len,
             hipStream_t stream, bool set_neg_zero = false) {
  auto d_src = in;
  auto d_dest = out;
 ...
 */
template <typename math_t, typename IdxType = int>
void seqRoot(math_t *inout, math_t scalar, IdxType len, hipStream_t stream,
...
void setSmallValuesZero(math_t *out, const math_t *in, IdxType len,
                        hipStream_t stream, math_t thres = 1e-15) {
  MLCommon::LinAlg::unaryOp(
    out, in, len,
    [=] __device__(math_t a) {
      if (a <= thres && -a <= thres) {
 ...
 */
template <typename math_t, typename IdxType = int>
void setSmallValuesZero(math_t *inout, IdxType len, hipStream_t stream,
                        math_t thres = 1e-15) {
...
void reciprocal(math_t *in, math_t *out, math_t scalar, int len,
                hipStream_t stream, bool setzero = false,
                math_t thres = 1e-15) {
  auto d_src = in;
  auto d_dest = out;
 ...
 */
template <typename math_t, typename IdxType = int>
void reciprocal(math_t *inout, math_t scalar, IdxType len, hipStream_t stream,
...
__global__ void argmaxKernel(const T *d_in, int D, int N, T *argmax) {
  ...
  for (int i = threadIdx.x; i < D; i += TPB) {
    int idx = rowStart + i;
    thread_data = hipcub::ArgMax()(thread_data, KVP(i, d_in[idx]));
  }
