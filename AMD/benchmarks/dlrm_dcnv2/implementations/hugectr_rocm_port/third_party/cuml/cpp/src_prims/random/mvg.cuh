...
class MultiVarGaussian {
 ...
 public:  // functions
  ...
  size_t init(hipblasHandle_t cublasH, hipsolverHandle_t cusolverH,
              hipStream_t stream) {
  ...
  void give_gaussian(const int nPoints, T *P, T *X, const T *x = 0) {
    ...
    CUDA_CHECK(hipStreamSynchronize(cudaStream));
    ...
    if (method == chol_decomp) {
      ...
      CUBLAS_CHECK(LinAlg::cublasgemm(cublasHandle, HIPBLAS_OP_N, HIPBLAS_OP_N,
      ...
      CUDA_CHECK(hipMemsetAsync(info, 0, sizeof(int), cudaStream));
