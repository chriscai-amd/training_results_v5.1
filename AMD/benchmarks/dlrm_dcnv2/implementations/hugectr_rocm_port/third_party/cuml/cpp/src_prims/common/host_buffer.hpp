 ...
 * @code{.cpp}
 * template<typename T>
 * void foo( const cumlHandle_impl& h, const T* in_d , T* out_d, ..., hipStream_t stream )
 * {
 *     ...
 *     host_buffer<T> temp( handle->getHostAllocator(), stream, 0 )
 *     
 *     temp.resize(n, stream);
 *     hipMemcpyAsync( temp.data(), in_d, temp.size()*sizeof(T), hipMemcpyDeviceToHost );
 *     ...
 *     hipMemcpyAsync( out_d, temp.data(), temp.size()*sizeof(T), hipMemcpyHostToDevice );
 *     temp.release(stream);
 * }
 * @endcode
...
class host_buffer : public buffer_base<T, hostAllocator> {
 public:
  ...
  host_buffer(std::shared_ptr<hostAllocator> allocator, hipStream_t stream,
              size_type n = 0)
