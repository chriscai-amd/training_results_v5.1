...
void getUniqueLabels(math_t *y, size_t n, math_t **y_unique, int *n_unique,
  ...
  // and allocate it
  hipcub::DeviceRadixSort::SortKeys(NULL, bytes, y, y2.data(), n);
  hipcub::DeviceSelect::Unique(NULL, bytes2, y2.data(), y3.data(),
                            d_num_selected.data(), n);
  bytes = max(bytes, bytes2);
  device_buffer<char> cub_storage(allocator, stream, bytes);

  // Select Unique classes
  hipcub::DeviceRadixSort::SortKeys(cub_storage.data(), bytes, y, y2.data(), n);
  hipcub::DeviceSelect::Unique(cub_storage.data(), bytes, y2.data(), y3.data(),
                            d_num_selected.data(), n);
...
void getOvrLabels(math_t *y, int n, math_t *y_unique, int n_classes,
                  math_t *y_out, int idx, hipStream_t stream) {
...
void make_monotonic(Type *out, Type *in, size_t N, hipStream_t stream,
                    Lambda filter_op) {
