...
void gemm(hipblasOperation_t transA, hipblasOperation_t transB, Index_ m,
          Index_ n, Index_ k, OType alpha, IType const *A, Index_ lda,
          IType const *B, Index_ ldb, OType beta, OType const *C, Index_ ldc,
          OType *D, Lambda op, FinalLambda fin_op, hipStream_t stream) {
 ...
 * @param alpha scalar
 * @param beta scalar
 * @param hipblas.h cublas handle
 * @param stream cuda stream
 */
template <typename math_t>
void gemm(const math_t *a, int n_rows_a, int n_cols_a, const math_t *b,
          math_t *c, int n_rows_c, int n_cols_c, hipblasOperation_t trans_a,
          hipblasOperation_t trans_b, math_t alpha, math_t beta,
          hipblasHandle_t hipblas.h, hipStream_t stream) {
  int m = n_rows_c;
  int n = n_cols_c;
  int k = trans_a == HIPBLAS_OP_T ? n_rows_a : n_cols_a;
  int lda = trans_a == HIPBLAS_OP_T ? k : m;
  int ldb = trans_b == HIPBLAS_OP_T ? n : k;
  int ldc = m;
 ...
 */
template <typename T>
void gemm(hipblasHandle_t handle, T *z, T *x, T *y, int _M, int _N, int _K,
          bool isZColMajor, bool isXColMajor, bool isYColMajor,
          hipStream_t stream, T alpha = T(1.0), T beta = T(0.0)) {
  ...
  if (isZColMajor == true) {
    ...
    // therefore trans_x needs to be HIPBLAS_OP_T. If x is in column major
    // layout, trans_b needs to be HIPBLAS_OP_N.
    trans_a = isXColMajor == true ? HIPBLAS_OP_N : HIPBLAS_OP_T;
    ...
    // therefore trans_x needs to be HIPBLAS_OP_T. If x is in column major
    // layout, trans_b needs to be HIPBLAS_OP_N.
    trans_b = isYColMajor == true ? HIPBLAS_OP_N : HIPBLAS_OP_T;
    ...
    // major layout. Therefore we can pass trans_a as HIPBLAS_OP_N. If y is in
    // column major layout, cublas needs to transpose y first, therefore
    // trans_a needs to be HIPBLAS_OP_T
    trans_a = isYColMajor == true ? HIPBLAS_OP_T : HIPBLAS_OP_N;
    // Set leading dimension appropriately
    lda = isYColMajor == true ? _K : _N;

    b = x;
    // If x is in row major layout, it can be interpreted as tr(x) on column
    // major layout. Therefore we can pass trans_b as HIPBLAS_OP_N. If x is in
    // column major layout, cublas needs to trasponse x first, therefore
    // trans_b needs to be HIPBLAS_OP_T
    trans_b = isXColMajor == true ? HIPBLAS_OP_T : HIPBLAS_OP_N;
    // Set leading dimension appropriately
