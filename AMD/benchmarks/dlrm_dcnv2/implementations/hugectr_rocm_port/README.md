...
**bs=1× post-Phase-16 (dedicated RCCL stream + dx-memset elim, default-ON wins only): 13.67 M sps** = 51.9 % of NV B200's bs=1× (26.33 M sps). Phase 17 (RCCL stream priority -1) is opt-in only due to intermittent first-iter divergence — see Phase 17 row.

| Batch (global) | Per-GPU | M sps (default-ON wins only) | vs Phase-13 | vs NV B200 May-13 (same batch) |
|---:|---:|---:|---:|---:|
| 55,296 (1×, MLPerf-spec) | 6,912 | **13.67** | +1.3 % (BUFFSIZE) +3.0 % (Phase-15) +1.5 % (Phase-16 + measurement variance) | 51.9 % of 26.33 |
| 55,296 with `HCTR_RCCL_STREAM_PRIORITY=-1` (opt-in) | 6,912 | ~13.77 (when run converges; ~33 % first-iter divergence rate) | + further +0.7-1.8 % | up to 52.3 % of 26.33 |
