...
| ~~19~~ | 2026-05-15 | **REVERTED twice — "match NV's dedicated memcpy lane" doesn't translate to AMD.** First attempt patched dead `embedding::tf::swizzle_key` (TF plugin, never called from HCTR pipeline) — silent no-op + +0.6-0.9 % "lift" was noise. Re-audit of the raw rocprofv3 csv (ignoring the converter's filtered view) found the real hot d2d site: `EmbeddingCollection::cache_ddl_output` (~22 d2d copies/iter on computation_stream, ~110 µs / 2.5 % of compute_stream busy at bs=1×). Second attempt routed those through `GPUResource::memcpy_stream_` with proper event sync. Trace confirmed mechanically correct (compute_stream d2d count dropped 185 → 95, NEW memcpy_stream lane appeared with 1578 d2d ops, +240 hipEventRecord API calls). **But the perf result was −7.3 % regression** (baseline 14.395 → phase19b 13.341 M sps, loss bit-equivalent). Root cause: cache_ddl_output's destinations are immediate inputs to `ebc_mp_model_forward` on the SAME compute_stream — moving the copies cross-stream gains zero parallelism (consumer waits for them anyway) but adds 240 graph FORK/JOIN sync ops/iter. **Conclusion: NV's "memcpy lane" is a cuBLAS/cuDNN-internal worker-stream artifact, not replicable from the application layer on AMD where hipBLASLt/MIOpen don't allocate worker streams.** See `perf_push_status.log` for the full post-mortem. | (reverted) | bs1x 14.20 → 14.20 | **0 %** (item retired) |
