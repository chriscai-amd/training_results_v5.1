...
|---:|---|---:|---|---|
| 1 | **ROCm 7.11+ / 7.12 preview container upgrade for HIP Graph Segmented Execution** | **5–15 %** (estimate, untested) | 1–3 days (build TheRock-style image on `rocm/TheRock` 7.12 base + revalidate HCTR build & convergence) | **Direct fix for the 4-stream-vs-10-stream gap** documented in §3.2 / §4.2. Per the [ROCm CLR changelog](https://github.com/ROCm/rocm-systems/blob/develop/projects/clr/CHANGELOG.md) (HIP 7.12 for ROCm 7.12, March 26 2026), HIP runtime adds **HIP Graph Segmented Execution** which "groups graph nodes into segments and dispatches them across multiple GPU streams to enable parallel execution … overlapping independent graph work across streams while preserving correct execution order." HIP 7.11 added the precursor "Improved hipGraphLaunch parallelism for complex data-parallel graphs". HIP 7.1 fixed multi-stream graph capture seg-fault with cross-stream deps. **This is the exact mechanism NV's CUDA graph uses to spread the bprop chain across 10 streams.** Same image likely also resolves the `hipEventWaitExternal` regression (§4.2 row 2) and the hipBLASLt stream-pinning blocker (§4.2 row 1) since both are downstream symptoms of the same single-stream dispatch behavior in 7.2.x. Caveat: ROCm 7.11/7.12 is a *preview* stream separate from the 7.0–7.2 production stream, slated to replace production mid-2026; no in-place upgrade path. Need a fresh container build off `rocm/TheRock 7.12` base. |
| 2 | **Custom RCCL chunking patches (RCCL source-level)** | 5–15 % | 10+ days, RCCL expert | Fundamental gap per §3.2: RCCL emits **3.2 × more device kernels** per logical collective vs NCCL (chunking + protocol overhead — 15.9 vs 5 calls/iter at bs=1×). RCCL chunks every primitive call internally based on `NCCL_PROTO` + message size; host-side `NCCL_BUFFSIZE` / native `ncclAllToAll` API swaps were both flat (see §4.2). Real fix is RCCL source patches — outside this repo's scope. |
| 3 | **int4-vectorized embedding ops** (`update4_kernel` + `multi_to_one_reduce_vec4_v2`) | 5–10 % | 3–5 days | Top embedding kernels consume ~2.1 ms/iter profile (~0.35 ms real = 8 % of bs=1× iter, per §3.2 trace). Already use `vec4` (8-byte int2) loads; bump to `int4` (16-byte) for `__half` slot data — same bounds-check rewrite pattern as Phase 12 concat (which landed +1.5 % at bs=8× via the same lever). Self-contained inside `embedding_storage/ragged_static_embedding.cu`. |
| 4 | **mscclpp xGMI-multicast custom AllReduce** | 1–3 % | 5–10 days | mscclpp source available at `/home/muabdulj/mscclpp/`. Need: (a) build mscclpp on ROCm 7.2 (untested), (b) write GPU-side AllReduce using mscclpp 1-sided put/get over xGMI, (c) integrate into HCTR's `NcclAllReduceInplaceComm` as a fallback path. **Only attacks AllReduce (1 of 17 RCCL calls)**; embedding all-to-all still uses RCCL → bounded upside. |
| 5 | **WarpSpeed via ROCm 7.11+ RCCL** (`RCCL_WARP_SPEED_AUTO=1`) | 1–2 % | 0 days dev (lands free with item #1's container upgrade) | RCCL 2.28+ ships **WarpSpeed** (PR #2073) which halves CU usage for AllReduce / AllGather / ReduceScatter on gfx950. Tested RCCL 2.28.3 develop branch on 2026-05-13 (commit [`2dc79d6`](https://github.com/chriscai-amd/training_results_v5.1/commit/2dc79d6)) — flat: WarpSpeed strings not in develop branch. AllReduce is only 14 % of exposed-RCCL → max +1-2 % even when engaged. |
| 6 | **BF16 mixed precision path** | 1–2 % | medium | NV submission uses BF16 mixed; we use FP16. Need `enable_bf16_compute` flag + `hip_bfloat16` template instantiations across `HugeCTR/src/layers/`. Eliminates loss-scaler stalls. |
| 7 | **Per-GPU NCCL stream affinity tuning** | < 1 % | 2 days HCTR core | `Send` / `Recv` on different streams could expose more concurrency; current HCTR uses single RCCL stream. Limited upside since RCCL kernels are CU-saturating. |
...
Sequence: **item #1 first** (single highest-impact, also unblocks items #5 + the §4.2 multi-stream blockers). Items #2 and #3 in parallel (different code areas, additive). #4–#8 are "if budget remaining" tail.
...
= 1–5 %, **L** = < 1 % or already at ceiling.

| Blocker / hypothesis | Impact if resolved | What we tried | Outcome | Owner / next step |
|---|:---:|---|---|---|
| **HIP graph capture flattens multi-stream dispatch onto the launch stream (hipBLAS pinning is the most-visible symptom)** | **H** | hipblasSetStream-per-call workaround (Phase 14n.6); GraphScheduleable cross-graph pipeline split (Phase 14n.5); restored `hipEventWaitExternal` in pipeline.cpp + async_data_reader.cpp (Phase 14p) | All workarounds fall back to single-stream dispatch under HIP 7.2.x graph capture: hipBLASLt ignores per-call stream binding inside capture, and the External-flag restore crashes with `std::bad_alloc` inside the OMP outlined train loop. Result: 4 active streams on AMD vs 10 on B200; ~50 % of NV throughput at bs=1× | **TRACTABLE: ROCm 7.11+ / 7.12 preview** — HIP 7.12 (March 2026) adds [HIP Graph Segmented Execution](https://github.com/ROCm/rocm-systems/blob/develop/projects/clr/CHANGELOG.md), which "groups graph nodes into segments and dispatches them across multiple GPU streams to enable parallel execution … overlapping independent graph work across streams." HIP 7.11 added the precursor "Improved hipGraphLaunch parallelism for complex data-parallel graphs". HIP 6.4.2 already added `hipEventWaitExternal` flag definition; the missing piece is the runtime cross-graph dispatch which 7.11/7.12 implements. **Test path** = Item #1 in §4.1 (build container off `rocm/TheRock 7.12` base). Caveat: 7.11/7.12 is preview stream, not production; mid-2026 it replaces production. |
| **RCCL per-call latency floor 815 µs vs NV 155 µs (5.3 ×)** | **H** | 15 RCCL knobs swept (`NCCL_P2P_*`, `NCCL_GDR_*`, `NCCL_SHM_*`, `NCCL_ALGO`, `NCCL_CHUNK_SIZE`, `NCCL_*_NCHANNELS`); CU-mask via `hipExtStreamCreateWithCUMask` (Phase 14g) | All within ±0.6 % of baseline. Per-call floor is platform-fundamental: no NVLS-equivalent on xGMI in ROCm 7.2 RCCL. Closing fully would save ~1.57 ms/iter (per §3.2) = up to +37 % bs=1× throughput, but rated H not "very high" because the floor is platform-fundamental and no clear timeline exists | AMD vendor (NVLS-equivalent unannounced for MI400+) |
| **RCCL emits 3.2 × more device kernels per logical collective than NCCL** | **H** | Bumped `NCCL_BUFFSIZE` 8 / 16 / 24 / 32 / 64 / 128 MiB; replaced looped `ncclSend` / `ncclRecv` with native `ncclAllToAll` / `ncclAllToAllv` API (Phase 14e, `HCTR_USE_NCCL_ALLTOALL=1`) | All flat at bs=1×. Trace: 17.4 → 16.5 RCCL kernels/iter (-5 %). Reverted as no-perf-win, added complexity. **RCCL chunks every primitive call internally based on `NCCL_PROTO` + message size** — host-side levers don't change kernel count. Overlaps with previous row (RCCL latency) — fixing chunking alone with current per-call floor still buys ~5–10 % | RCCL upstream (item #2 in §4.1 = real fix) |
| **`hipGraphLaunch` host overhead 5.97 ms p50 vs NV 555 µs virtualized (11 ×)** | **M** | 14 `DEBUG_HIP_GRAPH_*` and `DEBUG_CLR_*` knobs swept at bs=8× | All within ±0.3 % of baseline. ROCm runtime kernel-launch path. Per §3.2 linear-fit, matching NV's host const `c=0.995 ms` saves 0.30 ms/iter = +6 % bs=1× | AMD vendor (ROCm 7.3+ retest, possibly resolved by item #1 in §4.1 segmented execution) |
| **Per-shape hipBLASLt offline tuning at bs=1×** | **M** | `HIPBLASLT_TUNING_OVERRIDE_FILE` deployed | HCTR's `hipblasGemmEx` wrapper bypasses the override path → tunings never apply. Would need direct `hipblasLtMatmul` refactor. Per §3.2 the bs=1× MLP-GEMM gap is +0.82 ms (38 % of iter gap); per-shape tuning closes ~30 % of that = +5 % bs=1× | 3–5 day refactor, deferred behind higher-impact §4.1 items |
| **CK-Tile MLP backward dgrad as bs=1× win** | **L** | Implemented end-to-end CK-Tile dgrad path (Phase 14p.2): new `hctr_cktile_gemm_dgrad_fp16` entry point, zero-bias-buffer workaround for upstream `Ds tuple > 0` static_assert, init API called pre-graph-capture, mlp_layer.cu integration with hipblas fallback. Bit-exact validated against CPU reference + hipBLAS | bs=1× **+0.3 % (noise)**; bs=8× **−24.9 % regression** (CK-Tile fixed 128×128×32 tile is wrong shape vs hipBLASLt's per-shape picks at large M). Code kept gated default-OFF as scaffold | Would need CK-Tile multi-tile dispatch OR fused bwd megakernel (dgrad + wgrad + bgrad + dRELU in one MFMA kernel) — multi-week effort |
| **MSCCLPP custom AllReduce (HCTR-internal)** | **L** | Built RCCL 2.27.7 with `-DENABLE_MSCCLPP=ON` (Phase 14h, commit [`2dc79d6`](https://github.com/chriscai-amd/training_results_v5.1/commit/2dc79d6)) | **−2.5 % regression**. MSCCLPP only optimizes AllReduce (14 % of exposed RCCL); setup overhead exceeds savings → bounded by AllReduce share even if successful | (use mainline RCCL; revisit if RCCL exposes mscclpp-on-demand path) |
