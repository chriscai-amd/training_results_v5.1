...
| **17** | **2026-05-15** | **Higher-priority RCCL streams** (HIP stream priority `-1` for `rccl_emb_ar` + `rccl_mlp_wgrad`). NV implicitly gets this because cuBLASLt-internal worker streams are spawned at higher priority than user streams, so collectives never starve behind compute. HIP/HCTR doesn't get that for free, so we set it explicitly via `set_absolute_stream(name, -1)`. 2-trial bs=1× A/B at p=-1 vs p=0: 13.725 vs 13.667 (+0.4 %) and 13.816 vs 13.626 (+1.4 %) → +1.8 % avg lift over Item 1+2 baseline. Loss bit-equivalent (0.291638 vs 0.291672). Tested p=-2 → training diverges (loss cannot converge); MI350X HIP min priority is -1 (p=-5 clamps to same). Default ON; `HCTR_RCCL_STREAM_PRIORITY=0` to disable. | (perf-push branch, src/pybind/model_pipeline.cpp) | bs1x 13.55 → **13.77** | **+1.6 % @ bs1x** |

**Peak result post-Phase-14: 17.93 M sps at bs8x (442,368)** = 54.7 % of NV B200's May-13 auto+tmpfs bs8x peak (32.77 M sps).
**bs=1× post-Phase-17 (dedicated RCCL stream + dx-memset elim + RCCL stream priority): 13.77 M sps** = 52.3 % of NV B200's bs=1× (26.33 M sps).

| Batch (global) | Per-GPU | M sps (post-Phase-17) | vs Phase-13 | vs NV B200 May-13 (same batch) |
|---:|---:|---:|---:|---:|
| 55,296 (1×, MLPerf-spec) | 6,912 | **13.77** | +1.3 % (BUFFSIZE) +3.0 % (Phase-15) +0.7 % (Phase-16) +1.6 % (Phase-17) | 52.3 % of 26.33 |
